;;; piem-grammars.el --- Tree-sitter grammar recipes  -*- lexical-binding: t; -*-
...
(defconst piem-essential-grammar-recipes
...
(defconst piem-optional-grammar-recipes
...
(defconst piem-grammar-recipes
  (append piem-essential-grammar-recipes
          piem-optional-grammar-recipes)
  "All tree-sitter grammar recipes needed by piem.")
...
(defun piem--markdown-grammar-incompatible-p ()
  "Return non-nil when an installed Markdown grammar is incompatible."
  (and (treesit-language-available-p 'markdown)
       (not (piem--markdown-grammar-compatible-p))))

(defun piem--incompatible-markdown-grammar-message ()
 ...
 system packages, then restart Emacs or run M-x piem-install-grammars."
          (abbreviate-file-name (locate-user-emacs-file "tree-sitter"))))

(defvar piem--markdown-grammar-warning-done nil
  "Non-nil once the incompatible Markdown grammar warning was shown.")

(defun piem--maybe-warn-incompatible-markdown-grammar ()
  "Warn once when the loaded Markdown tree-sitter grammar is incompatible."
  (unless (or noninteractive piem--markdown-grammar-warning-done)
    (when (piem--markdown-grammar-incompatible-p)
      (setq piem--markdown-grammar-warning-done t)
      (display-warning
       'piem
       (piem--incompatible-markdown-grammar-message)
       :warning))))

;;;; Installation

(defun piem--install-grammars (grammars)
  ...
  (let ((total (length grammars))
    ...
    (condition-case err
        (dolist (lang grammars)
          (cl-incf idx)
          (message "[piem] Installing grammar %d/%d: %s..."
                   idx total lang)
...
%d/%d grammars installed before failure."
       ...
       (setq idx (1- idx))))
    (when (> idx 0)
      (message "[piem] Installed %d/%d grammars." idx total))
    (setq piem--markdown-grammar-warning-done nil)
    idx))

(defcustom piem-essential-grammar-action 'prompt
...
(defun piem--maybe-install-essential-grammars ()
  "Handle missing essential grammars.
Behavior comes from `piem-essential-grammar-action'.
Respects `noninteractive' (batch mode always skips)."
  (unless noninteractive
    (let ((missing (piem--missing-essential-grammars)))
      (when missing
        (let* ((names (mapconcat #'symbol-name missing ", "))
               (should-install
                (pcase piem-essential-grammar-action
                  ('auto t)
                  ('prompt
                   (y-or-n-p
          ...
          (if should-install
              (progn
                (message "[piem] Installing essential grammars: %s"
                         names)
                (let ((installed (piem--install-grammars missing)))
...
The chat buffer will not render properly.\n\
Install them manually, run M-x `piem-install-grammars',\n\
or set `piem-essential-grammar-action' to `auto'."
                     names)
             :warning)))))))

(defcustom piem-grammar-declined-set nil
...
If they declined, only grammars not in the declined set are new."
  (let ((missing (piem--missing-optional-grammars)))
    (if piem-grammar-declined-set
        (seq-remove (lambda (g) (memq g piem-grammar-declined-set))
                    missing)
      missing)))

(defun piem--maybe-install-optional-grammars ()
...
or the declined set is cleared."
  (unless (or noninteractive
              piem--grammar-prompt-done)
    (setq piem--grammar-prompt-done t)
    (let ((missing (piem--missing-optional-grammars))
          (new (piem--new-missing-grammars)))
      (when new
        (if (y-or-n-p
             (format "%d tree-sitter grammars missing; install for code\
 highlighting (M-x `piem-install-grammars' to do later)? "
                     (length missing)))
            (piem--install-grammars missing)
          (customize-save-variable 'piem-grammar-declined-set missing)
          (message "[piem] Skipped.  \
Run M-x piem-install-grammars anytime."))))))

;;;; Interactive Command

;;;###autoload
(defun piem-install-grammars ()
...
then offers to install the missing ones."
  ...
  (let ((installed (piem--installed-optional-grammars))
        (missing (piem--missing-optional-grammars))
        (essential-missing (piem--missing-essential-grammars))
        (markdown-incompatible
         (piem--markdown-grammar-incompatible-p)))
    (cond
     (markdown-incompatible
      (display-warning
       'piem
     ...
     ((and (null missing) (null essential-missing))
      (message "All %d tree-sitter grammars are installed. ✓"
               (length installed)))
     (t
      (let ((buf (get-buffer-create "*piem-grammars*")))
        (with-current-buffer buf
          (let ((inhibit-read-only t))
            (erase-buffer)
            (insert (format "piem Tree-sitter Grammars\n\
====================================\n\n"))
          ...
          (local-set-key "i" (lambda ()
                               ...
                               (let ((to-install (append essential-missing missing)))
                                 (when to-install
                                   (piem--install-grammars to-install)
                                   (piem-install-grammars)))))
          (local-set-key "q" #'quit-window)
          (goto-char (point-min)))
        (pop-to-buffer buf))))))

(provide 'piem-grammars)
;;; piem-grammars.el ends here
