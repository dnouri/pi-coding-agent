;;; piem-bench.el --- Benchmarks for chat-buffer table rendering -*- lexical-binding: t; -*-
...
;; Performance benchmarks for display-only table rendering in piem.
...
;; or directly:
;;
;;   xvfb-run -a emacs -Q -L . -l bench/piem-bench.el \
;;         -f piem-bench-run-batch
;;
...
(defun piem-bench--load-fixtures ()
  "Load fixture tables from the bench fixtures directory."
  (unless piem-bench--tables
    (let ((file (expand-file-name "tables.md"
                                  piem-bench--fixture-dir)))
      (unless (file-exists-p file)
        (error "Fixture not found: %s" file))
      (setq piem-bench--tables
            (piem-bench--extract-tables file)))))

;;;; Chat Buffer Builders

(defun piem-bench--make-chat-buffer (content-fn)
...
before a prose-only user/assistant exchange.  Returns the buffer."
  (let ((tpt (or turns-per-table 1)))
    (piem-bench--make-chat-buffer
     (lambda ()
       (piem-bench--insert-turn
...
(defvar piem-bench--instrumenting nil
  "Non-nil while `piem-bench--with-instrumentation' is active.")

(defvar piem-bench--stage-times nil
  "Alist of (STAGE-NAME . TOTAL-SECONDS) accumulated during instrumented runs.")

(defvar piem-bench--stage-calls nil
  "Alist of (STAGE-NAME . CALL-COUNT) accumulated during instrumented runs.")

(defmacro piem-bench--with-instrumentation (&rest body)
  "Execute BODY with stage-level timing instrumentation active.
Binds `piem-bench--stage-times' and `--stage-calls',
installs timing advice around key functions, and removes it afterward."
  (declare (indent 0))
  `(let ((piem-bench--instrumenting t)
         (piem-bench--stage-times nil)
         (piem-bench--stage-calls nil))
     (piem-bench--install-timing-advice)
     (unwind-protect
         (progn ,@body)
       (piem-bench--remove-timing-advice))
     (cons piem-bench--stage-times
           piem-bench--stage-calls)))

(defun piem-bench--record-stage (name elapsed)
  "Record ELAPSED seconds for stage NAME."
  (if-let* ((cell (assq name piem-bench--stage-times)))
      (setcdr cell (+ (cdr cell) elapsed))
    (push (cons name elapsed) piem-bench--stage-times))
  (if-let* ((cell (assq name piem-bench--stage-calls)))
      (setcdr cell (1+ (cdr cell)))
    (push (cons name 1) piem-bench--stage-calls)))

(defun piem-bench--make-timing-advice (name)
  "Return an :around advice function that records timing for stage NAME."
  (lambda (orig-fn &rest args)
    (if piem-bench--instrumenting
        (let ((t0 (float-time)))
          (prog1 (apply orig-fn args)
            (piem-bench--record-stage
             name (- (float-time) t0))))
      (apply orig-fn args))))

(defconst piem-bench--instrumented-functions
  '((treesit-detect   . piem--treesit-table-regions)
    (visible-string   . piem--markdown-visible-string)
    (compute-widths   . markdown-table-wrap-compute-widths)
    (wrap-cell        . markdown-table-wrap-cell)
    (render-row-lines . piem--render-table-row-lines)
    (display-groups   . piem--table-display-groups)
    (decorate-table   . piem--decorate-table)
    (remove-overlays  . piem--remove-table-overlays)
    (split-row        . markdown-table-wrap--split-table-row))
  "Alist of (STAGE-NAME . FUNCTION) to instrument.")

(defun piem-bench--install-timing-advice ()
  "Install :around timing advice on all instrumented functions."
  (dolist (entry piem-bench--instrumented-functions)
    (advice-add (cdr entry) :around
                (piem-bench--make-timing-advice (car entry))
                '((name . piem-bench-timing)))))

(defun piem-bench--remove-timing-advice ()
  "Remove timing advice from all instrumented functions."
  (dolist (entry piem-bench--instrumented-functions)
    (advice-remove (cdr entry) 'piem-bench-timing)))

;;;; Scenario Definitions

(defun piem-bench--stable-decorate ()
  "Decorate all fixture tables in a mixed chat buffer at width 80."
  (piem-bench--load-fixtures)
  (let ((buf (piem-bench--build-mixed-buffer
              piem-bench--tables)))
    (unwind-protect
        (benchmark-run 1
          (with-current-buffer buf
...
(defun piem-bench--stable-decorate-narrow ()
...
(defun piem-bench--streaming-refresh ()
  ...
  (let ((buf (piem-bench--build-mixed-buffer
    ...
    (unwind-protect
        (with-current-buffer buf
          ...
          (let* ((regions (piem--treesit-table-regions
                           (point-min) (point-max)))
                 (last-region (car (last regions)))
                 (lbeg (car last-region))
                 (lend (cdr last-region)))
            (benchmark-run 1
              (piem--remove-table-overlays lbeg lend)
              (piem--decorate-table lbeg lend 80))))
      (kill-buffer buf))))

(defun piem-bench--resize-full ()
...
(defun piem-bench--resize-hot-tail ()
...
(defun piem-bench--visible-string-extraction ()
...
(defun piem-bench--inline-fontification-cost ()
...
Uses a pre-existing chat-mode buffer to isolate font-lock-ensure cost."
  (let ((buf (piem-bench--make-chat-buffer #'ignore))
...
(defconst piem-bench--suite
  '(("stable-all"       . piem-bench--stable-decorate)
    ("stable-narrow"     . piem-bench--stable-decorate-narrow)
    ("stream-last"       . piem-bench--streaming-refresh)
    ("resize-full"       . piem-bench--resize-full)
    ("resize-hot-tail"   . piem-bench--resize-hot-tail)
    ("resize-sweep"      . piem-bench--resize-sweep)
    ("visible-string"    . piem-bench--visible-string-extraction)
    ("fontify-only"      . piem-bench--inline-fontification-cost))
  "Alist of (NAME . FUNCTION) for the benchmark suite.")

(defvar piem-bench--repetitions 5
  "Number of repetitions for each benchmark.")

(defun piem-bench-run (&optional repetitions)
  "Run all benchmarks REPETITIONS times, print results.
Default is 5 repetitions."
  (let ((k (or repetitions piem-bench--repetitions)))
    (piem-bench--load-fixtures)

    ;; Header
    (princ (format "\npiem table rendering benchmarks (%d reps)\n" k))
    (princ (format "Fixture: %d tables, %s rows\n"
                   (length piem-bench--tables)
    ...
    (let* ((result (piem-bench--with-instrumentation
                     (piem-bench--stable-decorate)))
