;;; piem-tool-update-bench.el --- Tool-update storm benchmarks -*- lexical-binding: t; -*-
...
;; Deterministic tool_execution_update storm benchmarks for piem.
...
;; `piem--handle-display-event', and user-perceived main-thread
;; blocking via a 50 ms probe timer's lateness, and tool block re-render
;; counts via advice on `piem--tool-block-replace-body' and
;; `piem--display-tool-end' (the environment-independent
...
(defconst piem-tu-bench-repo-root
...
(add-to-list 'load-path piem-tu-bench-repo-root)
(require 'piem)

(defun piem-tu-bench--env (name default)
...
(defun piem-tu-bench--env-int (name default)
  "Return environment variable NAME as an integer, or DEFAULT."
  (string-to-number (piem-tu-bench--env
                     name (number-to-string default))))

(defun piem-tu-bench--env-float (name default)
  "Return environment variable NAME as a float, or DEFAULT."
  (string-to-number (piem-tu-bench--env
                     name (number-to-string default))))

(defun piem-tu-bench--truthy-env-p (name default)
  "Return non-nil when environment variable NAME is truthy.
DEFAULT is used when NAME is unset."
  (let ((value (downcase (piem-tu-bench--env name default))))
    (and (member value '("1" "true" "yes" "on")) t)))

(defun piem-tu-bench--json-bool (value)
  "Return VALUE encoded as a JSON boolean sentinel."
  (if value t :json-false))

(defvar piem-tu-bench-scenario
  (piem-tu-bench--env "PI_TU_BENCH_SCENARIO" "standalone")
  "Scenario label written into tool-update benchmark artifacts.")

(defvar piem-tu-bench-iteration
  (piem-tu-bench--env-int "PI_TU_BENCH_ITERATION" 1)
  "Iteration number written into tool-update benchmark artifacts.")

(defvar piem-tu-bench-fill-bash
  (piem-tu-bench--env-int "PI_TU_BENCH_FILL_BASH" 58)
  "Number of completed bash tool executions in the synthetic fill phase.")

(defvar piem-tu-bench-fill-read
  (piem-tu-bench--env-int "PI_TU_BENCH_FILL_READ" 5)
  "Number of completed read tool executions in the synthetic fill phase.")

(defvar piem-tu-bench-fill-write
  (piem-tu-bench--env-int "PI_TU_BENCH_FILL_WRITE" 2)
  "Number of completed write tool executions in the synthetic fill phase.")

(defvar piem-tu-bench-fill-edit
  (piem-tu-bench--env-int "PI_TU_BENCH_FILL_EDIT" 1)
  "Number of completed edit tool executions in the synthetic fill phase.")

(defvar piem-tu-bench-fill-output-lines
  (piem-tu-bench--env-int "PI_TU_BENCH_FILL_OUTPUT_LINES" 20)
  "Number of synthetic output lines per fill phase tool result.")

(defvar piem-tu-bench-updates
  (piem-tu-bench--env-int "PI_TU_BENCH_UPDATES" 400)
  "Number of storm phase tool_execution_update events.")

(defvar piem-tu-bench-parallel-tools
  (piem-tu-bench--env-int "PI_TU_BENCH_PARALLEL_TOOLS" 3)
  "Number of parallel subagent tool executions in the storm phase.")

(defvar piem-tu-bench-gap-scale
  (piem-tu-bench--env-float "PI_TU_BENCH_GAP_SCALE" 1.0)
...
(defvar piem-tu-bench-seed
  (piem-tu-bench--env-int "PI_TU_BENCH_SEED" 20240817)
  "PRNG seed for the fake backend's gap pattern; recorded for artifacts.")

(defvar piem-tu-bench-timeout-seconds
  (piem-tu-bench--env-int "PI_TU_BENCH_TIMEOUT_SECONDS" 240)
  "Timeout in seconds for waiting on the storm to settle.")

(defvar piem-tu-bench-display-buffers
  (piem-tu-bench--truthy-env-p "PI_TU_BENCH_DISPLAY" "0")
  "Whether GUI benchmark runs should display chat and input windows.")

(defvar piem-tu-bench-probe-interval
  0.05
  "Probe timer interval in seconds for measuring main thread blocking.")

(defvar piem-tu-bench-hot-tail-turns
  (piem-tu-bench--env-int "PI_TU_BENCH_HOT_TAIL_TURNS" 1)
  "Number of headed turns kept hot by the cooling benchmark scenario.")

(defvar piem-tu-bench-command-interval
  (/ (piem-tu-bench--env-float
      "PI_TU_BENCH_COMMAND_INTERVAL_MS" 100.0)
     1000.0)
  "Seconds between routed scroll-heartbeat commands during cooling.")

(defconst piem-tu-bench-cooling-live-id "call-cooling-live"
  "Tool call ID of the live hot-tail sentinel fixture.")

(defconst piem-tu-bench-command-event
  'piem-tu-bench-scroll-heartbeat
  "Synthetic benchmark-safe command event routed during natural drain.")

(defvar piem-tu-bench-out-dir
  (file-name-as-directory
   (expand-file-name (piem-tu-bench--env
                      "PI_TU_BENCH_OUT_DIR"
                      "tmp/tool-update-bench/standalone")
                     piem-tu-bench-repo-root))
  "Output directory for one tool-update benchmark iteration.")

(defvar piem-tu-bench-runner-out-dir
  (file-name-as-directory
   (expand-file-name (piem-tu-bench--env
                      "PI_TU_BENCH_RUNNER_OUT_DIR"
                      piem-tu-bench-out-dir)
                     piem-tu-bench-repo-root))
  "Top-level runner output directory for reproduction commands.")

(defvar piem-tu-bench-fake-pi
  (expand-file-name "bench/fake-pi-tool-update-storm.py"
                    piem-tu-bench-repo-root)
  "Fake pi RPC executable used by tool-update benchmark runs.")

(defvar piem-tu-bench-fake-log
  (expand-file-name "fake-pi.jsonl" piem-tu-bench-out-dir)
  "Content-free fake RPC log path for one benchmark run.")

(defvar piem-tu-bench-result-file
  (expand-file-name "result.json" piem-tu-bench-out-dir)
  "JSON result artifact path for one benchmark run.")

(defvar piem-tu-bench-report-file
  (expand-file-name "report.md" piem-tu-bench-out-dir)
  "Markdown report artifact path for one benchmark run.")

(defvar piem-tu-bench-times-file
  (expand-file-name "times.tsv" piem-tu-bench-out-dir)
  "Per-event-type timing artifact path for one benchmark run.")

(defvar piem-tu-bench-cooling-slices-file
  (expand-file-name "cooling-slices.tsv" piem-tu-bench-out-dir)
  "Per-callback deferred cooling artifact path for one benchmark run.")

(defvar piem-tu-bench-commands-file
  (expand-file-name "commands.tsv" piem-tu-bench-out-dir)
  "Per-command scroll-heartbeat artifact path for one benchmark run.")

(defvar piem-tu-bench--event-log nil
  "List of (TYPE . ELAPSED-MS) entries for handled display events.")

(defvar piem-tu-bench--prompt-time nil
  "Float time when the storm prompt was sent.")

(defvar piem-tu-bench--agent-end-time nil
  "Float time when the agent_end event finished handling, or nil.")

(defvar piem-tu-bench--probe-expected nil
  "Float time the next probe firing was expected, or nil.")

(defvar piem-tu-bench--probe-lateness nil
  "List of probe firing lateness values in seconds.")

(defvar piem-tu-bench--probe-timer nil
  "The probe timer object, or nil.")

(defvar piem-tu-bench--render-log nil
...
(defun piem-tu-bench--overlay-tool-call-id (overlay)
  "Return OVERLAY's tool call ID, or nil."
  (when-let* ((record
               (overlay-get overlay 'piem-tool-block-record)))
    (piem--tool-block-tool-call-id record)))

(defun piem-tu-bench--overlay-count-for-id (tool-call-id)
  "Return the current buffer's tool overlay count for TOOL-CALL-ID."
  (cl-count tool-call-id (piem-tu-bench--tool-overlays)
            :key #'piem-tu-bench--overlay-tool-call-id
            :test #'equal))

(defun piem-tu-bench--cold-tool-metadata ()
  "Return cold tool metadata segments in `current-buffer' order."
  (let ((position (point-min))
        metadata)
    (while (< position (point-max))
      (let* ((value (get-text-property
                     position 'piem-cold-tool-block))
...
(defun piem-tu-bench--button-count ()
...
(defun piem-tu-bench--collapsed-tool-count ()
  "Return how many current tool overlays carry a collapse button."
  (cl-count-if
   (lambda (overlay)
     (piem--find-toggle-button-in-region
      (overlay-start overlay) (overlay-end overlay)))
   (piem-tu-bench--tool-overlays)))

(defun piem-tu-bench--semantic-lines ()
...
(defun piem-tu-bench--expected-semantic-lines ()
  "Return exact semantic sentinel lines expected from the cooling fixture."
  (append
   (cl-loop for index below (piem-tu-bench--fill-tool-count)
...
(defun piem-tu-bench--expected-semantic-hash ()
  "Return SHA-256 of the exact expected cooling semantic projection."
  (secure-hash 'sha256
               (string-join
                (piem-tu-bench--expected-semantic-lines) "\n")))

(defun piem-tu-bench--tree-root-row ()
  "Return tree-root counters from the current dynamic measurement phase."
  (list :phase piem-tu-bench--tree-root-phase
        :count piem-tu-bench--tree-root-count
        :totalMs piem-tu-bench--tree-root-ms
        :maxMs piem-tu-bench--tree-root-max-ms))

(defun piem-tu-bench--around-tree-root (orig &rest args)
  "Call ORIG with ARGS and record time in the active lightweight phase."
  (if (not piem-tu-bench--tree-root-phase)
      (apply orig args)
    (let ((start (float-time))
      ...
      (unwind-protect
        ...
        (let ((elapsed (* 1000.0 (- (float-time) start))))
          (setq piem-tu-bench--tree-root-count
                (1+ piem-tu-bench--tree-root-count)
                piem-tu-bench--tree-root-ms
                (+ piem-tu-bench--tree-root-ms elapsed)
                piem-tu-bench--tree-root-max-ms
                (max piem-tu-bench--tree-root-max-ms elapsed))))
      value)))

(defun piem-tu-bench--around-process-filter (orig proc output)
  "Call process filter ORIG for PROC and OUTPUT while timing the real path."
  (let* ((id (cl-incf piem-tu-bench--filter-sequence))
         (start (float-time))
         (piem-tu-bench--current-filter-id id)
         value)
    (setq value (funcall orig proc output))
    (let* ((elapsed (* 1000.0 (- (float-time) start)))
           (agent-end
            (and piem-tu-bench--agent-end-observation
                 (= id (plist-get piem-tu-bench--agent-end-observation
                                  :filterId))))
           (row (list :id id
                      :wallMs elapsed
      ...
      (push row piem-tu-bench--filter-log)
      (when agent-end
        (setq piem-tu-bench--agent-end-filter row)))
    value))

(defun piem-tu-bench--time-display-event (orig event type)
...
handler runs unmodified, with no delays or state overrides."
  (let* ((start (float-time))
    ...
    (push (cons type (* 1000.0 (- end start)))
          piem-tu-bench--event-log)
    (list :value value :end end :elapsedMs (* 1000.0 (- end start)))))

(defun piem-tu-bench--cooling-state-counts ()
...
helper, so the two views cannot drift apart.  Pure observation."
  (list :hotTailBoundary
        (and (markerp piem--hot-tail-start)
             (marker-position piem--hot-tail-start))
        :queueLength (length piem--tool-cooling-queue)
        :timer (and piem--tool-cooling-timer t)
        :coldTools (length (piem-tu-bench--cold-tool-metadata))
        :toolOverlays (length (piem-tu-bench--tool-overlays))
        :liveRegistry
        (if (hash-table-p piem--live-tool-blocks)
            (hash-table-count piem--live-tool-blocks)
          0)
        :currentOverlays
        (piem-tu-bench--overlay-count-for-id
         piem-tu-bench-cooling-live-id)))

(defun piem-tu-bench--semantic-summary ()
  "Return (LINE-COUNT . SHA-256) of the current semantic projection."
  (let ((lines (piem-tu-bench--semantic-lines)))
    (cons (length lines)
          (secure-hash 'sha256 (string-join lines "\n")))))

(defun piem-tu-bench--around-handle-event (orig event)
...
Calls ORIG with EVENT and appends to the benchmark event log."
  (let ((type (or (plist-get event :type) "unknown")))
    (if (and (equal type "agent_end")
             (piem-tu-bench--cooling-scenario-p))
        (let* ((before (piem-tu-bench--cooling-state-counts))
               (collapsed-before
                (piem-tu-bench--collapsed-tool-count))
               (gc-before gcs-done)
               (gc-time-before gc-elapsed)
               timed tree-row)
          (let ((piem-tu-bench--tree-root-phase "agent_end")
                (piem-tu-bench--tree-root-count 0)
                (piem-tu-bench--tree-root-ms 0.0)
                (piem-tu-bench--tree-root-max-ms 0.0))
            (setq timed (piem-tu-bench--time-display-event
                         orig event type)
                  tree-row (piem-tu-bench--tree-root-row)))
          (let* ((after (piem-tu-bench--cooling-state-counts))
                 (semantic (piem-tu-bench--semantic-summary)))
            (setq piem-tu-bench--agent-end-time
                  (plist-get timed :end)
                  piem-tu-bench--drain-start-time
                  (plist-get timed :end)
                  piem-tu-bench--agent-end-observation
                  (list
                   :wallMs (plist-get timed :elapsedMs)
                   :filterId piem-tu-bench--current-filter-id
      ...
      (let ((timed (piem-tu-bench--time-display-event
                    orig event type)))
...
(defun piem-tu-bench--around-cooling-slice
    (orig buffer generation)
  "Call cooling worker ORIG for BUFFER and GENERATION and record one slice."
  (if (not (piem-tu-bench--cooling-scenario-p))
      (funcall orig buffer generation)
    (let* ((index (cl-incf piem-tu-bench--cooling-slice-sequence))
           (buffer-live-before (buffer-live-p buffer))
           (queue-before
            (if buffer-live-before
                (with-current-buffer buffer
                  (length piem--tool-cooling-queue))
              -1))
           (candidate
            (and buffer-live-before
                 (with-current-buffer buffer
                   (car piem--tool-cooling-queue))))
           (candidate-id
            (and candidate
                 (piem-tu-bench--overlay-tool-call-id candidate)))
           (candidate-eligible
            (and candidate buffer-live-before
                 (with-current-buffer buffer
                   (when-let* ((boundary
                                (piem--tool-cooling-boundary)))
                     (piem--completed-tool-overlay-before-p
                      candidate boundary)))))
           (timer-before
            (and buffer-live-before
                 (with-current-buffer buffer
                   (and piem--tool-cooling-timer t))))
           (gc-before gcs-done)
           (gc-time-before gc-elapsed)
           (start (float-time))
           (piem-tu-bench--tree-root-phase "cooling_slice")
           (piem-tu-bench--tree-root-count 0)
           (piem-tu-bench--tree-root-ms 0.0)
           (piem-tu-bench--tree-root-max-ms 0.0)
           value end)
        ...
        (let* ((buffer-live-after (buffer-live-p buffer))
               ...
               (row
                (list
                 ...
                 :candidateEligible
                 (piem-tu-bench--json-bool candidate-eligible)
                 :candidateOverlayAliveAfter
                 (piem-tu-bench--json-bool
                  (and candidate (overlay-buffer candidate)))
                 :timerBefore
                 (piem-tu-bench--json-bool timer-before)
                 :timerAfter
                 (piem-tu-bench--json-bool timer-after)
                 :wallMs (* 1000.0 (- end start))
                 :gcs (- gcs-done gc-before)
                 :gcMs (* 1000.0 (- gc-elapsed gc-time-before))
                 :treeRoots (piem-tu-bench--tree-root-row)
                 :window (piem-tu-bench--capture-window-state))))
          (push row piem-tu-bench--cooling-slice-log)))
      value)))

(defun piem-tu-bench--around-display-warning (orig &rest args)
  "Call `display-warning' ORIG with ARGS and record scheduler errors."
  (let ((text (format "%s" (nth 1 args))))
    (when (and (piem-tu-bench--cooling-scenario-p)
               (string-match-p "Deferred tool block cooling failed" text))
      (push text piem-tu-bench--scheduler-errors)))
  (apply orig args))

(defun piem-tu-bench--install-advice ()
  "Install event, render, and scenario-specific cooling measurement advice."
  (advice-add 'piem--handle-display-event
              :around #'piem-tu-bench--around-handle-event)
  (advice-add 'piem--tool-block-replace-body
              :around #'piem-tu-bench--around-replace-body)
  (advice-add 'piem--display-tool-end
              :around #'piem-tu-bench--around-display-tool-end)
  (when (piem-tu-bench--cooling-scenario-p)
    (advice-add 'piem--process-filter
                :around #'piem-tu-bench--around-process-filter)
    (advice-add 'piem--run-tool-cooling-slice
                :around #'piem-tu-bench--around-cooling-slice)
    (advice-add 'display-warning
                :around #'piem-tu-bench--around-display-warning)
    (when (fboundp 'treesit-parser-root-node)
      (advice-add 'treesit-parser-root-node
                  :around #'piem-tu-bench--around-tree-root))))

(defun piem-tu-bench--remove-advice ()
  "Remove all benchmark advice."
  (advice-remove 'piem--process-filter
                 #'piem-tu-bench--around-process-filter)
  (advice-remove 'piem--handle-display-event
                 #'piem-tu-bench--around-handle-event)
  (advice-remove 'piem--tool-block-replace-body
                 #'piem-tu-bench--around-replace-body)
  (advice-remove 'piem--display-tool-end
                 #'piem-tu-bench--around-display-tool-end)
  (advice-remove 'piem--run-tool-cooling-slice
                 #'piem-tu-bench--around-cooling-slice)
  (advice-remove 'display-warning
                 #'piem-tu-bench--around-display-warning)
  (when (fboundp 'treesit-parser-root-node)
    (advice-remove 'treesit-parser-root-node
                   #'piem-tu-bench--around-tree-root)))

(defun piem-tu-bench--record-render (operation tool-call-id
                                                          elapsed-ms)
  "Record one OPERATION render for TOOL-CALL-ID taking ELAPSED-MS."
  (unless (hash-table-p piem-tu-bench--render-log)
    (setq piem-tu-bench--render-log (make-hash-table :test 'equal)))
  (let* ((key (format "%s\t%s" operation (or tool-call-id "(unkeyed)")))
         (row (gethash key piem-tu-bench--render-log)))
    (if row
        (progn
          (cl-incf (car row))
      ...
      (puthash key (list 1 elapsed-ms elapsed-ms)
               piem-tu-bench--render-log))))

(defun piem-tu-bench--around-replace-body (orig block &rest args)
  "Around advice counting and timing a body replacement for BLOCK.
Calls ORIG with BLOCK and ARGS."
  (let ((start (float-time)))
    (prog1 (apply orig block args)
      (piem-tu-bench--record-render
       "replace-body"
       (and block (piem--tool-block-tool-call-id block))
       (* 1000.0 (- (float-time) start))))))

(defun piem-tu-bench--around-display-tool-end (orig &rest args)
...
Calls ORIG with ARGS; the block comes from ARGS or the current block."
  (let ((start (float-time))
        (block (or (nth 5 args) (piem--current-tool-block))))
...
(defun piem-tu-bench--render-rows ()
...
(defun piem-tu-bench--render-count (operation tool-call-id)
  "Return how often OPERATION was recorded for TOOL-CALL-ID."
  (let ((row (and (hash-table-p piem-tu-bench--render-log)
                  (gethash (format "%s\t%s" operation tool-call-id)
                           piem-tu-bench--render-log))))
    (if row (car row) 0)))

(defun piem-tu-bench--render-total (operation)
  "Return the total number of recorded OPERATION renders."
  (cl-loop for row in (piem-tu-bench--render-rows)
           when (equal (plist-get row :operation) operation)
           sum (plist-get row :count)))

(defun piem-tu-bench--render-operation-json (operation)
...
(defun piem-tu-bench--probe-tick ()
  "Record how late this probe firing is relative to its expected time."
  (let ((now (float-time)))
    (when piem-tu-bench--probe-expected
      (push (max 0.0 (- now piem-tu-bench--probe-expected))
            piem-tu-bench--probe-lateness))
    (setq piem-tu-bench--probe-expected
          (+ now piem-tu-bench-probe-interval))))

(defun piem-tu-bench--start-probe ()
  "Start the probe timer and reset its state."
  (setq piem-tu-bench--probe-expected nil
        piem-tu-bench--probe-lateness nil)
  (setq piem-tu-bench--probe-timer
        (run-with-timer 0 piem-tu-bench-probe-interval
                        #'piem-tu-bench--probe-tick)))

(defun piem-tu-bench--stop-probe ()
  "Cancel the probe timer when it is running."
  (when piem-tu-bench--probe-timer
    (cancel-timer piem-tu-bench--probe-timer)
    (setq piem-tu-bench--probe-timer nil)))

(defun piem-tu-bench--event-count (type)
  "Return the number of handled display events of TYPE."
  (cl-count type piem-tu-bench--event-log
            :key #'car :test #'equal))

(defun piem-tu-bench--event-stats ()
...
Each row is a plist with :type :count :totalMs :meanMs and :maxMs."
  (let ((table (make-hash-table :test 'equal))
        (rows nil))
    (dolist (entry piem-tu-bench--event-log)
...
(defun piem-tu-bench--percentile (samples p)
...
(defun piem-tu-bench--probe-stats ()
  "Return probe timer statistics as a plist of millisecond values."
  (let ((lateness piem-tu-bench--probe-lateness))
    (list :intervalMs (* 1000.0 piem-tu-bench-probe-interval)
          :fires (length lateness)
          :p50Ms (* 1000.0 (piem-tu-bench--percentile lateness 0.50))
          :p95Ms (* 1000.0 (piem-tu-bench--percentile lateness 0.95))
          :maxMs (if lateness
...
(defun piem-tu-bench--window-line-at (buffer position)
    ...
    (if (not (and (buffer-live-p buffer) (window-live-p window)))
      ...
      (with-current-buffer buffer
        (let* ((start (window-start window))
               ...
               (view-ok
                (if (equal piem-tu-bench--command-last-route
                           "sentinel")
                    sentinel-p
                  following)))
          (list :available t
                ...
                :following
                (piem-tu-bench--json-bool following)
                :sentinel
                (piem-tu-bench--json-bool sentinel-p)
                :viewOk
                (piem-tu-bench--json-bool view-ok)))))))

(defun piem-tu-bench--command-pre-hook ()
  "Record entry through the benchmark command's pre-command hook."
  (when (eq this-command
            'piem-tu-bench--scroll-heartbeat-command)
    (setq piem-tu-bench--command-current-job
          (plist-put piem-tu-bench--command-current-job
                     :preHookAt (float-time)))))

(defun piem-tu-bench--command-post-hook ()
...
(defun piem-tu-bench--scroll-heartbeat-command ()
  "Route one deterministic sentinel/follow scroll heartbeat."
  (interactive)
  (let* ((job piem-tu-bench--command-current-job)
         (route (plist-get job :route))
         (buffer piem-tu-bench--cooling-chat-buffer)
         (window (and (buffer-live-p buffer)
                      (get-buffer-window buffer t))))
    ...
    (with-current-buffer buffer
      (let ((sentinel
             (and (markerp piem-tu-bench--window-sentinel-marker)
                  (marker-position
                   piem-tu-bench--window-sentinel-marker))))
        (unless sentinel
        ...
        (if (window-live-p window)
          ...
          (goto-char (if (equal route "sentinel")
                         sentinel
                       (point-max))))))
    (setq piem-tu-bench--command-last-route route)))

(defun piem-tu-bench--enqueue-command-event
    (sequence due route)
  "Enqueue scroll command SEQUENCE for DUE using logical ROUTE."
  (setq piem-tu-bench--command-timer nil)
  (when piem-tu-bench--command-drain-active
    (setq piem-tu-bench--command-pending
          (nconc piem-tu-bench--command-pending
    ...
    (setq unread-command-events
          (append unread-command-events
                  (list piem-tu-bench-command-event)))))

(defun piem-tu-bench--schedule-command-event ()
  "Arm one benchmark command event when the cooling drain is active."
  (when (and piem-tu-bench--command-drain-active
             (not piem-tu-bench--command-timer)
             (null piem-tu-bench--command-pending))
    (let* ((sequence
            (cl-incf piem-tu-bench--command-sequence))
           (route (if (cl-oddp sequence) "sentinel" "follow"))
           (due (+ (float-time)
                   piem-tu-bench-command-interval)))
      (setq piem-tu-bench--command-timer
            (run-at-time piem-tu-bench-command-interval nil
                         #'piem-tu-bench--enqueue-command-event
                         sequence due route)))))

(defun piem-tu-bench--route-command-event (event)
  "Route unread EVENT through key lookup, hooks, and `command-execute'."
  (when (eq event piem-tu-bench-command-event)
    (let* ((job (pop piem-tu-bench--command-pending))
           (buffer piem-tu-bench--cooling-chat-buffer)
           (command
      ...
      (if (not job)
        ...
        (setq job (plist-put job :startedAt started)
              piem-tu-bench--command-current-job job)
        ...
        (setq job piem-tu-bench--command-current-job))
             ...
             (row
              (append
               ...
               (list
                ...
                :window (piem-tu-bench--capture-window-state)))))
        (push row piem-tu-bench--command-log))
      (setq piem-tu-bench--command-current-job nil)
      (piem-tu-bench--schedule-command-event))
    t))

(defun piem-tu-bench--install-command-route (chat-buf)
  "Install the benchmark-safe unread command route in CHAT-BUF."
  (setq piem-tu-bench--cooling-chat-buffer chat-buf
        piem-tu-bench--command-last-route "follow")
  (with-current-buffer chat-buf
    (use-local-map (copy-keymap (current-local-map)))
    (define-key (current-local-map)
                (vector piem-tu-bench-command-event)
                #'piem-tu-bench--scroll-heartbeat-command)
    (add-hook 'pre-command-hook
              #'piem-tu-bench--command-pre-hook nil t)
    (add-hook 'post-command-hook
              #'piem-tu-bench--command-post-hook nil t)
    (save-excursion
      (goto-char (point-min))
      (unless (search-forward "COOLING-WINDOW-SENTINEL" nil t)
        (error "Cooling fixture did not render its window sentinel"))
      (setq piem-tu-bench--window-sentinel-marker
...
(defun piem-tu-bench--start-command-route ()
  "Start one-shot command scheduling for a natural cooling drain."
  (setq piem-tu-bench--command-drain-active t)
  (piem-tu-bench--schedule-command-event))

(defun piem-tu-bench--stop-command-route ()
  "Stop command scheduling and remove unread benchmark events."
  (setq piem-tu-bench--command-drain-active nil)
  (when (timerp piem-tu-bench--command-timer)
    (cancel-timer piem-tu-bench--command-timer))
  (setq piem-tu-bench--command-timer nil
        piem-tu-bench--command-pending nil
        unread-command-events
        (delq piem-tu-bench-command-event
              unread-command-events)))

(defun piem-tu-bench--cooling-drained-p (chat-buf)
  "Return non-nil when CHAT-BUF owns no cooling queue or timer."
  (and (buffer-live-p chat-buf)
       (with-current-buffer chat-buf
         (and (null piem--tool-cooling-queue)
              (null piem--tool-cooling-timer)))))

(defun piem-tu-bench--wait-for-natural-cooling
    (chat-buf timeout)
  "Wait up to TIMEOUT for CHAT-BUF's real one-shot cooling timers to drain."
  (let ((start (or piem-tu-bench--drain-start-time
                   (float-time)))
        (gc-before gcs-done)
        (gc-time-before gc-elapsed)
        (deadline (+ (float-time) timeout))
        settled end)
    (piem-tu-bench--install-command-route chat-buf)
    (piem-tu-bench--start-probe)
    (piem-tu-bench--start-command-route)
    (unwind-protect
        (while (and (not settled) (< (float-time) deadline))
          ...
          (when-let* ((event (read-event nil nil 0.01)))
            (piem-tu-bench--route-command-event event))
          (accept-process-output nil 0.005)
          (when (and (piem-tu-bench--cooling-drained-p chat-buf)
                     (null piem-tu-bench--command-pending))
            (setq settled t)))
      (setq end (float-time))
      (piem-tu-bench--stop-command-route)
      (piem-tu-bench--stop-probe))
    (list :settled (piem-tu-bench--json-bool settled)
          :timeoutSeconds timeout
          :wallMs (* 1000.0 (- end start))
          :activeMs
          (cl-loop for row in piem-tu-bench--cooling-slice-log
                   sum (plist-get row :wallMs))
          :callbacks (length piem-tu-bench--cooling-slice-log)
          :gcs (- gcs-done gc-before)
          :gcMs (* 1000.0 (- gc-elapsed gc-time-before))
          :finalWindow (piem-tu-bench--capture-window-state))))

(defun piem-tu-bench--command-stats ()
  "Return command scheduling and duration summary statistics."
  (let* ((rows piem-tu-bench--command-log)
    ...
    (list :count (length rows)
          ...
          (cl-count "follow" rows :key (lambda (row) (plist-get row :route))
                    :test #'equal)
          :latenessP50Ms
          (piem-tu-bench--percentile lateness 0.50)
          :latenessP95Ms
          (piem-tu-bench--percentile lateness 0.95)
          :latenessMaxMs (if lateness (apply #'max lateness) 0.0)
          :durationP50Ms
          (piem-tu-bench--percentile durations 0.50)
          :durationP95Ms
          (piem-tu-bench--percentile durations 0.95)
          :durationMaxMs (if durations (apply #'max durations) 0.0))))

(defun piem-tu-bench--storm-tool-ids ()
  "Return the expected storm phase subagent tool call IDs."
  (cl-loop for index below piem-tu-bench-parallel-tools
           collect (format "call-storm-%02d" index)))

(defun piem-tu-bench--fill-tool-count ()
  "Return the total number of fill phase tool executions."
  (+ piem-tu-bench-fill-bash
     piem-tu-bench-fill-read
     piem-tu-bench-fill-write
     piem-tu-bench-fill-edit))

(defun piem-tu-bench--wait-until (predicate timeout)
  ...
  (let ((start (float-time))
    ...
    (while (and (not (setq result (funcall predicate)))
      ...
      (when (and piem-tu-bench-display-buffers (not noninteractive))
        (redisplay t)))
    result))

(defun piem-tu-bench--pending-requests-count (proc)
...
(defun piem-tu-bench--count-occurrences (buffer text)
...
(defun piem-tu-bench--tool-block-overlay-count (buffer tool-call-id)
  ...
  (if (not (buffer-live-p buffer))
    ...
    (with-current-buffer buffer
      (cl-count-if
       (lambda (ov)
         (when-let* ((record (and (overlay-get ov 'piem-tool-block)
                                  (overlay-get
                                   ov 'piem-tool-block-record))))
           (equal (piem--tool-block-tool-call-id record)
                  tool-call-id)))
       (overlays-in (point-min) (point-max))))))

(defun piem-tu-bench--live-tool-block-count (buffer)
...
(defun piem-tu-bench--parser-font-lock-state (chat-buf)
  ...
  (if (not (buffer-live-p chat-buf))
    ...
    (with-current-buffer chat-buf
      (let ((piem-tu-bench--tree-root-phase "final_check")
            (piem-tu-bench--tree-root-count 0)
            (piem-tu-bench--tree-root-ms 0.0)
            (piem-tu-bench--tree-root-max-ms 0.0)
        ...
        (condition-case error-data
            (progn
              (setq parser (piem--markdown-parser))
              (unless parser
        ...
        (list
         ...
         :fontLockOk
         (piem-tu-bench--json-bool (null font-lock-error))
...
(defun piem-tu-bench--tool-name-counts (metadata)
...
(defun piem-tu-bench--cooling-final-state (chat-buf parser-state)
  "Return exact settled cooling state for CHAT-BUF and PARSER-STATE."
  (if (not (buffer-live-p chat-buf))
      (list :bufferLive :json-false :parser parser-state)
    (with-current-buffer chat-buf
      (let* ((counts (piem-tu-bench--cooling-state-counts))
             (semantic (piem-tu-bench--semantic-summary))
             (metadata (piem-tu-bench--cold-tool-metadata))
             (tool-overlays (piem-tu-bench--tool-overlays))
             (current
              (seq-find
               (lambda (overlay)
                 (equal (piem-tu-bench--overlay-tool-call-id
                         overlay)
                        piem-tu-bench-cooling-live-id))
               tool-overlays))
        ...
        (list
         ...
         :coldToolNames
         (piem-tu-bench--tool-name-counts metadata)
         ...
         (vconcat
          (sort (delq nil
                      (mapcar
                       #'piem-tu-bench--overlay-tool-call-id
                       tool-overlays))
                #'string<))
         :currentOverlayStart
         ...
         :buttons (piem-tu-bench--button-count)
         :diffOverlays
         (cl-count-if
          (lambda (overlay)
            (overlay-get overlay 'piem-diff-overlay))
          (overlays-in (point-min) (point-max)))
         :tableOverlays
         (cl-count-if
          (lambda (overlay)
            (overlay-get overlay 'piem-table-display))
          (overlays-in (point-min) (point-max)))
         ...
         (piem-tu-bench--expected-semantic-hash)
         :window (piem-tu-bench--capture-window-state)
         :geometry
         (if (and piem-tu-bench-display-buffers
                  (not noninteractive)
...
(defun piem-tu-bench--cooling-slices-in-order ()
  "Return cooling slice rows in callback order."
  (nreverse (copy-sequence piem-tu-bench--cooling-slice-log)))

(defun piem-tu-bench--commands-in-order ()
  "Return command rows in scheduling order."
  (nreverse (copy-sequence piem-tu-bench--command-log)))

(defun piem-tu-bench--tree-root-summary ()
  "Return lightweight tree-root metrics split across agent_end and slices."
  (let* ((agent-row
          (plist-get piem-tu-bench--agent-end-observation
                     :treeRoots))
         (slice-rows (piem-tu-bench--cooling-slices-in-order)))
    (list
...
(defun piem-tu-bench--check (name ok detail)
  "Return a correctness check entry for NAME with OK flag and DETAIL text."
  (list :name name
        :ok (piem-tu-bench--json-bool ok)
        :detail detail))

(defun piem-tu-bench--collect-checks (chat-buf)
...
Every entry must have a true :ok for the benchmark run to pass."
  (let* ((storm-ids (piem-tu-bench--storm-tool-ids))
         (expected-executions (+ (piem-tu-bench--fill-tool-count)
                                 piem-tu-bench-parallel-tools))
         (start-count (piem-tu-bench--event-count
                       "tool_execution_start"))
         (end-count (piem-tu-bench--event-count "tool_execution_end"))
         (update-count (piem-tu-bench--event-count
                        "tool_execution_update"))
         (checks nil))
    (push (piem-tu-bench--check
           "agent-end-received"
           (and piem-tu-bench--agent-end-time t)
           (if piem-tu-bench--agent-end-time
               "agent_end handled"
    ...
    (push (piem-tu-bench--check
           ...
           (= update-count piem-tu-bench-updates)
           (format "expected %d, handled %d"
                   piem-tu-bench-updates update-count))
          checks)
    (let ((bad-blocks
           (seq-filter
            (lambda (tool-call-id)
              (/= 1 (piem-tu-bench--tool-block-overlay-count
    ...
    (let ((bad-texts
           (seq-filter
            (lambda (tool-call-id)
              (/= 1 (piem-tu-bench--count-occurrences
      ...
      (push (piem-tu-bench--check
            ...
            checks))
    (let ((live-count (piem-tu-bench--live-tool-block-count
                       chat-buf)))
      (push (piem-tu-bench--check
...
(defun piem-tu-bench--collect-cooling-checks
    (chat-buf drain final-state)
  "Build structural check rows for CHAT-BUF cooling DRAIN and FINAL-STATE."
  (let* ((expected (piem-tu-bench--fill-tool-count))
         (observation piem-tu-bench--agent-end-observation)
         (slices (piem-tu-bench--cooling-slices-in-order))
         (commands (piem-tu-bench--commands-in-order))
         (names (plist-get final-state :coldToolNames))
         (parser (plist-get final-state :parser))
         (expected-paths (+ piem-tu-bench-fill-read
                            piem-tu-bench-fill-write
                            piem-tu-bench-fill-edit))
         (expected-semantic-lines (+ expected 2))
         (expected-hash (piem-tu-bench--expected-semantic-hash))
         (slice-deltas
    ...
    (push
     (piem-tu-bench--check
      ...
      (and piem-tu-bench--agent-end-time
           observation
           piem-tu-bench--agent-end-filter)
      (if piem-tu-bench--agent-end-filter
          (format "agent_end event %.3f ms inside filter %d (%.3f ms)"
                  (plist-get observation :wallMs)
                  (plist-get piem-tu-bench--agent-end-filter :id)
                  (plist-get piem-tu-bench--agent-end-filter :wallMs))
     ...
     (piem-tu-bench--check
      ...
      (and (= (or (plist-get final-state :coldTools) -1) expected)
           ...
           (equal (plist-get final-state :toolOverlayIds)
                  (vector piem-tu-bench-cooling-live-id)))
    ...
    (push
     (piem-tu-bench--check
      ...
      (format "inside=%s cold-property=%s live sentinel occurrences=%d"
              ...
              (piem-tu-bench--count-occurrences
               chat-buf "COOLING-SEMANTIC-LIVE")))
     checks)
    (push
     (piem-tu-bench--check
      ...
      (push
       (piem-tu-bench--check
        ...
        (and table-pos
             ...
             (or (not piem-tu-bench-display-buffers)
    ...
    (push
     (piem-tu-bench--check
      ...
      (and (= (or (plist-get final-state :queueLength) -1) 0)
           ...
           (null piem-tu-bench--scheduler-errors))
      (format "queue=%s timer=%s live=%s pending=%s scheduler-errors=%S"
              ...
              piem-tu-bench--scheduler-errors))
     checks)
    (push
     (piem-tu-bench--check
    ...
    (push
     (piem-tu-bench--check
      ...
      (and (= (piem-tu-bench--event-count
               "tool_execution_start")
              (1+ expected))
           (= (piem-tu-bench--event-count
               "tool_execution_end")
              expected)
           (= (piem-tu-bench--event-count
               "tool_execution_update")
              0)
           (= (piem-tu-bench--event-count "agent_end") 1))
      (format "starts=%d ends=%d updates=%d agent_end=%d"
              ...
              (piem-tu-bench--event-count
               "tool_execution_update")
              (piem-tu-bench--event-count "agent_end")))
     checks)
    (when (and piem-tu-bench-display-buffers
               (not noninteractive))
...
(defun piem-tu-bench--cleanup-session-directory (session-dir)
...
(defun piem-tu-bench--cleanup-session (chat-buf session-dir)
  "Kill CHAT-BUF and its fake process, then delete generated SESSION-DIR."
  (unwind-protect
      (when (buffer-live-p chat-buf)
        (let ((input-buf (buffer-local-value 'piem--input-buffer
                                             chat-buf)))
          (with-current-buffer chat-buf
    ...
    (piem-tu-bench--cleanup-session-directory session-dir)))

(defun piem-tu-bench--run-session ()
...
the storm scenarios retain their existing prompt-to-agent_end settle path."
  (let* ((session-dir (make-temp-file
                       (expand-file-name "session-"
                                         piem-tu-bench-out-dir)
                       t))
         (cooling (piem-tu-bench--cooling-scenario-p))
         (chat-buf nil)
         (ok nil)
         (error-text nil)
         (drain nil)
    ...
    (setq piem-executable
          (list (or (executable-find "python3") (error "Python3 not found"))
                piem-tu-bench-fake-pi))
    (setq piem-extra-args
          (list "--log-file" piem-tu-bench-fake-log))
    ;; Never prompt about grammars or versions from a benchmark run.
    (setq piem-essential-grammar-action 'warn)
    (when cooling
      (setq piem-hot-tail-turn-count
            piem-tu-bench-hot-tail-turns))
    (unwind-protect
        (condition-case err
            (progn
              ...
              (let ((piem--version-probe-delay 3600))
                (setq chat-buf (piem--setup-session session-dir)))
              (when (and piem-tu-bench-display-buffers
                         (not noninteractive))
                (piem--show-session-buffers
                 chat-buf
              ...
              (unless cooling
                (piem-tu-bench--start-probe))
              (setq piem-tu-bench--prompt-time (float-time))
              (with-current-buffer chat-buf
                (piem--prepare-and-send
                 (if cooling
              ...
              (if cooling
                  (progn
                    ...
                    (unless
                        (let ((piem-tu-bench-display-buffers nil))
                          (piem-tu-bench--wait-until
                           (lambda ()
                             (let ((proc
                                    (and (buffer-live-p chat-buf)
                                         (with-current-buffer chat-buf
                               ...
                               piem-tu-bench--agent-end-time))
                           piem-tu-bench-timeout-seconds))
                      (error "Timed out waiting for cooling agent_end after %d seconds"
                             piem-tu-bench-timeout-seconds))
                    (setq drain
                          (piem-tu-bench--wait-for-natural-cooling
                           chat-buf piem-tu-bench-timeout-seconds))
                    (let ((proc (with-current-buffer chat-buf
                                  piem--process)))
                      (setq ok
                            (and (eq (plist-get drain :settled) t)
                                 (= 0
                                    (piem-tu-bench--pending-requests-count
                                     proc)))))
                    (unless ok
    ...
    (when cooling
      (when (and ok (buffer-live-p chat-buf))
        (setq parser-state
              (piem-tu-bench--parser-font-lock-state chat-buf)
              final-state
              (piem-tu-bench--cooling-final-state
    ...
    (unwind-protect
        (let ((checks
               (if cooling
                   (piem-tu-bench--collect-cooling-checks
                    chat-buf drain final-state)
                 (piem-tu-bench--collect-checks chat-buf))))
          (list :settled (piem-tu-bench--json-bool ok)
                :error error-text
                :wallMs (when piem-tu-bench--agent-end-time
                          (* 1000.0
                             (- piem-tu-bench--agent-end-time
                                piem-tu-bench--prompt-time)))
                ...
                :final final-state
                :checks checks))
      (piem-tu-bench--cleanup-session chat-buf session-dir))))

(defun piem-tu-bench--git-string (&rest args)
  "Run git with ARGS in the repository root and return trimmed output."
  (string-trim
   (with-temp-buffer
     (let ((default-directory piem-tu-bench-repo-root))
       (if (zerop (apply #'process-file "git" nil t nil args))
           (buffer-string)
         "")))))

(defun piem-tu-bench--workload-json ()
  "Return the configured workload as a JSON-encodable plist."
  (list :fillBash piem-tu-bench-fill-bash
        :fillRead piem-tu-bench-fill-read
        :fillWrite piem-tu-bench-fill-write
        :fillEdit piem-tu-bench-fill-edit
        :fillOutputLines piem-tu-bench-fill-output-lines
        :updates piem-tu-bench-updates
        :parallelTools piem-tu-bench-parallel-tools
        :gapScale piem-tu-bench-gap-scale
        :seed piem-tu-bench-seed
        :hotTailTurns piem-tu-bench-hot-tail-turns
        :commandIntervalMs
        (* 1000.0 piem-tu-bench-command-interval)
        :expectedCoolingCohort
        (if (piem-tu-bench--cooling-scenario-p)
            (piem-tu-bench--fill-tool-count)
          0)))

(defun piem-tu-bench--write-times-tsv ()
  "Write event timing and render rows to `piem-tu-bench-times-file'."
  (with-temp-file piem-tu-bench-times-file
    (insert "series\tname\tcount\ttotal_ms\tmean_ms\tmax_ms\n")
    (dolist (row (piem-tu-bench--event-stats))
      (insert (format "event\t%s\t%d\t%.3f\t%.3f\t%.3f\n"
                      ...
                      (plist-get row :maxMs))))
    (dolist (row (piem-tu-bench--render-rows))
...
(defun piem-tu-bench--write-cooling-tsv ()
  "Write detailed cooling callback and command TSV artifacts."
  (when (piem-tu-bench--cooling-scenario-p)
    (with-temp-file piem-tu-bench-cooling-slices-file
      (insert (concat
      ...
      (dolist (row (piem-tu-bench--cooling-slices-in-order))
    ...
    (with-temp-file piem-tu-bench-commands-file
      (insert (concat
      ...
      (dolist (row (piem-tu-bench--commands-in-order))
...
(defun piem-tu-bench--checks-json (entries)
...
(defun piem-tu-bench--write-result-json (metrics)
  "Write run METRICS to `piem-tu-bench-result-file'."
  (let* ((dirty (not (string-empty-p
                      (piem-tu-bench--git-string
                       "status" "--porcelain" "--untracked-files=no"))))
         (probe (piem-tu-bench--probe-stats))
         (cooling (piem-tu-bench--cooling-scenario-p))
         (filters (nreverse
                   (copy-sequence piem-tu-bench--filter-log)))
         (command-stats (piem-tu-bench--command-stats))
         (object
          (list :scenario piem-tu-bench-scenario
                :iteration piem-tu-bench-iteration
                :commit (piem-tu-bench--git-string
                         "rev-parse" "--short" "HEAD")
                :dirty (piem-tu-bench--json-bool dirty)
                :display (piem-tu-bench--json-bool
                          piem-tu-bench-display-buffers)
                :emacsVersion emacs-version
                :markdownGrammar
                (piem-tu-bench--json-bool
                 (treesit-language-available-p 'markdown))
                :workload (piem-tu-bench--workload-json)
                :timingGuidance
                ...
                :agentEnd
                (or piem-tu-bench--agent-end-observation
                    :json-null)
                :processFilters
                (list :count (length filters)
                      ...
                      (if filters
                        ...
                        0.0)
                      :agentEnd
                      (or piem-tu-bench--agent-end-filter
                          :json-null))
                :eventStats
                (vconcat
                 (mapcar (lambda (row)
                ...
                :renders (list :replaceBody
                               (piem-tu-bench--render-operation-json
                                "replace-body")
                               :displayToolEnd
                               (piem-tu-bench--render-operation-json
                                "display-tool-end"))
                ...
                (if cooling
                    (list
                     ...
                     :slices
                     (vconcat
                      (piem-tu-bench--cooling-slices-in-order))
                     :commands
                     (list :stats command-stats
                           :rows
                           (vconcat
                            (piem-tu-bench--commands-in-order)))
                     :treeRoots
                     (piem-tu-bench--tree-root-summary)
                     :schedulerErrors
                     (vconcat
                      (nreverse
                       (copy-sequence
                     ...
                     :artifacts
                     (list :slicesTsv
                           piem-tu-bench-cooling-slices-file
                           :commandsTsv
                           piem-tu-bench-commands-file))
                  :json-null)
                :checks (piem-tu-bench--checks-json
                         (plist-get metrics :checks)))))
    (with-temp-file piem-tu-bench-result-file
      (insert (json-encode object) "\n"))))

(defun piem-tu-bench--write-report (metrics run-ok)
  ...
  (let ((dirty (not (string-empty-p
        ...
        (probe (piem-tu-bench--probe-stats))
        (cooling (piem-tu-bench--cooling-scenario-p)))
    (with-temp-file piem-tu-bench-report-file
      (insert (if cooling
      ...
      (insert (format "- Visible GUI buffers: `%s`\n"
                      (if piem-tu-bench-display-buffers
                          "yes" "no")))
      ...
      (insert (format "- Fill: `%d` bash, `%d` read, `%d` write, `%d` edit completed tool executions x `%d` output lines\n"
                      ...
                      piem-tu-bench-fill-write
                      piem-tu-bench-fill-edit
                      piem-tu-bench-fill-output-lines))
      (if cooling
        ...
        (insert (format "- Storm: `%d` updates across `%d` parallel subagent tools, gap scale `%.2f`, seed `%d`\n\n"
                        piem-tu-bench-updates
                        piem-tu-bench-parallel-tools
                        piem-tu-bench-gap-scale
                        piem-tu-bench-seed)))
      ...
      (when cooling
        ...
        (let* ((agent-end piem-tu-bench--agent-end-observation)
               (filter piem-tu-bench--agent-end-filter)
               (drain (plist-get metrics :drain))
               (final (plist-get metrics :final))
               (commands (piem-tu-bench--command-stats))
               (slices (piem-tu-bench--cooling-slices-in-order))
               (slice-max (if slices
                              (apply #'max
                                     (mapcar (lambda (row)
                                             ...
                                             slices))
                            0.0))
               (tree (piem-tu-bench--tree-root-summary)))
      ...
      (dolist (operation '("replace-body" "display-tool-end"))
        (let ((aggregate (piem-tu-bench--render-operation-json
                          operation))
              (rows (seq-filter
                     (lambda (row)
                       (equal (plist-get row :operation) operation))
      ...
      (unless cooling
        ...
        (insert "|---|---:|---:|\n")
        (dolist (tool-call-id (piem-tu-bench--storm-tool-ids))
          (insert (format "| `%s` | %d | %d |\n"
                          tool-call-id
                          (piem-tu-bench--render-count
      ...
      (insert (format "- Result JSON: `%s`\n"
                      piem-tu-bench-result-file))
      (insert (format "- Timing TSV: `%s`\n"
                      piem-tu-bench-times-file))
      (when cooling
      ...
      (insert (format "- Fake RPC log without content: `%s`\n"
                      piem-tu-bench-fake-log)))))

(defun piem-tu-bench--metrics-ok-p (metrics)
...
(defun piem-tu-bench--metrics-with-verdict (metrics)
  "Return METRICS with :ok set from its complete structural verdict."
  (plist-put metrics :ok
             (piem-tu-bench--json-bool
              (piem-tu-bench--metrics-ok-p metrics))))

(defun piem-tu-bench--verdict-label (run-ok)
  "Return the report verdict label for RUN-OK."
  (if run-ok "pass" "FAIL"))

(defun piem-tu-bench--exit-status (run-ok)
  "Return the benchmark process exit status for RUN-OK."
  (if run-ok 0 1))

(defun piem-tu-bench--validate-verdict-contract ()
...
benchmark scenario.  It prevents settled-only JSON success from regressing."
  (let* ((metrics
          (piem-tu-bench--metrics-with-verdict
           (list :settled t
                 :checks
    ...
    (unless (and (eq (plist-get metrics :ok) :json-false)
                 ...
                 (equal (piem-tu-bench--verdict-label run-ok)
                        "FAIL")
                 (= (piem-tu-bench--exit-status run-ok) 1))
      (error "Benchmark verdict contract self-check failed: %s" encoded))))

;;;###autoload
(defun piem-tu-bench-run ()
...
Timing thresholds are diagnostic only and are never enforced."
  (piem-tu-bench--validate-verdict-contract)
  (when (and piem-tu-bench-display-buffers
             (not noninteractive)
             (not (display-graphic-p)))
    (error "GUI benchmark lane requires a graphic display; use xvfb-run"))
  (make-directory piem-tu-bench-out-dir t)
  (ignore-errors (delete-file piem-tu-bench-fake-log))
  (setq piem-tu-bench--event-log nil
        piem-tu-bench--prompt-time nil
        piem-tu-bench--agent-end-time nil
        piem-tu-bench--probe-expected nil
        piem-tu-bench--probe-lateness nil
        piem-tu-bench--render-log (make-hash-table :test 'equal)
        piem-tu-bench--current-filter-id nil
        piem-tu-bench--filter-sequence 0
        piem-tu-bench--filter-log nil
        piem-tu-bench--agent-end-observation nil
        piem-tu-bench--agent-end-filter nil
        piem-tu-bench--drain-start-time nil
        piem-tu-bench--cooling-slice-log nil
        piem-tu-bench--cooling-slice-sequence 0
        piem-tu-bench--scheduler-errors nil
        piem-tu-bench--cooling-chat-buffer nil
        piem-tu-bench--command-timer nil
        piem-tu-bench--command-pending nil
        piem-tu-bench--command-log nil
        piem-tu-bench--command-sequence 0
        piem-tu-bench--command-current-job nil
        piem-tu-bench--command-drain-active nil
        piem-tu-bench--command-last-route "follow"
        piem-tu-bench--window-sentinel-marker nil)
  (piem-tu-bench--install-advice)
  (unwind-protect
      (let* ((metrics
              (piem-tu-bench--metrics-with-verdict
               (piem-tu-bench--run-session)))
             (run-ok (eq (plist-get metrics :ok) t)))
        (piem-tu-bench--write-times-tsv)
        (piem-tu-bench--write-cooling-tsv)
        (piem-tu-bench--write-result-json metrics)
        (piem-tu-bench--write-report metrics run-ok)
        ...
        (when (piem-tu-bench--cooling-scenario-p)
          ...
          (princ (format "Wrote %s\n"
                         piem-tu-bench-commands-file)))
        (princ (format "Wrote %s\n" piem-tu-bench-report-file))
        run-ok)
    (piem-tu-bench--remove-advice)))

(defun piem-tu-bench-run-batch ()
  "Run one tool-update benchmark iteration in batch mode and exit."
  (let ((standard-output #'external-debugging-output))
    (kill-emacs
     (piem-tu-bench--exit-status
      (piem-tu-bench-run)))))

(provide 'piem-tool-update-bench)
;;; piem-tool-update-bench.el ends here
