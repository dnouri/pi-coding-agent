...
BENCH_MARKER="$OUT_DIR/.pilish-reload-resume-bench"
if [[ -L "$OUT_DIR" ]]; then
    ...
    for ((iter = 1; iter <= REPS; iter++)); do
        ...
        if [[ "$BATCH" = "1" ]]; then
            ...
            if ! xvfb-run -a env GDK_BACKEND=x11 PATH="$PATH" \
                ...
                --eval "(let ((standard-output #'external-debugging-output)) (kill-emacs (if (pilish-rr-bench-run) 0 1)))" \
