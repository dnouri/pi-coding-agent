;;; piem-table.el --- Display-only table decoration -*- lexical-binding: t; -*-
...
;; Display-only markdown pipe table rendering for piem chat buffers.
;;
...
;; - `piem--decorate-tables-in-region' — stable path
;; - `piem--maybe-decorate-streaming-table' — streaming path
;; - `piem--maybe-refresh-hot-tail-tables' — resize path
;;
;; Depends on `piem-ui' for visible-text extraction and
...
(defvar piem--visible-string-buffer nil)

;;;; User Options

(defcustom piem-table-cell-render-function nil
...
`piem--markdown-visible-string' (markdown fontification).
...
(defvar-local piem--last-table-display-width nil
  "Last visible chat width used for display-only table wrapping.")

(defvar-local piem--table-decoration-pending nil
...
(defvar-local piem--visible-string-cache
  nil
  "Session-level cache for `piem--markdown-visible-string'.
...
(defun piem--chat-display-window ()
...
(defun piem--chat-buffer-hidden-p ()
...
buffers are not affected by the visibility guard."
  (and (not noninteractive)
       (null (piem--chat-display-window))))

(defun piem--chat-window-width ()
...
visible after streaming while hidden."
  (when-let* ((width (piem--chat-window-width)))
    (when (or piem--table-decoration-pending
              (not (equal width piem--last-table-display-width)))
      (setq piem--table-decoration-pending nil)
      (setq piem--last-table-display-width width)
      (let ((gc-cons-threshold (max gc-cons-threshold (* 8 1024 1024))))
        (piem--with-scroll-preservation
          (save-excursion
            (piem--refresh-hot-tail-tables width)))))))
...
(defvar piem--treesit-table-query nil
  "Pre-compiled tree-sitter query for pipe_table nodes.")

(defvar piem--treesit-data-row-query nil
  "Pre-compiled tree-sitter query for pipe_table data rows.")

(defun piem--ensure-treesit-queries ()
...
that thrashes tree-sitter's single-entry query cache."
  (unless piem--treesit-table-query
    (let ((lang (treesit-parser-language
                 (piem--markdown-parser))))
      (setq piem--treesit-table-query
            (treesit-query-compile lang '((pipe_table) @table)))
      (setq piem--treesit-data-row-query
            (treesit-query-compile lang '((pipe_table (pipe_table_row) @row)))))))

(defun piem--treesit-table-regions (beg end)
...
whose start position falls within the range."
  (let ((regions nil))
    (when-let* ((parser (piem--markdown-parser)))
      (piem--ensure-treesit-queries)
      (let ((captures (treesit-query-capture
                       (treesit-parser-root-node parser)
                       piem--treesit-table-query
                       beg end)))
        (dolist (cap captures)
...
(defun piem--remove-table-overlays (beg end)
  "Remove display-only table overlays in the region BEG..END."
  (dolist (ov (overlays-in beg end))
    (when (overlay-get ov 'piem-table-display)
      (delete-overlay ov))))

(defun piem--table-has-data-row-p (beg end)
...
decorating (header + separator alone is not enough)."
  (when-let* ((parser (piem--markdown-parser)))
    ...
    (let ((captures (treesit-query-capture
                     (treesit-parser-root-node parser)
                     piem--treesit-data-row-query
                     beg end)))
      (cl-some (lambda (cap) (eq (car cap) 'row)) captures))))

;;;; Streaming Decoration

(defun piem--maybe-decorate-streaming-table ()
...
visible window; sets `piem--table-decoration-pending'
  ...
  (when (and piem--message-start-marker
    ...
    (if (piem--chat-buffer-hidden-p)
      ...
      (let* ((start (marker-position piem--message-start-marker))
             ...
             (regions (piem--treesit-table-regions start end)))
        (when-let* ((last-region (car (last regions))))
          (let ((width (piem--chat-display-width)))
            (setq piem--last-table-display-width width)
            (when (piem--table-has-data-row-p
                   (car last-region) (cdr last-region))
              (piem--remove-table-overlays
               (car last-region) (cdr last-region))
              (piem--decorate-table
...
(defun piem--visible-string-buffer ()
  "Return the persistent fontification buffer, creating it if needed."
  (unless (buffer-live-p piem--visible-string-buffer)
    (setq piem--visible-string-buffer
          (generate-new-buffer " *pi-visible-string*"))
    (with-current-buffer piem--visible-string-buffer
      (md-ts-mode)
      (setq-local md-ts-hide-markup t)
      (md-ts--set-hide-markup t)))
  piem--visible-string-buffer)

(defconst piem--markdown-inline-chars-re "[*_`~\\[]"
...
(defun piem--markdown-visible-string (markdown)
  "Return the visible chat-buffer rendering of MARKDOWN.
This hides markdown delimiters the same way `piem-chat-mode'
...
Cells with no inline syntax characters are returned as-is."
  (if (not (string-match-p piem--markdown-inline-chars-re markdown))
      markdown
    (with-current-buffer (piem--visible-string-buffer)
      ...
      (condition-case-unless-debug nil
          (font-lock-ensure)
        (error nil))
      (piem--visible-text (point-min) (point-max)))))

;;;; Cell Rendering

(defun piem--render-table-row-lines (cells col-widths aligns)
  "Render table CELLS into display lines using COL-WIDTHS and ALIGNS.
When `piem-prettify-tables' is non-nil, emits Unicode
...
(defun piem--render-table-separator-line (col-widths aligns)
  "Render the separator line for COL-WIDTHS and ALIGNS.
When `piem-prettify-tables' is non-nil, emits a box-drawing
rule (├─┼─┤) directly; otherwise emits standard markdown syntax."
  (if piem-prettify-tables
...
(defun piem--table-alignments (separator-line)
...
(defun piem--table-display-groups (raw-lines width)
...
Plain tables (no prefix) take a fast path that skips prefix splitting."
  (let* ((no-prefix (cl-every (lambda (l) (and (> (length l) 0)
         ...
         (parts (unless no-prefix
                  (mapcar #'piem--split-table-line-prefix raw-lines)))
    ...
    (when (>= (length bare-lines) 2)
      (let* ((headers (markdown-table-wrap--split-table-row
             ...
             (aligns (piem--table-alignments (cadr bare-lines)))
             (rows (mapcar (lambda (line)
                             (markdown-table-wrap--split-table-row
                              (string-trim line)))
                           (nthcdr 2 bare-lines)))
             (visible-cache
              (or piem--visible-string-cache
                  (setq piem--visible-string-cache
                        (make-hash-table :test 'equal :size 256))))
             (display-cell
              (lambda (cell)
                (or (gethash cell visible-cache)
                    (puthash cell
                             (if piem-table-cell-render-function
                                 (funcall piem-table-cell-render-function cell)
                               (piem--markdown-visible-string cell))
                             visible-cache))))
             ...
             (header-lines
              (piem--render-table-row-lines
               display-headers col-widths aligns))
             (separator-line
              (piem--render-table-separator-line
               col-widths aligns))
             (row-groups
              (mapcar (lambda (row)
                        (piem--render-table-row-lines
...
(defun piem--table-line-mapping (raw-lines wrapped-lines)
             ...
             (t
              ...
              (let ((groups nil) (current nil))
                (dolist (line data-wrapped)
                  (if (piem--table-spacer-line-p line)
                      (when current
...
(defconst piem--visual-face-attrs
...
(defun piem--face-visual-attr (face-val attr)
  ...
  (cond
   ...
   ((listp face-val)
    (cl-loop for f in face-val
             for v = (piem--face-visual-attr f attr)
             unless (eq v 'unspecified) return v
             finally return 'unspecified))
   (t 'unspecified)))

(defun piem--visual-face-spec (face-val)
...
Returns nil when the face contributes no visual attributes."
  (let (spec)
    (dolist (attr piem--visual-face-attrs)
      (let ((val (piem--face-visual-attr face-val attr)))
        (unless (eq val 'unspecified)
          (setq spec (plist-put spec attr val)))))
    spec))

(defun piem--neutralize-fonts (str)
  ...
  (let ((result (copy-sequence str))
    ...
    (while (< pos len)
      (let* ((next (next-single-property-change pos 'face result len))
        ...
        (when face-val
          (let ((spec (piem--visual-face-spec face-val)))
            (if spec
...
(defun piem--decorate-table (beg end width)
...
blank-line spacing between a table and following text is not collapsed."
  (let* ((table-beg (save-excursion (goto-char beg) (line-beginning-position)))
    ...
    (when-let* ((raw-lines (split-string trimmed "\n"))
                (groups (piem--table-display-groups raw-lines width)))
            ...
            (dotimes (i n)
              (let* ((line-beg (line-beginning-position))
                ...
                (overlay-put ov 'display
                             (piem--neutralize-fonts display-str))
                (overlay-put ov 'face 'default)
                (overlay-put ov 'piem-table-display t)
                (overlay-put ov 'evaporate t))
              (forward-line 1))))
        t))))

(defun piem--decorate-tables-in-region (beg end &optional width)
...
`piem--chat-display-width'.  Tables inside fenced code
...
Idempotent: existing table overlays in the region are removed first."
  (let ((width (or width (piem--chat-display-width)))
    ...
    (setq piem--last-table-display-width width)
    (piem--remove-table-overlays beg end)
    (dolist (region (piem--treesit-table-regions beg end))
      (when (piem--table-has-data-row-p (car region) (cdr region))
        (piem--decorate-table (car region) (cdr region) width)))))

;;;; Cleanup

(defun piem--cleanup-visible-string-buffer ()
  "Kill the persistent visible-string fontification buffer.
Called from `piem--cleanup-on-kill' when a chat buffer dies."
  (when (buffer-live-p piem--visible-string-buffer)
    (kill-buffer piem--visible-string-buffer)
    (setq piem--visible-string-buffer nil)))
...
;; with a lightweight overlay (`piem-table-raw') so the
...
(defun piem--table-raw-p (beg end)
  "Return non-nil when the table at BEG..END is marked raw (toggled off)."
  (cl-some (lambda (ov) (overlay-get ov 'piem-table-raw))
           (overlays-in beg end)))

(defun piem--table-pretty-p (beg end)
  "Return non-nil when the table at BEG..END currently has display overlays."
  (cl-some (lambda (ov) (overlay-get ov 'piem-table-display))
           (overlays-in beg end)))

(defun piem--mark-table-raw (beg end)
  "Mark the table at BEG..END as raw so resize won't re-decorate it."
  (piem--unmark-table-raw beg end)
  (let ((ov (make-overlay beg end nil nil nil)))
    (overlay-put ov 'piem-table-raw t)
    (overlay-put ov 'evaporate t)))

(defun piem--unmark-table-raw (beg end)
  "Remove the raw marker from the table at BEG..END."
  (dolist (ov (overlays-in beg end))
    (when (overlay-get ov 'piem-table-raw)
      (delete-overlay ov))))

(defun piem--table-at-point ()
...
(defun piem--force-table-state (state regions width)
    ...
    (let ((beg (car r)) (end (cdr r)))
      (pcase state
        ('pretty
         (piem--unmark-table-raw beg end)
         (piem--remove-table-overlays beg end)
         (piem--decorate-table beg end width))
        ('raw
         (piem--remove-table-overlays beg end)
         (piem--mark-table-raw beg end))))))

(defun piem-toggle-table-pretty (&optional arg)
    ...
    (cond
     ...
     ((use-region-p)
      (let* ((regs (piem--treesit-table-regions
        ...
        (piem--force-table-state state regs width)
        (message "Region tables %s (%d)"
                 (if (eq state 'raw) "raw" "pretty") (length regs))))
     ;; Point on a table → toggle it.
     ((piem--table-at-point)
      (let* ((bounds (piem--table-at-point))
             (pretty (piem--table-pretty-p
                      (car bounds) (cdr bounds))))
        (piem--force-table-state
...
(defun piem--skip-raw-tables (orig-fn beg end width)
  "Call ORIG-FN to decorate the table at BEG..END at WIDTH, unless raw.
Used as `:around' advice on `piem--decorate-table' so the
resize / resume / hot-tail re-decoration path skips tables marked raw."
  (unless (piem--table-raw-p beg end)
    (funcall orig-fn beg end width)))
(advice-add 'piem--decorate-table :around
            #'piem--skip-raw-tables)

(provide 'piem-table)
;;; piem-table.el ends here
