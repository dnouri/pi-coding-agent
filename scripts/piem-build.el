;;; piem-build.el --- Batch helpers for local build tasks -*- lexical-binding: t; -*-
...
(defvar piem-grammar-recipes nil
  "Grammar recipes provided by `piem-grammars'.")

(defconst piem-build-main-file
  (expand-file-name "../piem.el"
                    (file-name-directory (or load-file-name buffer-file-name)))
  "Path to the package entry point used as build metadata source of truth.")

(defun piem-build--version-string (version)
  "Return VERSION list as a dotted string."
  (mapconcat #'number-to-string version "."))

(defun piem-build--read-package-requires ()
    ...
    (let ((header (lm-header "package-requires")))
      (unless header
        (error "Missing Package-Requires header in %s"
               piem-build-main-file))
      (car (read-from-string header)))))

(defun piem-build-package-requirements ()
  "Return non-Emacs package requirements from `piem.el'."
  (mapcar (pcase-lambda (`(,package ,version))
          ...
          (cl-remove-if (lambda (requirement)
                        ...
                        (with-temp-buffer
                          (insert-file-contents piem-build-main-file)
                          (piem-build--read-package-requires)))))

(defun piem-build--package-missing-p (package min-version)
  "Return non-nil when PACKAGE does not satisfy MIN-VERSION."
  (not (package-installed-p package min-version)))

(defun piem-build-install-deps (&optional requirements)
  "Install package REQUIREMENTS for local development.
REQUIREMENTS defaults to `piem-build-package-requirements'.
  ...
  (let ((requirements (or requirements
    ...
    (dolist (requirement requirements)
      (pcase-let ((`(,package . ,min-version) requirement))
        (when (piem-build--package-missing-p package min-version)
          (message "Installing dependency %s >= %s..."
                   package
                   (piem-build--version-string min-version))
...
(defun piem-build--default-grammars ()
  "Return all grammars needed by piem."
  (require 'piem-grammars)
  (mapcar #'car piem-grammar-recipes))

(defun piem-build-install-grammars (&optional grammars)
  "Install tree-sitter GRAMMARS used by piem.
GRAMMARS defaults to all recipes from `piem-grammar-recipes'.
...
failed, and total grammars.  Signals an error if any grammar fails."
  (let* ((grammars (or grammars (piem-build--default-grammars)))
         (already-installed 0)
         (installed 0)
         (failed 0)
         (failed-grammars nil)
