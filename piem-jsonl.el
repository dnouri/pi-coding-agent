;;; piem-jsonl.el --- JSONL session reading and tree projection -*- lexical-binding: t; -*-
...
;;   `piem--jsonl-entry-parent-id' or core's
;;   `piem--normalize-string-or-null', and this module never
...
;; - `piem--jsonl-shorten-path' replaces the HOME (or
...
;;   in practice.
;; - `piem-jsonl-read-session-info' reports :modified from
...
;; - `piem-jsonl-navigation-target's :current-p refines pi's
...
;; `piem--jsonl-encode-args', which normalizes nested JSON
...
(defun piem--jsonl-entry-parent-id (entry)
  "Return the normalized parent id of ENTRY: a string, or nil.
JSON null, absent, and non-string values all read as nil."
  (piem--normalize-string-or-null (plist-get entry :parentId)))

(defun piem--jsonl-filtered-entry-p (type)
...
(defun piem--jsonl-parse-session-header (line)
...
`piem-jsonl-navigation-lines' strips a copy for this
check only."
  (let ((data (piem--parse-json-line line)))
    (when (and (consp data)
               (equal (plist-get data :type) "session"))
      data)))

(defun piem-jsonl-read-file (path)
...
the session header (`piem--jsonl-parse-session-header's
  ...
  (when (file-readable-p path)
    (with-temp-buffer
      ...
      (while (and (not (eobp))
                 (looking-at-p "[ \t\r]*$"))
        (forward-line 1))
      (let ((header (piem--jsonl-parse-session-header
                ...
                (let ((type (plist-get data :type)))
                  (unless (equal type "session")
                    ...
                    (when (equal type "session_info")
                      (let ((raw (piem--normalize-string-or-null
                                  (plist-get data :name))))
                        (setq name
                              (when raw
...
(defconst piem--jsonl-line-type-re
  "[ \t]*{[ \t]*\"type\"[ \t]*:[ \t]*\"%s\""
...
(defun piem--jsonl-line-type-p (type)
  "Return non-nil when the current line has top-level session TYPE."
  (looking-at-p (format piem--jsonl-line-type-re
                        (regexp-quote type))))

(defun piem--jsonl-parse-current-line ()
  "Return the current line parsed as a plist, or nil when malformed."
  (piem--parse-json-line
   (buffer-substring-no-properties (point) (line-end-position))))

(defun piem-jsonl-sessions-root (&optional anchor)
...
(defun piem-jsonl-session-dir-for-cwd (cwd &optional root)
...
ROOT defaults to `piem-jsonl-sessions-root'.  The result
...
--C--x--, exactly like pi's character class."
  (let* ((base (directory-file-name
                (or root (piem-jsonl-sessions-root))))
    ...
    (when header
      ...
      (while (not (eobp))
        (cond
         ...
         ((piem--jsonl-line-type-p "session_info")
          (when-let* ((data (piem--jsonl-parse-current-line)))
            (let ((raw (piem--normalize-string-or-null
                        (plist-get data :name))))
         ...
         (t nil))
        (forward-line 1))
      (let ((id (piem--normalize-string-or-null
                 (plist-get header :id)))
            (cwd (piem--normalize-string-or-null
                  (plist-get header :cwd)))
            (parent (piem--normalize-string-or-null
                     (plist-get header :parentSession)))
            (created (piem--normalize-string-or-null
...
(defun piem-jsonl-read-session-info (path)
...
(defun piem-jsonl-build-tree (entries)
  ...
  (let* ((count (length entries))
    ...
    (dotimes (i count)
      (let ((entry (aref entries i)))
        ...
        (when (equal (plist-get entry :type) "label")
          (let ((target (piem--normalize-string-or-null
                         (plist-get entry :targetId)))
                (label (let ((raw (piem--normalize-string-or-null
    ...
    (dotimes (i count)
      (let* ((entry (aref entries i))
             ...
             (parent (piem--jsonl-entry-parent-id entry)))
        (if (or (null parent)
                (equal parent id)
...
(defun piem--jsonl-extract-text (content &optional max-len separator)
...
(defun piem--jsonl-normalize-preview (text)
...
(defun piem--jsonl-arg-number (args key)
...
(defun piem--jsonl-arg-string (args key)
...
(defun piem--jsonl-shorten-path (path)
...
(defun piem--jsonl-encode-args (args)
...
booleans and null) to the values `json-encode' expects, recursively."
  (cond
   ...
   ((eq args :null) nil)
   ((vectorp args)
    (vconcat (mapcar #'piem--jsonl-encode-args args)))
   ((consp args)
    (let (out)
      (while (consp args)
        (let ((key (pop args)))
          (push key out)
          (push (if (consp args)
                    (piem--jsonl-encode-args (pop args))
                  nil)
                out)))
      (nreverse out)))
   (t args)))

(defun piem-jsonl-format-tool-call (name args)
...
ARGS is a plist of parsed-JSON tool arguments (nil allowed)."
  (pcase name
    ("read"
     (let* ((path (piem--jsonl-shorten-path
                   (or (piem--jsonl-arg-string args :path)
                       (piem--jsonl-arg-string args :file_path)
                       "")))
            (offset (piem--jsonl-arg-number args :offset))
            (limit (piem--jsonl-arg-number args :limit)))
    ...
    ("bash"
     (let* ((command (or (piem--jsonl-arg-string args :command) ""))
            (normalized (piem--jsonl-normalize-preview command))
            (truncated (substring normalized 0 (min 50 (length normalized)))))
       (format "[bash: %s%s]" truncated
               (if (> (length normalized) 50) "..." ""))))
    ("grep"
     (format "[grep: /%s/ in %s]"
             (or (piem--jsonl-arg-string args :pattern) "")
             (piem--jsonl-shorten-path
              (or (piem--jsonl-arg-string args :path) "."))))
    ("find"
     (format "[find: %s in %s]"
             (or (piem--jsonl-arg-string args :pattern) "")
    ...
    (_
     (let* ((json (if args
                       (json-encode (piem--jsonl-encode-args args))
                     "{}"))
...
(defun piem--jsonl-assistant-tool-calls (message)
...
(defun piem--jsonl-build-tool-call-map (roots)
...
hides.  :stopReason is always present; :errorMessage only when set."
  (let* ((text (piem--jsonl-extract-text
                (plist-get message :content) 200))
         (stop-reason (piem--normalize-string-or-null
                       (plist-get message :stopReason)))
         (error-message (piem--normalize-string-or-null
         ...
         (preview (cond
                   ...
                   ((and error-message (> (length error-message) 0))
                    (piem--jsonl-normalize-preview error-message))
                   (t "(no content)"))))
    (append base
...
(defun piem--jsonl-project-tool-result (base message branch-calls global-calls)
...
(defun piem--jsonl-project-entry (base entry)
  ...
  (pcase (plist-get entry :type)
    ...
    ("model_change"
     (let ((provider (piem--normalize-string-or-null
                      (plist-get entry :provider)))
           (model-id (piem--normalize-string-or-null
...
(defun piem--jsonl-resolve-projected-leaf-id (roots leaf-id)
    ...
    (let ((parent-by-id (make-hash-table :test #'equal))
      ...
      (while stack
        (let* ((node (pop stack))
               (entry (plist-get node :entry))
               (id (plist-get entry :id)))
          (puthash id (piem--jsonl-entry-parent-id entry)
                   parent-by-id)
          (unless (piem--jsonl-filtered-entry-p
...
nearest visible entry.  Traversal is iterative."
  (let* ((global-calls (piem--jsonl-build-tool-call-map tree))
    ...
    (while stack
      (pcase-let ((`(,node ,parent-visible ,branch-calls) (pop stack)))
        (let ((entry (plist-get node :entry)))
          (if (piem--jsonl-filtered-entry-p
               (plist-get entry :type))
            ...
            (let ((child-map branch-calls))
              (when (and (equal (plist-get entry :type) "message")
                         (equal (plist-get (plist-get entry :message) :role)
                                "assistant"))
                (let ((calls (piem--jsonl-assistant-tool-calls
                              (plist-get entry :message))))
                  (when calls
    ...
    (dolist (record records)
      (pcase-let ((`(,node ,parent-visible ,branch-calls) record))
        (let* ((entry (plist-get node :entry))
               (label (piem--normalize-string-or-null
    ...
    (list :tree (vconcat built-roots)
          :leafId (piem--jsonl-resolve-projected-leaf-id
                   tree leaf-id))))

(defun piem-jsonl-project-session-file (path)
  "Read, build, and project the session file at PATH in one step.
Composition of `piem-jsonl-read-file',
`piem-jsonl-build-tree', and
`piem-jsonl-project-tree': return the (:tree :leafId)
...
reads of a file a live pi appends to concurrently."
  (when-let* ((session (piem-jsonl-read-file path)))
    (let ((built (piem-jsonl-build-tree
                  (plist-get session :entries))))
      (piem-jsonl-project-tree
       (plist-get built :tree) (plist-get built :leafId)))))

;;;; Tree Navigation

(defun piem--jsonl-resolve-visible (entries id)
...
`piem--jsonl-resolve-projected-leaf-id'.  Iterative with a
...
visible entry, both resolve to nil."
  (let ((index (make-hash-table :test #'equal)))
    (dotimes (i (length entries))
      ...
      ;; `piem-jsonl-build-tree' and pi's index build.
        ...
        (cond
         ...
         (t
          ...
          (let ((entry (gethash current index)))
            (if (and entry
                     (piem--jsonl-filtered-entry-p
                      (plist-get entry :type)))
                (setq current (piem--jsonl-entry-parent-id entry))
              (setq found (and entry current)))))))
      found)))

(defun piem-jsonl-navigation-target (session target-id)
  "Return the navigation target for TARGET-ID within SESSION.
SESSION is a `piem-jsonl-read-file' result; return nil when
...
the plist (:leaf-id ID-OR-NIL :prefill TEXT? :current-p BOOL):
  ...
  - :current-p reports that the file already sits on the computed
    ...
    raw leaf (`piem--jsonl-resolve-visible' on both sides,
      ...
      (let* ((rewind-p (or (equal (plist-get entry :type) "custom_message")
                           (and (equal (plist-get entry :type) "message")
                                (equal (plist-get (plist-get entry :message) :role)
                                       "user"))))
             (leaf-id (if rewind-p
                          (piem--jsonl-entry-parent-id entry)
                        target-id))
             (text (when rewind-p
                     (piem--jsonl-extract-text
        ...
        (append
         ...
         (when (and text (not (string-empty-p text)))
           (list :prefill text))
         (list :current-p
               (equal (piem--jsonl-resolve-visible entries leaf-id)
                      (piem--jsonl-resolve-visible entries raw-leaf))))))))

(defun piem-jsonl-navigation-lines (path leaf-id)
...
not the session header (`piem--jsonl-parse-session-header's
...
`piem-jsonl-build-tree's roots rule); malformed and blank
          ...
          (with-temp-buffer
            ...
            (let* ((contents (buffer-substring-no-properties
              ...
              (when (and header-index
                         (piem--jsonl-parse-session-header
