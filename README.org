...
(use-package pi-coding-agent
  ...
  (pi-coding-agent-hot-tool-turn-floor 4)         ; Keep the newest 4 tool-heavy turns fully rich
  (pi-coding-agent-hot-tool-byte-budget 65536)    ; Cool older rich tool history beyond ~64KB
...
Old tool-heavy history cools automatically at stable boundaries: when a
turn finishes and when session history is reloaded.  Recent tool turns
stay fully rich; older completed tool blocks fall back to a cheaper
preview-only fenced form without per-block overlays or language-tagged
syntax highlighting.  Tune =pi-coding-agent-hot-tool-turn-floor= and
=pi-coding-agent-hot-tool-byte-budget= if you want a larger or smaller
rich tail.
