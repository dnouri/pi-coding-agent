...
automatically when Evil is in use; to opt out, set this before loading
the package:
...
Setup also enables =pi-coding-agent-copy-raw-markdown= so that yanking
from the chat buffer preserves code fences and markup.  Each part can
be tuned independently before loading:

#+begin_src emacs-lisp
;; Different initial states:
(setq pi-coding-agent-evil-chat-state 'normal)
(setq pi-coding-agent-evil-input-state 'emacs)
...
shadow the chat buffer's =f= binding, so setup adds
=pi-coding-agent-chat-mode= to =evil-snipe-disabled-modes= for new
chat buffers — the same treatment =magit-mode= gets by default.  Fork
stays on =f=, and char-finding remains available on =F=, =t=, and
=T=.  Set =pi-coding-agent-evil-disable-snipe= to nil to keep snipe
active in chat buffers.
