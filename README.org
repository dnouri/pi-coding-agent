...
scope, =f= for named sessions only, =r= to rename, and =d= to delete
the session at point after confirmation (trashed when
=delete-by-moving-to-trash= is set; the currently active session is
refused).  Press =?= to see these commands in a small browser menu.
