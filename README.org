...
#+html: <a href="https://melpa.org/#/piem"><img alt="MELPA" src="https://melpa.org/packages/piem-badge.svg"/></a>
#+html: <a href="https://github.com/dnouri/piem/actions/workflows/test-unit.yml"><img alt="Unit Tests" src="https://github.com/dnouri/piem/actions/workflows/test-unit.yml/badge.svg"/></a>
#+html: <a href="https://github.com/dnouri/piem/actions/workflows/test-integration.yml"><img alt="Integration Tests" src="https://github.com/dnouri/piem/actions/workflows/test-integration.yml/badge.svg"/></a>
#+html: <a href="https://github.com/dnouri/piem/actions/workflows/test-gui.yml"><img alt="GUI Tests" src="https://github.com/dnouri/piem/actions/workflows/test-gui.yml/badge.svg"/></a>
#+html: <a href="https://github.com/dnouri/piem/actions/workflows/nightly.yml"><img alt="Nightly" src="https://github.com/dnouri/piem/actions/workflows/nightly.yml/badge.svg"/></a>
#+html: <a href="https://github.com/dnouri/piem/actions/workflows/melpazoid.yml"><img alt="melpazoid" src="https://github.com/dnouri/piem/actions/workflows/melpazoid.yml/badge.svg"/></a>
...
#+html: <a href="https://danielnouri.org/media/pi-coding-agent-demo.mp4"><img src="https://danielnouri.org/media/pi-coding-agent-preview.gif" alt="Narrated piem demo video (6:38) - click to play" width="600" height="600"></a>
...
Install from [[https://melpa.org/#/piem][MELPA]].  MELPA alone does not
...
#+begin_src
M-x package-install RET piem RET
#+end_src
...
On first start, =piem= may prompt to install the
...
1. *Delete the old package* after installing =piem=:

   #+begin_src
   M-x package-delete RET pi-coding-agent RET
   #+end_src

   This step is required: while both packages are installed, the old
   package's directory wins every =pi-coding-agent= lookup
   deterministically, so old code silently keeps running the old
   package and anything that also loads =piem= ends up with two
   independent variable and face families.  package.el warns about
   none of this; the only notice you get is a =piem= warning when
   loading =piem= while the old package is still installed.

2. Old-name =setq= and =custom-set-variables= values that are set
   /before/ =piem= loads are migrated to the new =piem-*=
   variables automatically, and a value set under the new name wins
   over one set under the old name.

3. Custom themes that reference old face names (=pi-coding-agent-*=)
   must be enabled /after/ =piem= loads.  Face aliases cannot
   retro-map specs a theme recorded under the old names before the
   aliases existed; re-enabling the theme (or renaming the faces in
   it) after loading =piem= applies them.

#+html: <a id="everyday-workflow"></a>
* Everyday workflow 🧭

Start a session with =M-x piem=.  =piem= opens
two windows:
...
piem never reopens the argument path, and text with obvious scripts
...
=M-x piem= again from a Pi buffer restores missing panes;
if both panes are visible, focus moves to the input window.  Use =M-x
piem-toggle= to hide or show the session windows in the
current frame.

=C-c C-r=, =/resume=, or =M-x piem-session-browser= opens
...
piem= and enter a name.  This is an Emacs live-session
...
press =C-c C-p R= (or =M-x piem-reload=) to restart the Pi
...
=piem= runs the command in =piem-executable=,
...
=piem-executable=.  An example:

#+begin_src emacs-lisp
;; npx users:
(setopt piem-executable
...
=piem= passes =--approve= by default so project-local
=.pi= prompts, skills, settings, themes, and extensions are active.

Set =piem-project-trust-policy= to =default= to pass no
...
(setopt piem-project-trust-policy 'default)
#+end_src
...
When the current project is a TRAMP directory, =piem=
...
=piem-executable= are interpreted on the host where Pi runs.
...
Use an absolute path in =piem-executable=, or add the
...
=piem= is the Emacs frontend for Pi.  Pi is an open,
...
- =piem=: best when you want the Pi runtime (providers,
...
=piem= into the rest of your Emacs setup.
...
(setopt piem-visit-file-other-window nil)
#+end_src
...
;; (setopt piem-input-markdown-highlighting nil)

;; Show more tool output before it collapses, counted in visual lines:
;; (setopt piem-tool-preview-lines 20)
;; (setopt piem-bash-preview-lines 10)

;; Lower the 3 MiB source limit for an outgoing prompt image:
;; (setopt piem-prompt-image-max-bytes (* 2 1024 1024))
...
;; (setopt piem-image-preview-max-width 640)
;; (setopt piem-image-preview-max-bytes (* 5 1024 1024))

;; Lower context warning/error colors in the header line:
;; (setopt piem-context-warning-threshold 40)
;; (setopt piem-context-error-threshold 60)

;; Keep more recent turns live for table rewrapping and tool overlays:
;; Older history is cooled to keep long sessions fast.
;; (setopt piem-hot-tail-turn-count 5)
...
;; (setopt piem-thinking-hidden-preview nil)
#+end_src

You can also inspect the whole customization group with:

#+begin_src
M-x customize-group RET piem RET
#+end_src

** Evil integration 😈

Optional Evil keybindings ship in =piem-evil.el=, modeled on
...
#+begin_src emacs-lisp
(setq piem-evil-integration nil)
#+end_src
...
Setup also sets =piem-copy-raw-markdown= buffer-locally in
...
(setq piem-evil-chat-state 'normal)
...
(setq piem-evil-input-state 'normal)

;; Keep copying only visible text instead of raw Markdown:
(setq piem-evil-copy-raw-markdown nil)
#+end_src

User bindings made after =piem-evil-setup= runs (e.g. with
=evil-define-key= or Doom's =map!=) take precedence over these
defaults, and =M-x piem-evil-setup= re-applies them after
changing options.
...
=piem-evil-disable-snipe= to nil to keep snipe active in
chat buffers.

** Display of thinking 💭

New chat buffers inherit =piem-thinking-display=.  The
...
=piem-thinking-display= in your init file or via =M-x
customize-option=.
...
=piem-hot-tail-turn-count= to choose how many recent
...
=piem-extension-status-faces=.

** Tree-sitter grammar management 🌳

=piem= uses Emacs's built-in tree-sitter support to render
...
=piem-essential-grammar-action= to =warn= to suppress the
essential grammar prompt:

#+begin_src
(setopt piem-essential-grammar-action 'warn)
#+end_src

** Activity phase hooks 🪝

For custom UI changes tied to activity, add functions to
=piem-activity-phase-functions=.  Each function receives:
...
Handlers should be idempotent.  =piem= may reapply the same
...
(add-hook 'piem-activity-phase-functions
...
automatically.  =piem= uses =md-ts-mode= only for its own
...
#+begin_src bash
git clone https://github.com/dnouri/piem ~/.emacs.d/site-lisp/piem
#+end_src

#+begin_src emacs-lisp
(require 'package)
(package-initialize)
(add-to-list 'load-path "~/.emacs.d/site-lisp/piem")
(require 'piem)
#+end_src
...
#+begin_src emacs-lisp
(use-package piem
  :load-path "~/.emacs.d/site-lisp/piem"
  :init (defalias 'pi 'piem))
#+end_src

If you prefer not to install the Pi CLI globally, customize
=piem-executable=.  For example:
