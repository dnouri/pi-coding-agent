...
The default local integration target runs both lanes, so it still needs
Docker for Ollama.  The fake-only lane does not.

#+begin_src bash
# Byte-compile, lint, and unit tests (fast, no Docker)
make check

# Shared integration contract: fake + real (requires Docker for the real lane)
...
# Run a single integration module or contract by selector
...
- =test-integration.yml= - Shared integration contract against fake pi and real pi
