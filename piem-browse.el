;;; piem-browse.el --- Session and tree browser -*- lexical-binding: t; -*-
...
;; Session and tree browsing for piem.
;;
...
(declare-function piem-set-session-name "piem-menu" (name))
(declare-function piem--resume-selected-session "piem-menu"
                  (proc chat-buf selected-path))
(declare-function piem--session-transition-ready-p "piem-menu"
                  (chat-buf action))
(declare-function piem--session-file-cwd-or-error "piem-menu"
                  (path))
(declare-function piem--session-list-directory "piem-menu"
                  (&optional chat-buf))

;;;; Response Parsers

(defun piem--parse-tree (response)
...
(defun piem--collapse-whitespace (str)
  "Collapse whitespace (including newlines) in STR to a single space."
  (replace-regexp-in-string "[\n\r\t ]+" " " str))

(defun piem--first-nonempty-line (str)
...
(defun piem--session-display-name (session)
...
Newlines and excess whitespace are collapsed to single spaces."
  (let ((raw (or (piem--normalize-string-or-null
                  (plist-get session :name))
                 (piem--normalize-string-or-null
                  (plist-get session :firstMessage)))))
    (if raw
        (piem--collapse-whitespace raw)
      "[empty session]")))

;;;; Margin Rendering Infrastructure

(defun piem--propertize-face (string face)
...
(defun piem--make-margin-overlay (string)
...
(defconst piem--session-margin-width 20
...
(defun piem--browse-apply-margins ()
  "Re-apply right margins for the current browse buffer.
Reads width from `piem--browse-margin-width'.
Intended as a `window-configuration-change-hook' callback."
  (when piem--browse-margin-width
    (piem--browse-set-window-margins
     piem--browse-margin-width)))

;;;; Margin Age Formatting

(defconst piem--age-spec
  '(("year"   31557600)
    ("month"   2629800)
    ("week"     604800)
    ("day"       86400)
...
Example: \" 5 minutes\", \" 1 hour   \", \"10 days   \"."
  (let* ((pair (piem--margin-age seconds))
         (count (car pair))
         (unit (cdr pair))
         (unit-str (if (= count 1) unit (concat unit "s"))))
    (format piem--margin-age-format count unit-str)))

(defun piem--format-margin-age-from-iso (iso-timestamp)
...
Returns nil on invalid input."
  (condition-case nil
      (let* ((time (date-to-time iso-timestamp))
        ...
        (piem--format-margin-age (max 0 diff)))
    (error nil)))

;;;; Tree Helpers

(defun piem--active-path-ids (tree leaf-id)
...
(defconst piem--empty-assistant-preview "(no content)"
...
(defun piem--browse-node-empty-assistant-p (node)
    ...
    (and (equal type "message")
         ...
         (let ((preview (or (plist-get node :preview) "")))
           (or (string-empty-p preview)
               (equal preview piem--empty-assistant-preview)))
         (not (equal (plist-get node :stopReason) "aborted"))
         (not (plist-get node :errorMessage)))))

(defun piem--browse-node-visible-p (node filter-mode)
...
Filtering is two-phase (matching TUI tree-selector.ts:282-311):
  ...
  (if (piem--browse-node-empty-assistant-p node)
...
(defun piem--flatten-tree-for-display (tree leaf-id filter-mode)
...
contains tree connectors and gutter characters for visual structure."
  (let ((active-ids (piem--active-path-ids tree leaf-id))
        (result nil))
    (piem--flatten-tree-walk
     (append tree nil) 0 active-ids filter-mode
     nil nil
     (lambda (node indent prefix) (push (list node indent prefix) result)))
    (nreverse result)))

(defun piem--flatten-tree-walk (nodes indent active-ids filter-mode
...
Uses an explicit stack to avoid overflow on deep trees."
  ...
  (let* ((vis-count (cl-count-if
                     (lambda (n)
                       (piem--browse-node-visible-p n filter-mode))
                     nodes))
      ...
      (let* ((frame (pop stack))
        ...
        (when siblings
          (let* ((node (car siblings))
                 (rest (cdr siblings))
                 (is-visible (piem--browse-node-visible-p
                              node filter-mode))
                 (children (plist-get node :children))
...
(defun piem--sort-active-first (children active-ids)
...
ACTIVE-IDS is the hash table of active path node IDs."
  (let ((active nil)
    ...
    (dolist (child children)
      (if (piem--subtree-contains-active-p child active-ids)
          (push child active)
        (push child inactive)))
    (append (nreverse active) (nreverse inactive))))

(defun piem--subtree-contains-active-p (node active-ids)
...
(defun piem--matches-filter-p (text tokens)
...
(defconst piem--session-sort-modes
  '("threaded" "recent" "relevance")
  "Available sort modes for the session browser.")

(defun piem--session-sort-next (current)
  "Return the sort mode after CURRENT in the cycle."
  (let ((modes piem--session-sort-modes))
    (or (cadr (member current modes))
        (car modes))))

(defun piem--session-sort-items (items sort-mode)
...
(defun piem--session-thread-items (items)
  ...
  (let ((by-path (make-hash-table :test 'equal))
    ...
    (let ((result nil))
      (dolist (root (nreverse root-items))
        (setq result (piem--collect-threaded
                      root children-of 0 result)))
      (nreverse result))))

(defun piem--collect-threaded (item children-of depth result)
...
(defun piem--session-filter-named (items)
...
(defun piem--session-filter-search (items tokens)
    ...
    (cl-remove-if-not
     (lambda (item)
       (let ((text (concat
         ...
         (piem--matches-filter-p text tokens)))
     items)))

;;;; Time-Based Section Headers

(defun piem--session-time-group (iso-timestamp)
...
(defclass piem-session-section (magit-section)
  ((keymap :initform 'piem-session-section-map))
  "Section class for a session entry in the session browser.")

;;;; Keymaps

(defvar piem-browse-mode-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map magit-section-mode-map)
    (define-key map (kbd "g") #'piem-browse-refresh)
    (define-key map (kbd "q") #'quit-window)
    map)
  "Base keymap for piem browse modes.")

(defvar piem-session-browser-mode-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map piem-browse-mode-map)
    (define-key map (kbd "s") #'piem-session-browser-cycle-sort)
    (define-key map (kbd "f") #'piem-session-browser-toggle-named)
    (define-key map (kbd "/") #'piem-session-browser-search)
    (define-key map (kbd "t") #'piem-session-browser-toggle-scope)
    (define-key map (kbd "r") #'piem-session-browser-rename)
    (define-key map (kbd "RET") #'piem-session-browser-switch)
    (define-key map (kbd "?") #'piem-session-browser-dispatch)
    (define-key map (kbd "h") #'piem-session-browser-dispatch)
    map)
  "Keymap for the session browser.")

(defvar piem-session-section-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "RET") #'piem-session-browser-switch)
    map)
...
(defvar-local piem--session-browser-scope "current"
  "Scope for session listing: \"current\" or \"all\".")

(defvar-local piem--session-browser-sort "threaded"
  "Sort mode: \"threaded\", \"recent\", or \"relevance\".")

(defvar-local piem--session-browser-named-only nil
  "When non-nil, show only named sessions.")

(defvar-local piem--session-browser-items nil
  "Session items from the last `--browse-load-sessions' callback.")

(defvar-local piem--session-browser-search-query nil
  "Current search query string, or nil.")

(defvar-local piem--session-browser-search-tokens nil
  "Parsed search tokens from `piem--session-browser-search-query'.")

(defvar-local piem--session-browser-loading nil
  "Non-nil while a fetch is in progress.")

(defvar-local piem--session-browser-fetch-anchor nil
...
(defvar-local piem--session-browser-error nil
  "Error message string from last fetch, or nil on success.")

(defvar-local piem--session-browser-fetch-token 0
  "Generation counter for session-browser fetches.
`piem--browse-load-sessions' bumps it per fetch; callbacks
...
(defun piem--session-dispatch-heading ()
...
`piem--session-browser-header-line' formats for the
...
browser's state on the real rendering path."
  (mapconcat #'identity
             (append (list (format "scope:%s"
                                   piem--session-browser-scope)
                           (format "sort:%s"
                                   piem--session-browser-sort))
                     (and piem--session-browser-named-only
                          '("named-only")))
             " │ "))

(transient-define-prefix piem-session-browser-dispatch ()
  "Session browser help."
  [:description piem--session-dispatch-heading
   ["Actions"
    ("RET" "switch" piem-session-browser-switch)
    ("r" "rename" piem-session-browser-rename)
    ("g" "refresh" piem-browse-refresh)
    ("q" "quit" quit-window)]
   ["Filter & Sort"
    ("s" "sort" piem-session-browser-cycle-sort)
    ("f" "named only" piem-session-browser-toggle-named)
    ("t" "scope" piem-session-browser-toggle-scope)
    ("/" "search" piem-session-browser-search)]])

;;;; Faces

(defface piem-session-name
  '((t :weight bold))
  "Face for session names in the session browser."
  :group 'piem)

(defface piem-session-message-count
  '((t :inherit shadow))
  "Face for message counts in the session browser."
  :group 'piem)

(defface piem-session-age
  '((t :inherit shadow))
  "Face for relative age in the session browser margin."
  :group 'piem)

(defface piem-session-thread-connector
  '((t :inherit shadow))
  "Face for threading connectors (├─, └─) in the session browser."
  :group 'piem)

(defface piem-session-group-header
...
(define-derived-mode piem-browse-mode magit-section-mode
  "Pi-Browse"
  "Base mode for piem browse buffers.
Inherits section navigation from `magit-section-mode'."
  :group 'piem)

(define-derived-mode piem-session-browser-mode
  piem-browse-mode "Pi-Sessions"
  "Major mode for browsing pi sessions.
\\{piem-session-browser-mode-map}"
  :group 'piem
  (setq-local header-line-format
              '(:eval (piem--session-browser-header-line)))
  (setq piem--browse-margin-width
        piem--session-margin-width)
  (setq-local right-margin-width piem--session-margin-width)
  (add-hook 'window-configuration-change-hook
            #'piem--browse-apply-margins nil t))

;;;; Buffer Management

(defun piem--session-browser-buffer-name (dir)
  "Return session browser buffer name for DIR."
  (format "*piem-sessions:%s*"
          (piem--route-preserving-abbreviate-file-name dir)))

(defun piem--get-or-create-session-browser (dir)
  "Get or create session browser buffer for DIR."
  (let* ((name (piem--session-browser-buffer-name dir))
         (buf (get-buffer name)))
    (or buf
...
(defun piem--session-browser-render (buf)
  "Render the session browser in BUF from its buffer-local state."
  (with-current-buffer buf
    (let* ((inhibit-read-only t)
           (items (or piem--session-browser-items '()))
           ;; Apply filters
           (filtered (if piem--session-browser-named-only
                        (piem--session-filter-named items)
                      items))
           (filtered (piem--session-filter-search
                      filtered
                      piem--session-browser-search-tokens)))
      (magit-insert-section (root)
        (cond
         (piem--session-browser-loading
          (insert (piem--propertize-face
                   "Loading sessions..."
                   'piem-activity-phase)
                  "\n"))
         ...
         ((equal piem--session-browser-sort "threaded")
          (piem--session-browser-render-threaded filtered))
         ((equal piem--session-browser-sort "recent")
          (piem--session-browser-render-recent filtered))
         (t
          (let ((sorted (piem--session-sort-items
                         filtered piem--session-browser-sort)))
            (piem--session-browser-render-flat sorted))))))))

(defun piem--session-browser-render-flat (items)
  "Render ITEMS as a flat list."
  (dolist (item items)
    (piem--session-browser-insert-session item 0 nil)))

(defun piem--session-browser-render-threaded (items)
  "Render ITEMS in threaded view with connectors."
  (let ((threaded (piem--session-thread-items items)))
    (dolist (entry threaded)
      (let ((item (car entry))
            (depth (cdr entry)))
        (piem--session-browser-insert-session item depth t)))))

(defun piem--session-browser-render-recent (items)
  "Render ITEMS sorted by recency with time-group headers."
  (let ((sorted (piem--session-sort-items items "recent"))
        (last-group nil))
    (dolist (item sorted)
      (let ((group (piem--session-time-group
                    (plist-get item :modified))))
        (unless (equal group last-group)
          (magit-insert-section (time-group group)
            (magit-insert-heading
              (piem--propertize-face
               group 'piem-session-group-header)))
...
Message count and age are rendered as a right-margin overlay."
  (let* ((path (plist-get session :path))
         (name (piem--session-display-name session))
         ...
         (prefix (cond
                  ((and threaded (> depth 0))
                   (concat (make-string (* 2 (1- depth)) ?\s)
                           (piem--propertize-face
                            "└─ " 'piem-session-thread-connector)))
                  ((and is-fork (not threaded))
                   (piem--propertize-face
                    "fork: " 'piem-session-thread-connector))
                  (t "")))
         (heading (concat prefix
                          (piem--propertize-face
                           name 'piem-session-name)))
         (margin-str (concat
                      (piem--propertize-face
                       (format "%4d msgs " count)
                       'piem-session-message-count)
                      (piem--propertize-face
                       (or (piem--format-margin-age-from-iso modified)
                           (format (format "%%%ds"
                                           (+ 3 piem--margin-age-unit-width))
                                   "?"))
                       'piem-session-age))))
    (magit-insert-section (session path)
      (magit-insert-heading heading)
      (piem--make-margin-overlay margin-str))))

;;;; Header-Line

(defun piem--session-browser-header-line ()
  "Return header-line string for the session browser."
  (let* ((scope piem--session-browser-scope)
         (sort piem--session-browser-sort)
         (named piem--session-browser-named-only)
         (query piem--session-browser-search-query)
         (count (length (or piem--session-browser-items '()))))
    (mapconcat #'identity
               (append (list (format "Sessions [%s]" scope)
                       ...
                       (list (format "(%d)" count)
                             (piem--propertize-face "?:help" 'shadow)))
               " │ ")))

;;;; Session Browser Interactive Commands

(defun piem-session-browser-cycle-sort ()
  "Cycle the session browser sort mode."
  (interactive)
  (setq piem--session-browser-sort
        (piem--session-sort-next
         piem--session-browser-sort))
  (piem--session-browser-rerender)
  (message "Pi: Sort: %s" piem--session-browser-sort))

(defun piem-session-browser-toggle-named ()
  "Toggle named-only filter in the session browser."
  (interactive)
  (setq piem--session-browser-named-only
        (not piem--session-browser-named-only))
  (piem--session-browser-rerender)
  (message "Pi: Named-only: %s"
           (if piem--session-browser-named-only "on" "off")))

(defun piem-session-browser-toggle-scope ()
  "Toggle scope between current and all projects."
  (interactive)
  (setq piem--session-browser-scope
        (if (equal piem--session-browser-scope "current")
            "all" "current"))
  (piem--session-browser-fetch-and-render)
  (message "Pi: Scope: %s" piem--session-browser-scope))

(defun piem-session-browser-search ()
  "Set or clear search filter in the session browser."
  (interactive)
  (let ((query (read-string "Filter (regexp tokens): "
                            piem--session-browser-search-query))
        (need-rerender t))
    (if (string-empty-p query)
        (setq piem--session-browser-search-query nil
              piem--session-browser-search-tokens nil)
      ;; Validate regexp tokens
      (condition-case err
          (let ((tokens (split-string query)))
            (dolist (tok tokens)
              (string-match-p tok ""))
            (setq piem--session-browser-search-query query
                  piem--session-browser-search-tokens tokens))
        (invalid-regexp
...
(defun piem-session-browser-switch ()
  "Switch to the session at point."
  (interactive)
  (if-let* ((section (magit-current-section))
            (path (oref section value)))
      (piem--browse-switch-session path)
    (message "Pi: No session at point")))

(defun piem--browse-clean-session-name (name)
...
(defun piem--browse-last-entry-id-in-buffer ()
        ...
        (let* ((bol (line-beginning-position))
          ...
          (cond
           ((and (consp data) (equal (plist-get data :type) "session"))
            (throw 'done nil))
           ((consp data)
            (throw 'done (piem--normalize-string-or-null
                          (plist-get data :id))))
           ;; Blank or malformed line: step over it and retry.
           (t (goto-char bol))))))))

(defun piem--browse-session-file-state (path)
...
non-header line (see `piem--browse-last-entry-id-in-buffer').
      ...
      (when (file-readable-p path)
        (with-temp-buffer
          ...
          (let ((ids (make-hash-table :test #'equal)))
            ...
            (list :last-id (piem--browse-last-entry-id-in-buffer)
                  :ids ids
                  :newline-p (and (> (point-max) (point-min))
                                  (eq (char-before (point-max)) ?\n))))))
    (error nil)))

(defun piem--browse-fresh-entry-id (ids)
...
(defun piem--browse-append-session-entry (path type payload action)
...
`piem--browse-session-file-state' so :parentId is the id of
...
`piem--browse-fresh-entry-id').  When the file does not end
...
message) when PATH is unreadable."
  (if-let* ((state (piem--browse-session-file-state path)))
      (let ((line (json-encode
                   (append (list :type type
                                 :id (piem--browse-fresh-entry-id
                                      (plist-get state :ids))
...
(defun piem--browse-append-session-info (path name)
  "Append a session_info entry naming session file PATH to NAME, out-of-band.
Thin wrapper over `piem--browse-append-session-entry' with
...
appended."
  (piem--browse-append-session-entry
   path "session_info" (list :name name) "rename"))

(defun piem-session-browser-rename ()
...
`piem--browse-session-file-matches-p'):
  ...
  (if-let* ((section (magit-current-section))
      ...
      (let* ((item (cl-find path piem--session-browser-items
             ...
             (clean (piem--browse-clean-session-name input)))
        (if (string-empty-p clean)
            (message "Pi: Rename cancelled")
          (if (piem--browse-session-file-matches-p
               (piem--get-chat-buffer) path)
              (progn
                (piem-set-session-name clean)
                (piem--session-browser-fetch-and-render))
            (when (piem--browse-append-session-info path clean)
              (piem--session-browser-fetch-and-render)))))
    (message "Pi: No session at point")))

;;;; Point-Preserving Rerender

(defun piem--browse-capture-point-anchor ()
...
(defun piem--browse-rerender-preserving-point (buf render-fn
                                                            &optional fallback)
...
`piem--browse-capture-point-anchor') captured before an
...
point-at-top (same idiom as `piem--with-scroll-preservation'
in ui.el).  The sync covers both restore paths — the point-min
fallback included."
  (with-current-buffer buf
    (let* ((anchor (or (piem--browse-capture-point-anchor)
                       fallback))
           (ident (car anchor))
           (offset (cdr anchor)))
      (let ((inhibit-read-only t))
        ...
        (let ((new (and ident (magit-get-section ident))))
          ...
          ;; idiom as `piem--with-scroll-preservation').
...
(defun piem--session-browser-fetch-and-render ()
...
cycle's anchor (see `piem--session-browser-fetch-anchor')."
  (let* ((buf (current-buffer))
         (anchor (or (piem--browse-capture-point-anchor)
                     ...
                     (and piem--session-browser-loading
                          piem--session-browser-fetch-anchor))))
    (setq piem--session-browser-loading t
          piem--session-browser-fetch-anchor anchor)
    ...
    (piem--browse-load-sessions
     ...
     (lambda (items error)
       (when (buffer-live-p buf)
         ...
         (with-current-buffer buf
           (setq piem--session-browser-loading nil
                 piem--session-browser-fetch-anchor nil
                 piem--session-browser-error error
                 piem--session-browser-items items)
           (piem--session-browser-rerender anchor)))))))

(defun piem--session-browser-rerender (&optional fallback)
  "Re-render the session browser from local state, preserving point.
FALLBACK is a pre-fetch `(IDENT . OFFSET)' anchor handed to
`piem--browse-rerender-preserving-point' for the fetch
cycle's final render."
  (piem--browse-rerender-preserving-point
   (current-buffer) #'piem--session-browser-render fallback))

;;;; Tree Browser Section Classes and Keymaps

(defclass piem-tree-node-section (magit-section)
  ((keymap :initform 'piem-tree-node-section-map))
  "Section class for a tree node in the tree browser.")

(defun piem--register-section-types ()
...
that is fine for display-only sections."
  ...
  (setf (alist-get 'tree-node magit--section-type-alist)
        'piem-tree-node-section))
(piem--register-section-types)

(defvar piem-tree-browser-mode-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map piem-browse-mode-map)
    (define-key map (kbd "f") #'piem-tree-browser-cycle-filter)
    (define-key map (kbd "l") #'piem-tree-browser-set-label)
    (define-key map (kbd "/") #'piem-tree-browser-search)
    (define-key map (kbd "RET") #'piem-tree-browser-navigate)
    (define-key map (kbd "?") #'piem-tree-browser-dispatch)
    (define-key map (kbd "h") #'piem-tree-browser-dispatch)
    map)
  "Keymap for the tree browser.")

(defvar piem-tree-node-section-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "RET") #'piem-tree-browser-navigate)
    (define-key map (kbd "l") #'piem-tree-browser-set-label)
    map)
...
(defvar-local piem--tree-browser-filter "no-tools"
  "Filter mode: \"no-tools\", \"default\", \"user-only\", \"labeled-only\", \"all\".")

(defvar-local piem--tree-browser-tree nil
  "Projected tree from the last `--browse-load-tree' callback.
Vector of root nodes in the browse node dialect.")

(defvar-local piem--tree-browser-leaf-id nil
  "Current leaf node ID from the tree response.")

(defvar-local piem--tree-browser-visible-count 0
  "Count of visible entries from the last render.
Cached to avoid re-flattening the tree in the header-line.")

(defvar-local piem--tree-browser-search-query nil
  "Current search query string, or nil.")

(defvar-local piem--tree-browser-search-tokens nil
  "Parsed search tokens.")

(defvar-local piem--tree-browser-loading nil
  "Non-nil while a fetch is in progress.")

(defvar-local piem--tree-browser-fetch-anchor nil
  "Point anchor carried across the in-flight fetch cycle.
Same lifecycle as `piem--session-browser-fetch-anchor': set
...
(defvar-local piem--tree-browser-loaded-file nil
...
rendering path."
  (format "filter:%s" piem--tree-browser-filter))

(transient-define-prefix piem-tree-browser-dispatch ()
  "Tree browser help."
  [:description piem--tree-dispatch-heading
   ["Actions"
    ("RET" "navigate" piem-tree-browser-navigate)
    ("l" "label" piem-tree-browser-set-label)
    ("g" "refresh" piem-browse-refresh)
    ("q" "quit" quit-window)]
   ["Filter"
    ("f" "filter" piem-tree-browser-cycle-filter)
    ("/" "search" piem-tree-browser-search)]])

;;;; Tree Browser Faces

(defface piem-tree-user
  '((t :inherit font-lock-keyword-face))
  "Face for user messages in the tree browser."
  :group 'piem)

(defface piem-tree-assistant
  '((t :inherit font-lock-string-face))
  "Face for assistant messages in the tree browser."
  :group 'piem)

(defface piem-tree-tool
  '((t :inherit shadow))
  "Face for tool results in the tree browser."
  :group 'piem)

(defface piem-tree-compaction
  '((t :inherit shadow :slant italic))
  "Face for compaction entries in the tree browser."
  :group 'piem)

(defface piem-tree-summary
  '((t :inherit warning))
  "Face for branch summaries in the tree browser."
  :group 'piem)

(defface piem-tree-active
  '((t :weight bold))
  "Face for active-path marker in the tree browser."
  :group 'piem)

(defface piem-tree-label
  '((t :inherit success :weight bold))
  "Face for node labels in the tree browser."
  :group 'piem)

(defface piem-tree-connector
  '((t :inherit shadow))
...
(define-derived-mode piem-tree-browser-mode
  piem-browse-mode "Pi-Tree"
  "Major mode for browsing pi conversation tree.
\\{piem-tree-browser-mode-map}"
  :group 'piem
  (setq-local header-line-format
              '(:eval (piem--tree-browser-header-line)))
  (setq piem--browse-margin-width
        piem--tree-margin-width)
  (setq-local right-margin-width piem--tree-margin-width)
  (add-hook 'window-configuration-change-hook
            #'piem--browse-apply-margins nil t))

;;;; Tree Browser Buffer Management

(defun piem--tree-browser-buffer-name (dir)
  "Return tree browser buffer name for DIR."
  (format "*piem-tree:%s*"
          (piem--route-preserving-abbreviate-file-name dir)))

(defun piem--get-or-create-tree-browser (dir)
  "Get or create tree browser buffer for DIR."
  (let* ((name (piem--tree-browser-buffer-name dir))
         (buf (get-buffer name)))
    (or buf
        (with-current-buffer (generate-new-buffer name)
          (setq default-directory dir)
...
(defun piem--tree-node-face (node)
  ...
  (let ((type (plist-get node :type))
    ...
    (pcase type
      ("message"
       (pcase role
         ("user" 'piem-tree-user)
         ("assistant" 'piem-tree-assistant)
         ("branchSummary" 'piem-tree-summary)
         ("compactionSummary" 'piem-tree-compaction)
         (_ 'default)))
      ("tool_result" 'piem-tree-tool)
      ("compaction" 'piem-tree-compaction)
      ("branch_summary" 'piem-tree-summary)
      ("model_change" 'shadow)
      ("thinking_level_change" 'shadow)
      (_ 'default))))

(defun piem--tree-node-type-label (node)
...
(defun piem--tree-strip-bracket-preview (node)
...
(defun piem--tree-node-preview (node)
  ...
  (let ((type (plist-get node :type)))
    (pcase type
      ...
      ("tool_result"
       (piem--tree-strip-bracket-preview node))
      ("message"
...
(defun piem--tree-format-node-line (node is-active)
...
Labels are rendered separately as right-margin overlays."
  (let* ((face (piem--tree-node-face node))
         (type-label (piem--tree-node-type-label node))
         (preview (piem--tree-node-preview node))
         (marker (if is-active
                     (piem--propertize-face
                      "• " 'piem-tree-active)
                   "  "))
         (type-str (piem--propertize-face
                    (format "%-7s" type-label) face))
         (preview-str (piem--propertize-face preview face)))
    (concat marker type-str " " preview-str)))

;;;; Tree Browser Rendering

(defun piem--tree-browser-render (buf)
  "Render the tree browser in BUF from its buffer-local state."
  (with-current-buffer buf
    (let* ((inhibit-read-only t)
           (tree piem--tree-browser-tree)
           (leaf-id piem--tree-browser-leaf-id)
           (filter piem--tree-browser-filter))
      (magit-insert-section (root)
        (cond
         (piem--tree-browser-loading
          (setq piem--tree-browser-visible-count 0)
          (insert (piem--propertize-face
                   "Loading tree..."
                   'piem-activity-phase)
                  "\n"))
         ...
         (t
          (let* ((flat (piem--flatten-tree-for-display
                        tree leaf-id filter))
                 (active-ids (piem--active-path-ids tree leaf-id))
                 ;; Apply search filter if active
                 (visible (if piem--tree-browser-search-tokens
                              (cl-remove-if-not
                               (lambda (entry)
                                 (piem--matches-filter-p
                                  (piem--tree-node-preview
                                   (nth 0 entry))
                                  piem--tree-browser-search-tokens))
                               flat)
                            flat)))
            (setq piem--tree-browser-visible-count
                  (length visible))
            (if (null visible)
              ...
              (dolist (entry visible)
                (let* ((node (nth 0 entry))
                       ...
                       (prefix-str (piem--propertize-face
                                    prefix
                                    'piem-tree-connector))
                       (line (piem--tree-format-node-line
                              node is-active)))
                  (magit-insert-section (tree-node node-id)
                    ...
                    (when-let* ((label (plist-get node :label)))
                      ;; 3 = "[" + "]" + 1 char padding
                      (let ((truncated
                             (piem--truncate-string
                              label
                              (- piem--tree-margin-width 3))))
                        (piem--make-margin-overlay
                         (piem--propertize-face
                          (format "[%s]" truncated)
                          'piem-tree-label)))))))))))))))
...
(defun piem--tree-browser-header-line ()
...
tree flattening on every redisplay cycle."
  (let* ((filter piem--tree-browser-filter)
         (query piem--tree-browser-search-query)
         (total piem--tree-browser-visible-count))
    (mapconcat #'identity
               (append (list (format "Tree [%s]" filter)
                             (format "(%d)" total))
                       (and query (list (format "/%s" query)))
                       (list (piem--propertize-face "?:help" 'shadow)))
               " │ ")))

;;;; Tree Browser Interactive Commands

(defun piem-tree-browser-cycle-filter ()
  "Cycle the tree browser filter mode."
  (interactive)
  (let* ((modes piem--tree-filter-modes)
         (current piem--tree-browser-filter)
         (next (or (cadr (member current modes)) (car modes))))
    (setq piem--tree-browser-filter next)
    (piem--tree-browser-rerender)
    (message "Pi: Filter: %s" next)))

(defun piem-tree-browser-search ()
  "Set or clear search filter in the tree browser."
  (interactive)
  (let ((query (read-string "Filter (regexp tokens): "
                            piem--tree-browser-search-query))
        (need-rerender t))
    (if (string-empty-p query)
        (setq piem--tree-browser-search-query nil
              piem--tree-browser-search-tokens nil)
      (condition-case err
          (let ((tokens (split-string query)))
            (dolist (tok tokens)
              (string-match-p tok ""))
            (setq piem--tree-browser-search-query query
                  piem--tree-browser-search-tokens tokens))
        (invalid-regexp
         (message "Pi: Invalid regexp: %s" (error-message-string err))
         (setq need-rerender nil))))
    (when need-rerender
...
(defun piem-tree-browser-navigate ()
  "Navigate to the tree node at point."
  (interactive)
  (if-let* ((section (magit-current-section))
            (node-id (oref section value)))
      (piem--browse-navigate node-id)
    (message "Pi: No tree node at point")))

(defun piem-tree-browser-set-label ()
  "Set or clear a label on the tree node at point."
  (interactive)
  (when-let* ((section (magit-current-section))
              (node-id (oref section value)))
    (let* ((current-label (when piem--tree-browser-tree
                            (piem--tree-find-label
                             piem--tree-browser-tree node-id)))
      ...
      (piem--browse-set-label node-id label))))

(defun piem--tree-find-label (tree node-id)
...
(defun piem--tree-node-with-label (node label)
...
is `piem--tree-apply-label's business.  A newly set :label
...
(defun piem--tree-apply-label (tree node-id label)
...
\\(`piem--tree-node-with-label'\\) `aset' into each container,
    ...
    (let* ((root-frame (cons tree 0))
      ...
      (when found
        (let* ((target-frame (car path))
               ...
               (patched (piem--tree-node-with-label target label))
               (result nil)
               (frames path))
          (while frames
...
(defun piem--browse-current-session-directory ()
...
all.  Signals when resolution itself fails."
  (or (and (fboundp 'piem--session-list-directory)
           (piem--session-list-directory))
      (when-let* ((chat-buf piem--chat-buffer))
        (and (buffer-live-p chat-buf)
             (let ((cwd (piem--chat-session-directory chat-buf)))
               (piem-jsonl-session-dir-for-cwd
                cwd (piem-jsonl-sessions-root cwd)))))
      (piem-jsonl-session-dir-for-cwd
       (piem--session-directory))))

(defun piem--browse-session-directories (scope)
  "Return the list of session directories to scan for SCOPE.
\"current\" is the single current-project directory (see
`piem--browse-current-session-directory').  \"all\" is
...
Missing roots read as empty; signals when resolution itself fails."
  (if (not (equal scope "all"))
      (list (piem--browse-current-session-directory))
    (let* ((cur (piem--browse-current-session-directory))
           (root (if cur
                     (piem-jsonl-sessions-root
...
(defun piem--browse-session-files (dirs)
...
(defun piem--browse-scan-session-files (buf token files items callback)
...
mapping over `piem-jsonl-read-session-info' output).
...
in `piem--browse-load-tree')."
  (if (and (buffer-live-p buf)
           (eq token (buffer-local-value
                      'piem--session-browser-fetch-token buf)))
        ...
        (condition-case err
            (progn
              (while (and files (< (float-time) deadline))
                (let ((info (piem-jsonl-read-session-info
                             (car files))))
                  (when info (push info items)))
                (setq files (cdr files)))
              (if files
                  (run-at-time 0 nil #'piem--browse-scan-session-files
...
(defun piem--browse-load-sessions (scope callback)
...
sessions: …\"."
  (let ((buf (current-buffer)))
    (setq piem--session-browser-fetch-token
          (1+ piem--session-browser-fetch-token))
    (let ((token piem--session-browser-fetch-token))
      (let ((dirs nil)
            (failure nil))
        (condition-case err
            (setq dirs (piem--browse-session-directories scope))
          (error
        ...
        (if failure
          ...
          (run-at-time 0 nil #'piem--browse-scan-session-files
                       buf token
                       (piem--browse-session-files dirs)
                       nil callback))))))

(defun piem--tree-browser-chat-session-file ()
  "Return the linked chat buffer's current session file, or nil.
A live `piem--chat-buffer' link supplies the normalized
  ...
  (when-let* ((chat-buf piem--chat-buffer))
    (when (buffer-live-p chat-buf)
      (with-current-buffer chat-buf
        (when (plistp piem--state)
          (piem--normalize-string-or-null
           (plist-get piem--state :session-file)))))))

(defun piem--browse-load-tree (callback)
...
`piem-jsonl-project-session-file' — never an RPC get_tree.
...
manually with \\[piem-browse-refresh].

The buffer's fetch token is bumped here (mirroring
`piem--browse-load-sessions'); then a missing chat link
...
loading state first (see `piem--tree-browser-fetch-and-render':
...
never run and the browser would sit on its loading state forever."
  (let ((buf (current-buffer)))
    (setq piem--tree-browser-fetch-token
          (1+ piem--tree-browser-fetch-token))
    (let ((token piem--tree-browser-fetch-token)
          (linked (and piem--chat-buffer
                       (buffer-live-p piem--chat-buffer)))
          (path (piem--tree-browser-chat-session-file)))
      (cond
       ((not linked)
        ...
        (run-at-time
         ...
         (lambda ()
           (when (and (buffer-live-p buf)
                      (eq token (buffer-local-value
                                 'piem--tree-browser-fetch-token
                                 buf)))
             (let ((tree nil)
                   (leaf-id nil)
                   (failure nil))
               (condition-case nil
                   (let ((result (piem-jsonl-project-session-file
                                  path)))
                     (if result
...
(defun piem--browse-session-file-matches-p (chat-buf path)
...
when it returns nil)."
  (if (piem--session-transition-active-p chat-buf)
      (progn
        (message "Pi: Cannot %s while switching sessions" action)
        t)
    (not (piem--session-transition-ready-p chat-buf action))))

(defun piem--browse-switch-session (path)
...
`piem--browse-transition-refused-p' gates both an active
...
`piem--resume-selected-session' (PROC CHAT-BUF PATH); its
synchronous `user-error's (bad cwd, duplicate open) surface in the
browser.  Afterwards `piem--browse-quit-when-settled' waits
...
messaged).  No extra refresh logic: \\[piem-browse-refresh]
...
browser per project directory."
  (let ((chat-buf piem--chat-buffer))
    ...
    (let ((proc (buffer-local-value 'piem--process chat-buf)))
      (unless (piem--session-live-process-p proc)
        (user-error "Pi process is not running"))
      (unless (piem--browse-transition-refused-p chat-buf "switch")
        (piem--resume-selected-session proc chat-buf path)
        (piem--browse-quit-when-settled
         chat-buf (selected-window) path)))))

(defun piem--browse-poll-settled (chat-buf win path deadline)
  "Polling body of `piem--browse-quit-when-settled'.
  ...
  (if (not (buffer-live-p chat-buf))
    ...
    (if (piem--session-transition-active-p chat-buf)
        (when (time-less-p (current-time) deadline)
          (run-at-time 0.05 nil #'piem--browse-poll-settled
                       chat-buf win path deadline))
      (when (and (piem--browse-session-file-matches-p chat-buf path)
                 (window-live-p win)
                 (with-current-buffer (window-buffer win)
                   (derived-mode-p 'piem-session-browser-mode
                                   'piem-tree-browser-mode)))
        (quit-window nil win)))))

(defun piem--browse-quit-when-settled (chat-buf win path)
  "Wait out CHAT-BUF's session transition, then dismiss the browser window.
Polls `piem--session-transition-active-p' every 0.05 s with
...
replaced it."
  (piem--browse-poll-settled
   chat-buf win path (time-add (current-time) 30)))

(defun piem--browse-navigate (node-id)
...
navigateTree without a navigate RPC:
 ...
 4. no in-flight session transition (the first half of
    `piem--browse-transition-refused-p') —
...
12. `piem--resume-selected-session' (PROC CHAT-BUF PATH) —
  ...
  (let ((chat-buf piem--chat-buffer))
    ...
    (let ((input-buf (buffer-local-value 'piem--input-buffer
      ...
      (cond
       ...
       ((not (equal piem--tree-browser-loaded-file path))
       ...
       (t
        (let ((proc (buffer-local-value 'piem--process chat-buf)))
          (unless (piem--session-live-process-p proc)
            (user-error "Pi process is not running"))
          (cond
           ((piem--browse-transition-refused-p chat-buf "navigate")
            nil)
           (t
            (let ((session (piem-jsonl-read-file path)))
              (cond
               ...
               (t
                (let ((target (piem-jsonl-navigation-target
                               session node-id)))
                  (cond
                   ((null target)
                   ...
                   ((plist-get target :current-p)
                    (if (not (plist-get target :prefill))
                      ...
                      (piem--browse-prefill-input
                       input-buf (plist-get target :prefill))
                      (piem--browse-navigate-message node-id)
                      (piem--browse-quit-when-settled
                       chat-buf (selected-window) path)))
                   (t
                    (condition-case err
                        (piem--session-file-cwd-or-error path)
                      ;; Re-signal the guard's own wording before any
                      ;; write happens.
                      (user-error (signal (car err) (cdr err))))
                    (let ((lines (piem-jsonl-navigation-lines
                                  path (plist-get target :leaf-id))))
                      (if (null lines)
                          (message
                           "Pi: Cannot navigate: session file is unreadable or not a pi session file: %s"
                           path)
                        (when (piem--browse-rewrite-session-file
                               path lines)
...
(defun piem--browse-navigate-message (node-id)
  "Message the navigate success for NODE-ID from the cached tree.
The preview comes from `piem--tree-find-node' and
`piem--tree-node-preview' over the browser's cached tree
with no refetch, truncated to 60; without a preview the bare
\"Pi: Navigated\"."
  (let* ((node (when (vectorp piem--tree-browser-tree)
                 (piem--tree-find-node
                  piem--tree-browser-tree node-id)))
         (preview (if node (piem--tree-node-preview node) "")))
    (if (and (stringp preview) (not (string-empty-p preview)))
        (message "Pi: Navigated to %s"
                 (piem--truncate-string preview 60))
      (message "Pi: Navigated"))))

(defun piem--browse-rewrite-session-file (path lines)
  "Atomically replace the session file at PATH with LINES.
LINES is the `piem-jsonl-navigation-lines' vector; the
...
(defun piem--browse-prefill-input (input-buf text)
...
\\(`piem--tree-apply-label'\\) and re-rendered: section
...
disagrees with `piem--tree-browser-loaded-file' (the chat
...
longer reflects."
  (let ((path (piem--tree-browser-chat-session-file)))
    (cond
     ((not path)
     ...
     ((not (equal piem--tree-browser-loaded-file path))
      (message "Pi: Session changed since the tree was loaded — refresh with g"))
     (t
      (when (piem--browse-append-session-entry
             path "label"
             (append (list :targetId node-id)
                     (when label (list :label label)))
             "label")
        (setq piem--tree-browser-tree
              (or (piem--tree-apply-label
                   piem--tree-browser-tree node-id label)
                  piem--tree-browser-tree))
        (piem--tree-browser-rerender)
        (if label
...
(defun piem--tree-browser-fetch-and-render ()
...
visible count.  On success, `piem--tree-browser-loaded-file'
...
across the cycle the final render would find nothing under point to
restore (see `piem--browse-rerender-preserving-point').
...
`piem--tree-browser-fetch-anchor').
...
to redisplay between its slices; one timer hop never does)."
  (let* ((buf (current-buffer))
         (loaded (piem--tree-browser-chat-session-file))
         (anchor (or (piem--browse-capture-point-anchor)
                     ;; Mid-flight refresh: the loading render already
                     ;; destroyed the sections under point, so carry
                     ;; the anchor the in-flight cycle captured.
                     (and piem--tree-browser-loading
                          piem--tree-browser-fetch-anchor))))
    (setq piem--tree-browser-loading t
          piem--tree-browser-fetch-anchor anchor)
    ;; Loading-state render: default point behavior (nothing to keep).
    (piem--tree-browser-rerender)
    ...
    (piem--browse-load-tree
     (lambda (tree leaf-id message)
       (when (buffer-live-p buf)
         ...
         ;; current; render in the browser buffer, not that one.
         (with-current-buffer buf
           (setq piem--tree-browser-loading nil
                 piem--tree-browser-fetch-anchor nil
                 piem--tree-browser-error message
                 piem--tree-browser-tree tree
                 piem--tree-browser-leaf-id leaf-id
                 piem--tree-browser-loaded-file
                 (and (not message) loaded))
           (piem--tree-browser-rerender anchor)))))))

(defun piem--tree-browser-rerender (&optional fallback)
  "Re-render the tree browser from local state, preserving point.
FALLBACK is a pre-fetch `(IDENT . OFFSET)' anchor handed to
`piem--browse-rerender-preserving-point' for the fetch
cycle's final render."
  (piem--browse-rerender-preserving-point
   (current-buffer) #'piem--tree-browser-render fallback))

;;;; Tree Browser Refresh Integration

(defun piem-browse-refresh ()
  "Refresh the current browse buffer from disk."
  (interactive)
  (cond
   ((derived-mode-p 'piem-session-browser-mode)
    (piem--session-browser-fetch-and-render))
   ((derived-mode-p 'piem-tree-browser-mode)
...
error forever)."
  ...
  (let* ((dir (piem--session-directory))
         (new-p (not (get-buffer
                      (piem--tree-browser-buffer-name dir))))
         (buf (piem--get-or-create-tree-browser dir)))
    ;; Link to the chat session.  Only the chat buffer is cached here;
