;;; piem-render-test.el --- Tests for piem-render -*- lexical-binding: t; -*-
...
(ert-deftest piem-test-append-to-chat-inserts-text ()
  "piem--append-to-chat inserts text at end of chat buffer."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--append-to-chat "Hello")
    (should (equal (buffer-string) "Hello"))))

(ert-deftest piem-test-append-to-chat-appends ()
  "piem--append-to-chat appends to existing content."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((inhibit-read-only t))
      (insert "First"))
    (piem--append-to-chat " Second")
    (should (equal (buffer-string) "First Second"))))

(ert-deftest piem-test-display-agent-start-inserts-separator ()
...
(ert-deftest piem-test-display-message-delta-appends-text ()
  "message_update text_delta appends text to chat."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)  ; Creates streaming marker
    (piem--display-message-delta "Hello, ")
    (piem--display-message-delta "world!")
    (should (string-match-p "Hello, world!" (buffer-string)))))

(ert-deftest piem-test-display-message-delta-skips-table-scan-without-pipe ()
  "Streaming text without pipes should not query for markdown tables."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (let ((table-scan-count 0))
      (cl-letf (((symbol-function 'piem--maybe-decorate-streaming-table)
                 (lambda () (setq table-scan-count (1+ table-scan-count)))))
        (piem--display-message-delta "ordinary prose\nmore prose\n"))
      (should (= table-scan-count 0)))))

(ert-deftest piem-test-display-message-delta-scans-table-after-pipe-newline ()
  ...
  (with-temp-buffer
    ...
    (let ((table-scan-count 0))
      (cl-letf (((symbol-function 'piem--maybe-decorate-streaming-table)
                 (lambda () (setq table-scan-count (1+ table-scan-count)))))
        (piem--display-message-delta "| a | b |")
        (should (= table-scan-count 0))
        (piem--display-message-delta "\n|---|---|\n"))
      (should (= table-scan-count 1)))))

(ert-deftest piem-test-text-end-clears-streaming-table-candidate ()
  ...
  (with-temp-buffer
    ...
    (let ((table-scan-count 0))
      ...
      (should (= table-scan-count 1))
      (should-not piem--streaming-table-candidate))))

(ert-deftest piem-test-delta-transforms-atx-headings ()
...
as the top-level structure."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (piem--display-message-delta "# Heading 1\n## Heading 2")
...
(ert-deftest piem-test-spacing-blank-line-after-assistant-header ()
  "Assistant header has a blank line after setext underline."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (piem--display-message-delta "Hi")
    ;; Pattern: setext heading (Assistant + underline), blank line, content
    (should (string-match-p "Assistant\n=+\n\nHi" (buffer-string)))))

(ert-deftest piem-test-spacing-delta-leading-newlines-stripped ()
...
(ert-deftest piem-test-spacing-thinking-leading-newlines-stripped ()
...
(ert-deftest piem-test-thinking-empty-lifecycle-no-visible-blockquote ()
  "Empty thinking start/end should not leave a visible blank blockquote."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (piem--display-thinking-start)
    (piem--display-thinking-end "")
    (goto-char (point-min))
    (should-not (re-search-forward "^>\\s-*$" nil t))))

(ert-deftest piem-test-thinking-leading-trailing-newlines-normalized ()
...
(ert-deftest piem-test-hidden-thinking-shows-live-then-collapses-to-preview-line ()
...
(ert-deftest piem-test-hidden-thinking-falls-back-when-first-line-is-too-long ()
...
(ert-deftest piem-test-hidden-thinking-falls-back-when-first-line-is-too-short ()
  ...
  (let ((piem-thinking-display 'hidden))
    (with-temp-buffer
      ...
      (piem--display-agent-start)
      (piem--display-thinking-start)
      (piem--display-thinking-delta "ok\nSecond line")
      (piem--display-thinking-end "")
      (should (string-match-p
               (regexp-quote "> Thinking hidden… (2 lines)")
               (buffer-string))))))

(ert-deftest piem-test-hidden-thinking-preview-can-be-disabled ()
  "Collapsed thinking can always use the generic hidden label when configured."
  (let ((piem-thinking-display 'hidden)
        (piem-thinking-hidden-preview nil))
    (with-temp-buffer
      (piem-chat-mode)
      (piem--display-agent-start)
      (piem--display-thinking-start)
...
(ert-deftest piem-test-visible-thinking-stays-expanded-on-end ()
  ...
  (let ((piem-thinking-display 'visible))
    (with-temp-buffer
      ...
      (piem--display-agent-start)
      (piem--display-thinking-start)
      (piem--display-thinking-delta "Need to double-check.")
      (piem--display-thinking-end "")
      (let ((text (buffer-string)))
        (should (string-match-p "> Need to double-check\\." text))
        (should-not (string-match-p
                     (regexp-quote
                      (piem-test--collapsed-thinking-stub
                       "Need to double-check."))
                     text))))))

(ert-deftest piem-test-live-visible-thinking-end-stamps-toggle-metadata ()
...
(ert-deftest piem-test-hidden-thinking-collapse-keeps-blank-line-separators ()
  "Collapsing completed thinking should keep stable blank-line separation."
  (let ((piem-thinking-display 'hidden))
    (with-temp-buffer
      (piem-chat-mode)
      (piem--display-agent-start)
      (piem--display-message-delta "Answer first.")
      (piem--display-thinking-start)
      (piem--display-thinking-delta "Need to double-check.")
      (piem--display-thinking-end "")
      (piem--display-message-delta "Final answer.")
      (let* ((text (buffer-string))
             (stub (piem-test--collapsed-thinking-stub
                    "Need to double-check."))
...
(ert-deftest piem-test-toggle-thinking-display-during-streaming-defers-rebuild ()
  ...
  (let ((piem-thinking-display 'visible))
    (with-temp-buffer
      ...
      (piem--display-agent-start)
      (piem--display-thinking-start)
      (piem--display-thinking-delta "Live reasoning")
      (let ((before (buffer-string))
            (marker piem--thinking-marker))
        (cl-letf (((symbol-function 'message) #'ignore))
          (piem-toggle-thinking-display))
        (should (eq piem--thinking-display 'hidden))
        (should (equal before (buffer-string)))
        (should (eq marker piem--thinking-marker))
        (should (marker-buffer piem--thinking-marker)))
      (piem--display-thinking-end "")
      (let ((text (buffer-string)))
        (should (string-match-p
                 (regexp-quote
...
(ert-deftest piem-test-thinking-normalization-preserves-first-line-indentation ()
...
(ert-deftest piem-test-thinking-whitespace-only-delta-does-not-rewrite-buffer ()
  "Adding ignorable trailing whitespace should not rewrite rendered thinking."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (piem--display-thinking-start)
    (piem--display-thinking-delta "Stable")
    (let ((before (buffer-string))
...
(ert-deftest piem-test-thinking-incremental-appends-suffix-only ()
...
because it inserts at the end, while a full rewrite would lose it."
  (with-temp-buffer
    ...
    (piem--display-agent-start)
    (piem--display-thinking-start)
    (piem--display-thinking-delta "First thought.")
    (should piem--thinking-prev-rendered)
    ;; Place a marker property inside the rendered thinking region
    (let ((think-start (marker-position piem--thinking-start-marker))
          (inhibit-read-only t))
      ...
      (piem--display-thinking-delta " Second thought.")
...
(ert-deftest piem-test-thinking-paragraph-spacing-no-runaway-blank-lines ()
...
(ert-deftest piem-test-thinking-interleaved-with-tool-has-stable-spacing ()
  ...
  (let ((piem-thinking-display 'visible))
    (with-temp-buffer
      ...
      (piem-test--send-assistant-message-update
...
(ert-deftest piem-test-thinking-after-text-has-blank-line-separator ()
  ...
  (let ((piem-thinking-display 'visible))
    (with-temp-buffer
      ...
      (piem--display-thinking-end "")
      ;; Text between blocks
      (piem--display-message-delta "Here is my answer.")
...
(ert-deftest piem-test-thinking-delta-allows-syntax-propertize ()
...
subsequent font-lock-ensure calls."
  (with-temp-buffer
    ...
    (piem--display-thinking-delta "First paragraph with text.")
    ;; Fontify
...
(ert-deftest piem-test-spacing-blank-line-before-tool ()
  "Tool block is preceded by blank line when after text."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (piem--display-message-delta "Let me check.")
...
(ert-deftest piem-test-spacing-blank-line-after-tool ()
...
(ert-deftest piem-test-spacing-single-blank-line-between-turns ()
...
agent_end + next section's leading newline must not create triple newlines."
  (with-temp-buffer
    ...
    (piem--display-user-message "Bye")
    ;; Should never have triple newlines (which would be two blank lines)
    (should-not (string-match-p "\n\n\n" (buffer-string)))))

(ert-deftest piem-test-spacing-single-blank-line-before-compaction ()
  "Only one blank line between agent response and compaction header."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (piem--display-message-delta "Some response.")
    (piem--render-complete-message)
    (piem--display-agent-end)
    (piem--display-compaction-result 50000 "Summary.")
    (should-not (string-match-p "\n\n\n" (buffer-string)))))

(ert-deftest piem-test-spacing-no-double-blank-between-tools ()
  ...
  (with-temp-buffer
    ...
    (piem--display-tool-end "bash" '(:command "ls")
                          ...
                          nil nil)
    (piem--display-tool-start "read" '(:path "file.txt"))
...
(ert-deftest piem-test-history-renders-user-string-content ()
...
(ert-deftest piem-test-history-renders-assistant-string-content ()
...
(ert-deftest piem-test-history-renders-branch-summary-between-assistant-groups ()
  ...
  (with-temp-buffer
    ...
    (let* ((timestamp-ms 1704067201000)
      ...
      (let* ((text (buffer-substring-no-properties (point-min) (point-max)))
        ...
        (should (= 2 (piem-test--count-matches
                      "^Assistant\n=+\n" text)))))))

(ert-deftest piem-test-history-replays-assistant-thinking-after-text ()
...
(ert-deftest piem-test-history-replays-thinking-like-live-rendering ()
    ...
    (should
     (equal
      (with-temp-buffer
        ...
        (piem--display-agent-start)
        (piem--display-thinking-start)
        (piem--display-thinking-delta raw-thinking)
        (piem--display-thinking-end "")
        (buffer-string))
      (with-temp-buffer
        (piem-chat-mode)
...
(ert-deftest piem-test-history-hides-completed-thinking-when-display-hidden ()
...
(ert-deftest piem-test-display-session-history-batches-postprocessing ()
  ...
  (with-temp-buffer
    ...
    (let ((font-lock-count 0)
      ...
      (cl-letf (((symbol-function 'font-lock-ensure)
                ...
                ((symbol-function 'piem--decorate-tables-in-region)
...
(ert-deftest piem-test-branch-summary-table-uses-deferred-history-postprocessing ()
  "Branch-summary tables reach one consolidated, non-eager history pass."
  (with-temp-buffer
    (piem-chat-mode)
    (let (decoration-calls)
      (cl-letf (((symbol-function 'piem--decorate-tables-in-region)
...
(ert-deftest piem-test-display-session-history-postprocesses-hot-tail-only ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-hot-tail-turn-count 2)
      ...
      (let ((start (marker-position piem--hot-tail-start))
            (end (point-max)))
...
(ert-deftest piem-test-display-session-history-skips-table-postprocess-without-table-candidate ()
...
(ert-deftest piem-test-display-session-history-renders-custom-messages ()
...
(defun piem-test--history-with-toggleable-thinking ()
...
(defun piem-test--tail-screen-lines (window)
...
(defun piem-test--setup-long-live-thinking (buffer display)
  "Populate BUFFER with history and a long live thinking block using DISPLAY."
  (with-current-buffer buffer
    (piem-chat-mode)
    (setq piem--thinking-display display)
    (let ((inhibit-read-only t))
      (dotimes (i 80)
    ...
    (piem--display-thinking-start)
    (piem--display-thinking-delta
     (piem-test--long-thinking-text))))

(defmacro piem-test--with-long-live-thinking-buffer (spec &rest body)
...
and DISPLAY controls how completed thinking is rendered."
  ...
  (let ((buffer (car spec))
    ...
    `(let ((,buffer (generate-new-buffer " *pi-long-live-thinking*")))
       (unwind-protect
           (progn
             (piem-test--setup-long-live-thinking ,buffer ,display)
             ,@body)
         (when (buffer-live-p ,buffer)
           (kill-buffer ,buffer))))))

(ert-deftest piem-test-tab-expands-completed-thinking-stub ()
  "TAB on a hidden completed-thinking stub expands that block."
  (let ((piem-thinking-display 'hidden))
    (with-temp-buffer
      (piem-chat-mode)
      (piem--display-session-history
       (piem-test--history-with-toggleable-thinking)
       (current-buffer))
      (goto-char (point-min))
      (search-forward (piem-test--collapsed-thinking-stub
                       "Need to double-check.\n\nSecond paragraph."))
      (beginning-of-line)
      (piem-toggle-tool-section)
      (let ((text (buffer-string)))
...
(ert-deftest piem-test-tab-collapses-expanded-completed-thinking-block ()
...
(ert-deftest piem-test-thinking-toggle-wins-before-tool-or-outline ()
...
(ert-deftest piem-test-rerender-clears-temporary-thinking-expansion ()
  ...
  (let ((piem-thinking-display 'hidden))
    (with-temp-buffer
      ...
      (piem--rerender-canonical-history)
      (let ((text (buffer-string)))
        (should (string-match-p
                 (regexp-quote
                  (piem-test--collapsed-thinking-stub
...
(ert-deftest piem-test-thinking-toggle-preserves-window-start-before-block ()
...
(ert-deftest piem-test-hidden-thinking-end-keeps-tail-window-filled ()
  "Collapsing long live thinking keeps a tail-following window filled."
  (let ((piem-thinking-display 'hidden))
    (piem-test--with-long-live-thinking-buffer (buf 'hidden)
      (let ((win (display-buffer buf)))
        (with-selected-window win
          (goto-char (point-max))
          (recenter -1)
          (should (piem-test--window-mostly-filled-p win))
          (piem--display-thinking-end "")
          (should (piem-test--window-shows-tail-p win))
          (should (piem-test--window-mostly-filled-p win)))))))

(ert-deftest piem-test-thinking-tab-collapse-keeps-tail-window-filled ()
  "Collapsing completed thinking with TAB keeps a tail view filled."
  (let ((piem-thinking-display 'visible))
    (piem-test--with-long-live-thinking-buffer (buf 'visible)
      (with-current-buffer buf
        (piem--display-thinking-end ""))
      (let ((win (display-buffer buf)))
        (with-selected-window win
...
(ert-deftest piem-test-tab-collapse-preserves-window-after-thinking-block ()
      ...
      (save-window-excursion
        ...
        (let* ((reader (display-buffer buf))
          ...
          (let ((start-line-before
                 (piem-test--window-start-line reader)))
            (with-selected-window toggle
              (goto-char (point-min))
              (search-forward "thinking line 120")
              (piem-toggle-tool-section))
            (should (equal (piem-test--window-start-line reader)
                           start-line-before))))))))

(ert-deftest piem-test-live-thinking-end-keeps-inspected-block-visible ()
...
(ert-deftest piem-test-chat-thinking-display-preserves-window-after-earlier-block ()
  ...
  (let ((piem-thinking-display 'visible))
    (piem-test--with-long-live-thinking-buffer (buf 'visible)
      ...
      (let ((win (display-buffer buf)))
        ...
        (let ((start-line-before
               (piem-test--window-start-line win)))
          (with-current-buffer buf
            (cl-letf (((symbol-function 'message) #'ignore))
              (piem--set-chat-thinking-display 'hidden)))
          (should (equal (piem-test--window-start-line win)
                         start-line-before)))))))

(ert-deftest piem-test-chat-thinking-display-noop-preserves-window-start ()
...
(ert-deftest piem-test-tab-falls-back-to-outline-when-not-on-section ()
...
(ert-deftest piem-test-history-preserves-assistant-block-order ()
...
(ert-deftest piem-test-history-renders-tool-with-output ()
  ...
  (with-temp-buffer
    ...
    (should (cl-some (lambda (ov) (overlay-get ov 'piem-tool-block))
                     (overlays-in (point-min) (point-max))))))

(ert-deftest piem-test-display-session-history-does-not-accumulate-stale-tool-overlays ()
...
(ert-deftest piem-test-display-session-history-clears-stale-live-tool-state ()
  ...
  (with-temp-buffer
    ...
    (should (= 2 (hash-table-count piem--live-tool-blocks)))
    (should (= 2 (hash-table-count piem--tool-args-cache)))
    (piem--display-session-history
     [(:role "assistant"
    ...
    (should-not (cl-some (lambda (ov) (overlay-get ov 'piem-tool-block))
                         (overlays-in (point-min) (point-max))))))

(ert-deftest piem-test-display-session-history-decorates-user-table ()
  ...
  (with-temp-buffer
    ...
    (let ((messages [(:role "user"
      ...
      (should (> (length (seq-filter
                          (lambda (ov) (overlay-get ov 'piem-table-display))
                          (overlays-in (point-min) (point-max))))
                 0)))))

(ert-deftest piem-test-history-renders-multiple-tools-in-order ()
...
(ert-deftest piem-test-history-renders-tools-across-assistant-messages ()
...
(ert-deftest piem-test-history-renders-tool-error ()
...
(ert-deftest piem-test-history-renders-tool-without-result ()
...
(ert-deftest piem-test-history-displays-compaction-summary ()
...
(ert-deftest piem-test-history-tolerates-malformed-content-blocks ()
...
(ert-deftest piem-test-streaming-marker-created-on-agent-start ()
  "Streaming marker is created on agent_start."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (should (markerp piem--streaming-marker))
    (should (= (marker-position piem--streaming-marker) (point-max)))))

(ert-deftest piem-test-streaming-marker-advances-with-delta ()
  "Streaming marker advances as deltas are inserted."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (let ((initial-pos (marker-position piem--streaming-marker)))
      (piem--display-message-delta "Hello")
      (should (= (marker-position piem--streaming-marker)
                 (+ initial-pos 5))))))

(ert-deftest piem-test-streaming-inserts-at-marker ()
  "Deltas are inserted at the streaming marker position."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (piem--display-message-delta "First")
    (piem--display-message-delta " Second")
    (should (string-match-p "First Second" (buffer-string)))))

;;; Auto-scroll

(ert-deftest piem-test-window-following-p-at-end ()
  "piem--window-following-p detects when window-point is at end."
  (with-temp-buffer
    (insert "some content")
    ;; Mock window-point to return point-max
    (cl-letf (((symbol-function 'window-point) (lambda (_w) (point-max))))
      (should (piem--window-following-p 'mock-window)))))

(ert-deftest piem-test-window-following-p-not-at-end ()
  "piem--window-following-p returns nil when window-point is earlier."
  (with-temp-buffer
    (insert "some content")
    ;; Mock window-point to return position before end
    (cl-letf (((symbol-function 'window-point) (lambda (_w) 1)))
      (should-not (piem--window-following-p 'mock-window)))))

(ert-deftest piem-test-rewrite-tail-window-p-keeps-lower-tail-view-following ()
  "A lower-window tail view should stay in tail-following mode after a rewrite."
  (should (piem--rewrite-tail-window-p 10 99 100 18 30))
  (should (piem--rewrite-tail-window-p 99 50 100 5 30))
  (should-not (piem--rewrite-tail-window-p 10 50 100 18 30)))

(ert-deftest piem-test-rewrite-tail-window-p-keeps-mid-buffer-context-when-tall-window-shows-tail ()
  "A tall window showing the tail should not outrank an in-view mid-buffer point."
  (should-not (piem--rewrite-tail-window-p 60 199 200 10 36)))

(ert-deftest piem-test-rewrite-tail-window-p-ignores-offscreen-point ()
  "A stale tail-reaching window end should not make an offscreen point follow."
  (should-not (piem--rewrite-tail-window-p 60 199 200 14 11)))

(ert-deftest piem-test-clamp-rewrite-point-row-pushes-point-lower-when-tail-shrinks ()
  "Shrinking the tail should move point lower so the rewritten window stays filled."
  (should (= 12 (piem--clamp-rewrite-point-row 3 40 8 20))))

(ert-deftest piem-test-clamp-rewrite-point-row-falls-back-when-buffer-too-short ()
  "When the whole buffer is shorter than the window, preserve the highest visible row."
  (should (= 5 (piem--clamp-rewrite-point-row 10 5 8 20))))

;;; Pandoc Conversion

(ert-deftest piem-test-message-start-marker-created ()
  ...
  (with-temp-buffer
    ...
    (piem--display-agent-start)
    (should (markerp piem--message-start-marker))
    (should (= (marker-position piem--message-start-marker)
               (marker-position piem--streaming-marker)))))

(ert-deftest piem-test-render-complete-message-applies-fontlock ()
...
(ert-deftest piem-test-chat-mode-derives-from-markdown-ts ()
  "Chat mode derives from md-ts-mode for tree-sitter highlighting."
  (with-temp-buffer
    (piem-chat-mode)
    (should (derived-mode-p 'md-ts-mode))))

(ert-deftest piem-test-chat-mode-fontifies-code ()
...
(ert-deftest piem-test-incomplete-code-block-does-not-break-fontlock ()
...
block is closed."
  (with-temp-buffer
    ...
    (let ((inhibit-read-only t))
      ...
      (should (eq major-mode 'piem-chat-mode))
      (goto-char (point-min))
...
(ert-deftest piem-test-display-user-message-inserts-text ()
  "User message is inserted into chat buffer."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-user-message "Hello world")
    (should (string-match-p "Hello world" (buffer-string)))))

(ert-deftest piem-test-display-user-message-has-prefix ()
  "User message has You label in setext heading."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-user-message "Test message")
    (should (string-match-p "^You" (buffer-string)))))

(ert-deftest piem-test-display-user-message-has-separator ()
  "User message has setext underline separator."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-user-message "Test")
    (should (string-match-p "^===" (buffer-string)))))

(ert-deftest piem-test-send-displays-user-message ()
  "Accepted prompt preflight displays the user message in chat."
  (let ((chat-buf (get-buffer-create "*piem-test-chat*"))
        (input-buf (get-buffer-create "*piem-test-input*"))
        (rpc-callback nil)
...
(ert-deftest piem-test-send-slash-command-not-displayed-locally ()
...
(ert-deftest piem-test-slash-command-after-abort-no-duplicate-headers ()
...
since we don't display them locally. Let pi's message_start handle it."
  (let ((chat-buf (get-buffer-create "*piem-test-abort-cmd*"))
        (input-buf (get-buffer-create "*piem-test-abort-cmd-input*")))
    (unwind-protect
        (progn
          (with-current-buffer chat-buf
            (piem-chat-mode)
...
(ert-deftest piem-test-ms-to-time-converts-correctly ()
  "piem--ms-to-time converts milliseconds to Emacs time."
  ;; 1704067200000 ms = 2024-01-01 00:00:00 UTC
  (let ((time (piem--ms-to-time 1704067200000)))
    (should time)
    (should (equal (format-time-string "%Y-%m-%d" time t) "2024-01-01"))))

(ert-deftest piem-test-ms-to-time-returns-nil-for-nil ()
  "piem--ms-to-time returns nil when given nil."
  (should (null (piem--ms-to-time nil))))

(ert-deftest piem-test-format-message-timestamp-includes-date-for-today ()
...
(ert-deftest piem-test-display-retry-start-shows-attempt ()
  "auto_retry_start event shows attempt number and delay."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-retry-start '(:type "auto_retry_start"
                               :attempt 1
                               :maxAttempts 3
                               :delayMs 2000
...
(ert-deftest piem-test-display-retry-start-with-overloaded-error ()
...
(ert-deftest piem-test-display-error-shows-message ()
  "piem--display-error shows error message with proper face."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-error "API error: insufficient quota")
    (should (string-match-p "Error:" (buffer-string)))
    (should (string-match-p "insufficient quota" (buffer-string)))))

(ert-deftest piem-test-display-error-handles-nil ()
  "piem--display-error handles nil error message."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-error nil)
    (should (string-match-p "Error:" (buffer-string)))
    (should (string-match-p "unknown" (buffer-string)))))

(ert-deftest piem-test-display-startup-error ()
...
(ert-deftest piem-test-display-startup-error-env-node-hint ()
...
(ert-deftest piem-test-display-startup-error-no-env-node-hint ()
...
(ert-deftest piem-test-display-extension-error ()
  "extension_error event shows extension name and error."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-extension-error '(:type "extension_error"
...
(ert-deftest piem-test-handle-display-event-retry-start ()
  "piem--handle-display-event handles auto_retry_start."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem--status 'streaming)
          (piem--state nil))
      (piem--handle-display-event '(:type "auto_retry_start"
                                  :attempt 1
                                  :maxAttempts 3
                                  :delayMs 2000
                                  :errorMessage "429 rate_limit_error"))
...
(ert-deftest piem-test-handle-display-event-retry-end ()
  "piem--handle-display-event handles auto_retry_end."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem--status 'streaming)
          (piem--state nil))
      (piem--handle-display-event '(:type "auto_retry_end"
                                  :success t
                                  :attempt 2))
      (should (string-match-p "succeeded" (buffer-string))))))

(ert-deftest piem-test-handle-display-event-extension-error ()
  "piem--handle-display-event handles extension_error."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem--status 'streaming)
          (piem--state (list :last-error nil)))
      (piem--handle-display-event '(:type "extension_error"
                                  :extensionPath "/path/extension.ts"
                                  :event "before_send"
                                  :error "Extension failed"))
      (should (string-match-p "Extension error" (buffer-string))))))

(ert-deftest piem-test-handle-display-event-message-error ()
  "piem--handle-display-event handles message_update with error type."
  (with-temp-buffer
    (piem-chat-mode)
    ;; Need to set up markers first
    (piem--display-agent-start)
    (let ((piem--status 'streaming)
          (piem--state (list :current-message '(:role "assistant"))))
      (piem--handle-display-event '(:type "message_update"
                                  :message (:role "assistant")
...
(ert-deftest piem-test-display-no-model-warning ()
  "piem--display-no-model-warning shows setup instructions."
  (with-temp-buffer
    (piem-chat-mode)
...
(ert-deftest piem-test-extension-ui-notify ()
  ...
  (let ((message-shown nil))
    (cl-letf (((symbol-function 'message)
      ...
      (with-temp-buffer
        (piem-chat-mode)
        (let ((piem--process nil))
          (piem--handle-extension-ui-request
...
(ert-deftest piem-test-extension-ui-confirm-no ()
...
(ert-deftest piem-test-extension-ui-select ()
...
(ert-deftest piem-test-extension-ui-input ()
...
(ert-deftest piem-test-extension-ui-set-editor-text ()
...
(ert-deftest piem-test-prompt-image-draft-replacements-clear-attachment ()
  ...
  (piem-test-with-prompt-image-session (dir chat-buf input-buf)
    (let ((path (piem-test--write-prompt-image (expand-file-name "stale.png" dir) 'png)))
      (with-current-buffer input-buf
        (insert "old draft")
      ...
      (with-current-buffer input-buf
        ...
        (should-not (string-match-p "stale.png" (piem-test--input-header)))
        (piem-test--attach-image path))
      (piem--browse-prefill-input input-buf "Browser replacement")
      (with-current-buffer input-buf
        (should (equal (buffer-string) "Browser replacement"))
        (should-not (string-match-p "stale.png" (piem-test--input-header)))))))

(ert-deftest piem-test-extension-ui-set-status ()
  "extension_ui_request setStatus updates extension status storage."
  (with-temp-buffer
    (piem-chat-mode)
    (setq piem--extension-status nil)
    (piem--handle-extension-ui-request
     '(:type "extension_ui_request"
       :id "req-7"
       :method "setStatus"
    ...
    (should (equal (cdr (assoc "my-ext" piem--extension-status))
                   "Processing..."))))

(ert-deftest piem-test-extension-ui-set-status-strips-ansi ()
  ...
  (with-temp-buffer
    ...
    (should (equal (cdr (assoc "plan-mode" piem--extension-status))
                   "⏸ plan"))))

(ert-deftest piem-test-extension-ui-set-status-clear ()
  "extension_ui_request setStatus with nil clears the status."
  (with-temp-buffer
    (piem-chat-mode)
    (setq piem--extension-status '(("my-ext" . "Old status")))
    (piem--handle-extension-ui-request
     '(:type "extension_ui_request"
       :id "req-8"
       :method "setStatus"
       :statusKey "my-ext"
       :statusText nil))
    (should-not (assoc "my-ext" piem--extension-status))))

(ert-deftest piem-test-extension-ui-set-working-message ()
  "extension_ui_request setWorkingMessage stores working text."
  (with-temp-buffer
    (piem-chat-mode)
    (setq piem--working-message nil)
    (piem--handle-extension-ui-request
     '(:type "extension_ui_request"
       :id "req-working"
       :method "setWorkingMessage"
       :message "📖 Skimming…"))
    (should (equal piem--working-message "📖 Skimming…"))))

(ert-deftest piem-test-extension-ui-set-working-message-strips-ansi ()
...
(ert-deftest piem-test-extension-ui-set-working-message-clear ()
  "extension_ui_request setWorkingMessage with nil clears message."
  (with-temp-buffer
    (piem-chat-mode)
    (setq piem--working-message "Old")
    (piem--handle-extension-ui-request
     '(:type "extension_ui_request"
...
(ert-deftest piem-test-extension-ui-unsupported-warns ()
  "Unsupported extension_ui_request method warns via `message'.
See https://github.com/dnouri/piem/issues/176."
...
(ert-deftest piem-test-extension-ui-unsupported-warns-once-per-method ()
...
(ert-deftest piem-test-extension-ui-unsupported-warnings-are-buffer-local ()
...
(ert-deftest piem-test-header-format-extension-status ()
  "Extension status formatter returns inline neutral status text without pipe."
  ;; Empty status returns empty string
  (should (equal (piem--header-format-extension-status nil) ""))
  ;; Single status
  (let* ((result (piem--header-format-extension-status '(("ext1" . "Processing..."))))
  ...
  (let ((result (piem--header-format-extension-status
...
(ert-deftest piem-test-extension-ui-unknown-cancels ()
  "extension_ui_request with unknown method sends cancelled response."
  (let ((response-sent nil))
    (cl-letf (((symbol-function 'piem--send-extension-ui-response)
               (lambda (_proc msg)
                 (setq response-sent msg)))
...
(ert-deftest piem-test-extension-ui-editor-cancels ()
...
(ert-deftest piem-test-pretty-print-json-simple-plist ()
  "Pretty-print helper produces 2-space indented JSON from plist."
  (let ((result (piem--pretty-print-json
...
(ert-deftest piem-test-pretty-print-json-nested ()
...
(ert-deftest piem-test-pretty-print-json-unicode ()
...
(ert-deftest piem-test-pretty-print-json-nil ()
  "Pretty-print helper returns nil for nil input."
  (should-not (piem--pretty-print-json nil)))

;;; Tool Header

(ert-deftest piem-test-tool-header-faces ()
  "Tool header applies tool-name face on prefix and tool-command on args."
  ;; bash: "$" is tool-name, command is tool-command
  (let ((header (piem--tool-header "bash" '(:command "ls -la"))))
...
(ert-deftest piem-test-generic-tool-header-with-args ()
  "Generic tool header shows tool name and JSON args."
  (let ((header (piem--tool-header
...
(ert-deftest piem-test-generic-tool-header-escapes-control-and-format-chars ()
...
(ert-deftest piem-test-generic-tool-header-compact-when-short ()
  "Short args produce a single-line compact header."
  (let* ((fill-column 70)
         (header (piem--tool-header "subagent" '(:agent "worker")))
  ...
  (let ((args '(:agent "worker" :task "Search")))
    ...
    (let* ((fill-column 200)
           (text (substring-no-properties
                  (piem--tool-header "subagent" args))))
      (should (= 1 (length (split-string text "\n")))))
...
(ert-deftest piem-test-generic-tool-header-faces ()
...
(ert-deftest piem-test-builtin-tools-unaffected-by-generic-header ()
...
(ert-deftest piem-test-tool-path-header-escapes-controls-for-display-only ()
  ...
  (with-temp-buffer
    ...
    (let ((path (concat "/tmp/project/a\nread /etc/passwd\r\t"
                        ...
                        (string #x202e)
                        ".el")))
      (piem--display-tool-start "read" (list :path path))
      (save-excursion
        (goto-char (point-min))
      ...
      (let ((ov piem--pending-tool-overlay))
        (should ov)
        (should (equal path (overlay-get ov 'piem-tool-path)))
        (should (equal path (overlay-get ov 'piem-tool-raw-path)))))))

;;; Tool Output

(ert-deftest piem-test-tool-start-inserts-header ()
  "tool_execution_start inserts tool header."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "bash" (list :command "ls -la"))
    ;; Should have $ command format
    (should (string-match-p "\\$ ls -la" (buffer-string)))))

(ert-deftest piem-test-tool-start-handles-file-path-key ()
  "tool_execution_start handles :file_path key (alternative to :path)."
  (with-temp-buffer
    (piem-chat-mode)
    ;; Test read tool with :file_path
    (piem--display-tool-start "read" '(:file_path "/tmp/test.txt"))
    (should (string-match-p "read /tmp/test.txt" (buffer-string))))
  (with-temp-buffer
    (piem-chat-mode)
    ;; Test write tool with :file_path
    (piem--display-tool-start "write" '(:file_path "/tmp/out.py"))
    (should (string-match-p "write /tmp/out.py" (buffer-string))))
  (with-temp-buffer
    (piem-chat-mode)
    ;; Test edit tool with :file_path
    (piem--display-tool-start "edit" '(:file_path "/tmp/edit.rs"))
    (should (string-match-p "edit /tmp/edit.rs" (buffer-string)))))

(ert-deftest piem-test-tool-end-inserts-result ()
...
(defun piem-test--image-preview-positions ()
    ...
    (while (setq position
      ...
      (setq position
            (or (next-single-property-change
                 position 'piem-image-preview nil (point-max))
                (point-max))))
    (nreverse positions)))

(defconst piem-test--png-base64
...
(ert-deftest piem-test-tool-result-image-has-terminal-placeholder ()
  "An image result is useful text inside its tool block in a terminal."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((block (piem--display-tool-start
      ...
      (cl-letf (((symbol-function 'display-images-p) (lambda (&rest _) nil)))
        (piem--display-tool-end
         ...
         `[(:type "text" :text "generated")
           (:type "image" :data ,piem-test--png-base64
            :mimeType "image/png")]
         nil nil block))
      (let* ((overlay (piem--tool-block-overlay block))
             (positions (piem-test--image-preview-positions))
             (position (car positions)))
        (should (= 1 (length positions)))
        (should (<= (overlay-start overlay) position))
        (should (< position (overlay-end overlay)))
        (should (get-text-property position 'piem-no-fontify))
...
(ert-deftest piem-test-prompt-image-live-and-history-use-image-preview ()
        ...
        (cl-letf (((symbol-function 'display-images-p) (lambda (&rest _) nil)))
          (let ((message
            ...
            (if (eq route 'live)
              ...
              (piem--display-history-messages (vector message)))))
        (should (string-match-p "Visual question" (buffer-string)))
        (should (string-match-p "Image: image/png, 69 B" (buffer-string)))
        (should (= 1 (length (piem-test--image-preview-positions))))))))

(ert-deftest piem-test-tool-result-image-inserts-scaled-display-property ()
  ...
  (with-temp-buffer
    ...
    (cl-letf (((symbol-function 'display-images-p) (lambda (&rest _) t))
      ...
      (piem--display-tool-end
       ...
       nil nil))
    (should (= 5 (length (piem-test--image-preview-positions))))
    (let ((text (buffer-substring-no-properties (point-min) (point-max))))
...
(ert-deftest piem-test-tool-result-images-obey-source-and-count-caps ()
  "Oversized and excess images become textual placeholders."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-image-preview-max-bytes 2)
          (piem--image-previews-per-content-limit 2))
      (cl-letf (((symbol-function 'display-images-p) (lambda (&rest _) t))
                ((symbol-function 'create-image)
                 (lambda (&rest _)
...
(ert-deftest piem-test-tool-result-image-history-replays-preview ()
      ...
      (piem--display-history-messages
       `[(:role "assistant"
         ...
         (:role "toolResult" :toolCallId "image-history" :toolName "generate"
          :content [(:type "image" :data ,piem-test--png-base64
                     :mimeType "image/png")]
          :isError :json-false)]))
    (should (= 1 (length (piem-test--image-preview-positions))))
...
(ert-deftest piem-test-tool-result-image-toggle-reuses-preview ()
  "Collapse and expand preserve one decoded preview inside stable bounds."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-tool-preview-lines 2)
          (create-count 0)
        ...
        (let ((block (piem--display-tool-start
          ...
          (dotimes (_ 2)
            (let* ((overlay (piem--tool-block-overlay block))
              ...
              (let ((position (car (piem-test--image-preview-positions))))
                (should position)
                (should (< position (overlay-end overlay)))
...
(ert-deftest piem-test-tool-result-image-survives-cooling ()
      ...
      (cl-letf (((symbol-function 'display-images-p) (lambda (&rest _) t))
        ...
        (let ((block (piem--display-tool-start
          ...
          (piem--cool-completed-tool-blocks
           (list (piem--tool-block-overlay block)))))
      (let* ((positions (piem-test--image-preview-positions))
             (position (car positions)))
        (should (= 1 create-count))
        (should (= 1 (length positions)))
        (should (eq 'cooled-image-spec
                    (get-text-property position 'display)))
        (should (get-text-property position 'piem-cold-tool-block))
        (should-not (piem-test--all-tool-overlays))))))

(ert-deftest piem-test-read-svg-uses-complete-returned-text ()
...
(ert-deftest piem-test-read-svg-refuses-truncated-result ()
  ...
  (with-temp-buffer
    ...
    (should-not (piem-test--image-preview-positions))
    (should (string-match-p "<svg" (buffer-string)))))

(ert-deftest piem-test-read-svg-history-and-resource-refusal ()
...
(ert-deftest piem-test-keyed-final-does-not-use-legacy-tool-block ()
  "A keyed final miss creates its own block instead of using the legacy block."
  (with-temp-buffer
    (piem-chat-mode)
    (let* ((legacy (piem--display-tool-start "bash" nil))
           (legacy-overlay (piem--tool-block-overlay legacy)))
      (cl-letf (((symbol-function 'display-images-p) (lambda (&rest _) nil)))
        (piem--handle-display-event
...
(ert-deftest piem-test-text-only-tool-result-adds-no-image-preview ()
...
(ert-deftest piem-test-bash-output-wrapped-in-bare-fence ()
...
(ert-deftest piem-test-bash-output-strips-ansi-codes ()
  ...
  (with-temp-buffer
    ...
    (let ((ansi-output "\033[34m▶ AmbientSoundConfig\033[0m\n\033[32m  ✓ \033[0mshould pass"))
      (piem--display-tool-end "bash" '(:command "test")
...
(ert-deftest piem-test-tool-output-shows-preview-when-long ()
...
(ert-deftest piem-test-tool-output-short-shows-all ()
...
(defun piem-test--insert-generic-tool (content-text &optional details)
  "Insert a subagent tool start+end in current buffer.
CONTENT-TEXT is the text block string.  DETAILS is an optional plist.
Call inside `with-temp-buffer' after `piem-chat-mode'."
  (piem--display-tool-start "subagent" '(:agent "worker"))
  (piem--display-tool-end "subagent" '(:agent "worker")
                        (list (list :type "text" :text content-text))
                        details nil))

(ert-deftest piem-test-generic-tool-content-follows-header ()
  "Generic tool content is fenced directly after the header line."
  (with-temp-buffer
    (piem-chat-mode)
    (piem-test--insert-generic-tool "Task completed")
    (let ((content (buffer-string)))
      ;; Content is fenced (bare fence, no language tag)
      (should (string-match-p "}\n```\nTask completed" content)))))

(ert-deftest piem-test-bash-no-blank-line-after-header ()
...
(ert-deftest piem-test-generic-tool-details-appended ()
  "Generic tool with non-nil details shows details JSON after content."
  (with-temp-buffer
    (piem-chat-mode)
    (piem-test--insert-generic-tool
...
(ert-deftest piem-test-generic-tool-details-face ()
  "Details label and JSON both use piem-tool-output face."
  (with-temp-buffer
    (piem-chat-mode)
    (piem-test--insert-generic-tool
...
(ert-deftest piem-test-generic-tool-details-marked-no-fontify ()
...
(ert-deftest piem-test-generic-tool-with-path-toggle-skips-details-font-lock ()
  ...
  (with-temp-buffer
    ...
    (let* ((piem-tool-preview-lines 5)
           ...
           (details (list :summary (make-string 3000 ?x)))
           (font-lock-calls nil))
      (piem--display-tool-start "custom_tool" '(:path "/tmp/example.py"))
      (piem--display-tool-end
      ...
      (let ((btn (button-at (match-beginning 0))))
        (should btn)
        (let ((full-content (button-get btn 'piem-full-content)))
...
(ert-deftest piem-test-generic-tool-nil-details-unchanged ()
...
(ert-deftest piem-test-generic-tool-details-nested ()
...
(ert-deftest piem-test-generic-tool-details-in-expanded-view ()
  ...
  (with-temp-buffer
    ...
    (let* ((long-output (mapconcat (lambda (n) (format "line%d" n))
      ...
      (piem-test--insert-generic-tool long-output details)
...
queue drained before the deadline."
  (cl-letf (((symbol-function 'input-pending-p)
    ...
    (let ((deadline (+ (float-time) 3)))
      (while (and piem--tool-cooling-queue
                  (< (float-time) deadline))
        (sit-for 0.05))
      (not piem--tool-cooling-queue))))

(defun piem-test--render-completed-tool-turn
...
Synthetic turns also reset `piem--assistant-header-shown' so
...
settled event loop without wall-clock timing."
  ...
  (piem-test--drain-tool-cooling))

(defmacro piem-test--with-recorded-cooling-timers
    (jobs &rest body)
...
(defun piem-test--render-headed-completed-read-block (tool-call-id)
  "Render and return a headed completed read overlay for TOOL-CALL-ID."
  (setq piem--assistant-header-shown nil)
  (piem--display-agent-start)
  (piem-test--render-completed-read-block tool-call-id))

(defun piem-test--render-long-cooling-read-block
...
When EXPANDED is non-nil, expand its preview before returning the overlay."
  (let* ((piem-tool-preview-lines 3)
    ...
    (let ((overlay (piem--tool-block-overlay block)))
      (when expanded
        (let ((button (piem--find-toggle-button-in-region
                       (overlay-start overlay) (overlay-end overlay))))
          (should button)
          (piem--toggle-tool-output button)))
      overlay)))

(defun piem-test--render-collapsed-cooling-bash-block (tool-call-id)
  "Render a collapsed completed bash block for TOOL-CALL-ID."
  (let* ((piem-bash-preview-lines 3)
...
(defun piem-test--window-point-text-p (window text)
...
(ert-deftest piem-test-deferred-tool-cooling-agent-end-schedules-cohort ()
  "agent_end queues the cold cohort without synchronously rewriting it."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-hot-tail-turn-count 1)
          cooled)
      (piem-test--with-recorded-cooling-timers jobs
        (let ((older
               (piem-test--render-headed-completed-read-block "old"))
              (newer
               (piem-test--render-headed-completed-read-block "new")))
          (cl-letf (((symbol-function 'piem--cool-tool-overlay)
                     (lambda (overlay)
                       (push overlay cooled))))
            (piem--handle-display-event '(:type "agent_end")))
          (should-not cooled)
          ...
          (should (equal piem--tool-cooling-queue (list older)))
          (should (eq piem--tool-cooling-timer
...
(ert-deftest piem-test-deferred-tool-cooling-position-map-boundaries ()
  "Cooling position mapping is continuous at replacement boundaries."
  ;; Shrinking [10, 20) to [10, 15).
  (should (= 9 (piem--map-tool-cooling-position 9 10 20 15)))
  (should (= 10 (piem--map-tool-cooling-position 10 10 20 15)))
  (should (= 12 (piem--map-tool-cooling-position 12 10 20 15)))
  (should (= 15 (piem--map-tool-cooling-position 19 10 20 15)))
  (should (= 15 (piem--map-tool-cooling-position 20 10 20 15)))
  (should (= 20 (piem--map-tool-cooling-position 25 10 20 15)))
  ;; Expanding preserves inside offsets and shifts the old end and suffix.
  (should (= 19 (piem--map-tool-cooling-position 19 10 20 25)))
  (should (= 25 (piem--map-tool-cooling-position 20 10 20 25)))
  (should (= 30 (piem--map-tool-cooling-position 25 10 20 25))))

(ert-deftest piem-test-deferred-tool-cooling-view-after-body-stays-logical ()
  ...
  (let ((buffer (generate-new-buffer " *pi-cooling-view-after*"))
    ...
    (unwind-protect
        (progn
          (with-current-buffer buffer
            (piem-chat-mode)
            (piem-test--render-long-cooling-read-block
             "view-after" t)
            (let ((inhibit-read-only t))
              (goto-char (point-max))
              (dotimes (index 10)
          ...
          (piem-test--with-recorded-cooling-timers jobs
            (with-current-buffer buffer
              (let ((piem-hot-tail-turn-count 0))
                (piem--handle-display-event '(:type "agent_end"))))
            (should (= 1 (length jobs)))
            (save-window-excursion
              ...
              (let* ((selected (selected-window))
                ...
                (cl-letf (((symbol-function 'input-pending-p)
                           (lambda (&rest _) nil)))
                  (piem-test--invoke-recorded-cooling-timer
                   (pop jobs)))
                (should-not jobs)
                (should (equal "VIEW-A-START context"
                               (piem-test--window-start-line
                                selected)))
                (should (piem-test--window-point-text-p
                         selected "VIEW-A-POINT sentinel"))
                (should (equal "VIEW-B-START context"
                               (piem-test--window-start-line other)))
                (should (piem-test--window-point-text-p
...
(ert-deftest piem-test-deferred-tool-cooling-view-inside-body-maps-and-clamps ()
    ...
    (unwind-protect
        (progn
          (with-current-buffer buffer
            (piem-chat-mode)
            (let* ((overlay
                    (piem-test--render-collapsed-cooling-bash-block
                     "view-inside"))
                   (button (piem--find-toggle-button-in-region
                            (overlay-start overlay) (overlay-end overlay))))
              (should button)
          ...
          (piem-test--with-recorded-cooling-timers jobs
            ...
            (save-window-excursion
              ...
              (let* ((window (selected-window))
                     (overlay
                      (car (piem-test--all-tool-overlays)))
                     (body-start
                      (marker-position
                ...
                (let* ((new-body-end
                        (next-single-property-change
                         body-start 'piem-cold-tool-block
                         nil (point-max)))
...
(ert-deftest piem-test-deferred-tool-cooling-view-tail-keeps-following ()
          ...
          (piem-test--with-recorded-cooling-timers jobs
            ...
            (save-window-excursion
              ...
              (let ((window (selected-window))
                ...
                (goto-char (point-max))
                (recenter -1)
                (should (piem--window-following-p window))
                (cl-letf (((symbol-function 'input-pending-p)
                           (lambda (&rest _) nil)))
                  (piem-test--invoke-recorded-cooling-timer
                   (pop jobs)))
                ...
                (should (piem-test--window-shows-tail-p window))))))
      (when (buffer-live-p buffer)
        (kill-buffer buffer)))))

(ert-deftest piem-test-deferred-tool-cooling-worker-does-one-and-rearms ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-hot-tail-turn-count 0)
          mutations)
      (dotimes (index 3)
        (piem-test--render-completed-read-block
         (format "call-%d" index)))
      (piem-test--with-recorded-cooling-timers jobs
        (piem--handle-display-event '(:type "agent_end"))
        (should (= 3 (length piem--tool-cooling-queue)))
        (let ((original (symbol-function 'piem--cool-tool-overlay)))
          (cl-letf (((symbol-function 'input-pending-p)
                     (lambda (&rest _) nil))
                    ((symbol-function 'piem--cool-tool-overlay)
                     (lambda (overlay)
                       (push (cons overlay piem--tool-cooling-timer)
                             mutations)
                       (funcall original overlay))))
            (dotimes (index 3)
              (should (= 1 (length jobs)))
              (let ((before (length (piem-test--all-tool-overlays)))
                    (job (pop jobs)))
                (should (eq piem--tool-cooling-timer
                            (plist-get job :timer)))
                (should-not (plist-get job :owner))
                (piem-test--invoke-recorded-cooling-timer job)
                (should (= (1- before)
                           (length (piem-test--all-tool-overlays))))
                (if (< index 2)
                    (progn
                      (should (= 1 (length jobs)))
                      (should piem--tool-cooling-timer))
                  (should-not jobs)
                  (should-not piem--tool-cooling-queue)
                  (should-not piem--tool-cooling-timer))))))
        (should (= 3 (length mutations)))
        (should (cl-every (lambda (entry) (null (cdr entry))) mutations))))))

(ert-deftest piem-test-deferred-tool-cooling-input-yields-without-progress ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-hot-tail-turn-count 0)
      ...
      (piem-test--with-recorded-cooling-timers jobs
        (piem--handle-display-event '(:type "agent_end"))
        (let ((queue-before (copy-sequence piem--tool-cooling-queue))
              (text-before (buffer-string))
              (timer-before piem--tool-cooling-timer)
              (job (pop jobs)))
          (cl-letf (((symbol-function 'input-pending-p)
                     (lambda (&rest _) t))
                    ((symbol-function 'piem--cool-tool-overlay)
            ...
            (piem-test--invoke-recorded-cooling-timer job))
          (should (= 0 cool-calls))
          (should (equal queue-before piem--tool-cooling-queue))
          (should (equal text-before (buffer-string)))
          (should (= 1 (length jobs)))
          (should piem--tool-cooling-timer)
          (should-not (eq timer-before piem--tool-cooling-timer))
          (should-not (plist-get (car jobs) :owner))
          (piem--cancel-tool-cooling))))))

(ert-deftest piem-test-deferred-tool-cooling-revalidates-each-candidate ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-hot-tail-turn-count 0)
      ...
      (piem-test--with-recorded-cooling-timers jobs
        (piem--handle-display-event '(:type "agent_end"))
        (let* ((candidates (copy-sequence piem--tool-cooling-queue))
               ...
               (original (symbol-function 'piem--cool-tool-overlay)))
          ...
          (cl-labels
            ...
            (cl-letf (((symbol-function 'input-pending-p)
              ...
              (let* ((record (overlay-get live
                                          'piem-tool-block-record))
                     (tool-call-id
                      (piem--tool-block-tool-call-id record)))
                (puthash tool-call-id record piem--live-tool-blocks)
                (run-next)
                (remhash tool-call-id piem--live-tool-blocks))
              (should (overlay-buffer live))
              ;; Completion metadata disappeared before this slice.
              (overlay-put incomplete 'piem-header-end nil)
              (run-next)
...
(ert-deftest piem-test-deferred-tool-cooling-cancel-invalidates-clear-and-history ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-hot-tail-turn-count 0)
      ...
      (piem-test--with-recorded-cooling-timers jobs
        ...
        (piem--handle-display-event '(:type "agent_end"))
        (let ((job (pop jobs))
              (generation piem--tool-cooling-generation))
          (piem--clear-render-artifacts)
          (should (> piem--tool-cooling-generation generation))
          (cl-letf (((symbol-function 'piem--cool-tool-overlay)
                     (lambda (&rest _)
                       (setq cool-calls (1+ cool-calls)))))
            (piem-test--invoke-recorded-cooling-timer job))
...
(ert-deftest piem-test-deferred-tool-cooling-cancel-invalidates-buffer-kill ()
  "Killing a chat cancels cooling and makes its recorded callback harmless."
  (let ((buffer (generate-new-buffer "*piem-test-cooling-kill*"))
        (piem-quit-without-confirmation t)
        cancelled
        cool-calls)
    (unwind-protect
        (piem-test--with-recorded-cooling-timers jobs
          (with-current-buffer buffer
            (piem-chat-mode)
            (let ((piem-hot-tail-turn-count 0))
              (piem-test--render-completed-read-block "kill")
              (piem--handle-display-event '(:type "agent_end")))
            (let ((job (car jobs))
                  (timer piem--tool-cooling-timer))
...
(ert-deftest piem-test-deferred-tool-cooling-error-fails-closed ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-hot-tail-turn-count 0)
      ...
      (piem-test--with-recorded-cooling-timers jobs
        (piem--handle-display-event '(:type "agent_end"))
        (let ((generation piem--tool-cooling-generation)
              (job (pop jobs)))
          (cl-letf (((symbol-function 'input-pending-p)
                     (lambda (&rest _) nil))
                    ((symbol-function 'piem--cool-tool-overlay)
        ...
        (piem--queue-tool-cooling-outside-hot-tail)
        (should (= 2 (length piem--tool-cooling-queue)))
        (should (= 1 (length jobs)))
        (should piem--tool-cooling-timer)
        (piem--cancel-tool-cooling)))))

(ert-deftest piem-test-deferred-tool-cooling-quit-cancels-and-resignals ()
...
(ert-deftest piem-test-deferred-tool-cooling-stale-a-leaves-b-owned ()
  "A cancelled generation callback cannot clear or mutate generation B."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-hot-tail-turn-count 0)
          mutations)
      (piem-test--render-completed-read-block "generation")
      (piem-test--with-recorded-cooling-timers jobs
        (piem--handle-display-event '(:type "agent_end"))
        (let ((stale-job (pop jobs)))
          (piem--cancel-tool-cooling)
          (piem--queue-tool-cooling-outside-hot-tail)
          (let ((generation-b piem--tool-cooling-generation)
                (queue-b (copy-sequence piem--tool-cooling-queue))
                (timer-b piem--tool-cooling-timer)
                (job-b (car jobs)))
            (cl-letf (((symbol-function 'piem--cool-tool-overlay)
                       (lambda (overlay)
                         (push overlay mutations))))
              (piem-test--invoke-recorded-cooling-timer stale-job))
            (should (= generation-b piem--tool-cooling-generation))
            (should (equal queue-b piem--tool-cooling-queue))
            (should (eq timer-b piem--tool-cooling-timer))
            (should-not mutations)
            (should (eq job-b (pop jobs)))
            (cl-letf (((symbol-function 'input-pending-p)
                       (lambda (&rest _) nil)))
              (piem-test--invoke-recorded-cooling-timer job-b))
            (should-not jobs)
            (should-not piem--tool-cooling-queue)
            (should-not piem--tool-cooling-timer)))))))

(ert-deftest piem-test-deferred-tool-cooling-uses-latest-expanded-state ()
    ...
    (let ((piem-hot-tail-turn-count 0))
      (let ((overlay
        ...
        (piem-test--with-recorded-cooling-timers jobs
          ...
          (cl-letf (((symbol-function 'input-pending-p)
                     (lambda (&rest _) nil)))
            (piem-test--invoke-recorded-cooling-timer (pop jobs)))
          (should (string-match-p "read body line 20" (buffer-string)))
          (should-not (string-match-p "more lines" (buffer-string)))
          (should-not (piem-test--all-tool-overlays))
          (goto-char (point-min))
...
(ert-deftest piem-test-deferred-tool-cooling-second-enqueue-deduplicates ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-hot-tail-turn-count 0))
      ...
      (piem-test--with-recorded-cooling-timers jobs
        (piem--handle-display-event '(:type "agent_end"))
        (let ((timer piem--tool-cooling-timer))
          (piem--handle-display-event '(:type "agent_end"))
          (should (eq timer piem--tool-cooling-timer))
          (should (= 2 (length piem--tool-cooling-queue)))
          (should (= 2 (length (delete-dups
                                (copy-sequence
                                 piem--tool-cooling-queue)))))
          (should (= 1 (length jobs)))
          (piem-test--render-completed-read-block "call-new")
          (piem--handle-display-event '(:type "agent_end"))
          (should (eq timer piem--tool-cooling-timer))
          (should (= 3 (length piem--tool-cooling-queue)))
          (should (= 3 (length (delete-dups
                                (copy-sequence
                                 piem--tool-cooling-queue)))))
          (should (= 1 (length jobs)))
          (let ((expected (copy-sequence piem--tool-cooling-queue))
                cooled
                (original (symbol-function 'piem--cool-tool-overlay)))
            (cl-letf (((symbol-function 'input-pending-p)
                       (lambda (&rest _) nil))
...
(ert-deftest piem-test-deferred-tool-cooling-real-timers-drain-cohort ()
  "Real one-shot timer delivery cools the queued cohort and keeps the newest turn hot."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-hot-tail-turn-count 1))
      (let ((oldest
             (piem-test--render-headed-completed-read-block "drain-old-1"))
            (middle
             (piem-test--render-headed-completed-read-block "drain-old-2"))
            (third
             (piem-test--render-headed-completed-read-block "drain-old-3"))
            (newest
             (piem-test--render-headed-completed-read-block "drain-new")))
        (piem--handle-display-event '(:type "agent_end"))
        (should (equal piem--tool-cooling-queue
                       (list third middle oldest)))
        (should piem--tool-cooling-timer)
        (should (piem-test--drain-tool-cooling-via-real-timers))
        (should-not piem--tool-cooling-queue)
        (should-not piem--tool-cooling-timer)
        (dolist (cooled (list oldest middle third))
          (should-not (overlay-buffer cooled)))
        (should (overlay-buffer newest))
        (should (equal (piem-test--all-tool-overlays) (list newest)))
        (goto-char (point-min))
...
(ert-deftest piem-test-deferred-tool-cooling-nil-boundary-drain-keeps-block-hot ()
  "Real timer slices skip cooling once the hot tail covers the whole buffer."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-hot-tail-turn-count 1))
      (let ((older
             (piem-test--render-headed-completed-read-block "cover-old"))
            (newest
             (piem-test--render-headed-completed-read-block "cover-new")))
        (piem--handle-display-event '(:type "agent_end"))
        (should (equal piem--tool-cooling-queue (list older)))
        (should piem--tool-cooling-timer)
        (set-marker piem--hot-tail-start (point-min))
        (should (piem-test--drain-tool-cooling-via-real-timers))
        (should-not piem--tool-cooling-queue)
        (should-not piem--tool-cooling-timer)
        (let ((live (piem-test--all-tool-overlays)))
...
(ert-deftest piem-test-deferred-tool-cooling-real-timer-error-fails-closed ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-hot-tail-turn-count 1)
          warning)
      (dotimes (index 3)
        (piem-test--render-headed-completed-read-block
         (format "fail-%d" index)))
      (piem--handle-display-event '(:type "agent_end"))
      (should piem--tool-cooling-queue)
      (should piem--tool-cooling-timer)
      (let ((generation piem--tool-cooling-generation))
        (cl-letf (((symbol-function 'piem--cool-tool-overlay)
                   (lambda (&rest _)
...
(ert-deftest piem-test-cooled-file-target-preserves-local-authority-and-line-map ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-tool-preview-lines 3))
      ...
      (let ((hot-target (piem--file-target-at-point)))
        (should (eq :tool (plist-get hot-target :source)))
        (should (= 12 (plist-get hot-target :line)))
        (piem--cool-completed-tool-blocks
         (piem-test--all-tool-overlays))
        (should (= 0 (piem-test--count-overlays-with-prop
                      'piem-tool-block)))
        (goto-char (point-min))
        (search-forward "src/fallback.el")
        (let ((cold-target (piem--file-target-at-point)))
...
(ert-deftest piem-test-cooled-file-target-preserves-multi-hop-boundary ()
  ...
  (with-temp-buffer
    ...
    (piem--display-tool-start "read" '(:path "src/app.py"))
    (piem--display-tool-end
...
(ert-deftest piem-test-cooled-file-target-preserves-path-errors ()
  ...
  (dolist (case
    ...
    (with-temp-buffer
      (piem-chat-mode)
      (piem--set-chat-session-identity (nth 0 case))
      (let ((args (list :path (nth 1 case))))
        (piem--display-tool-start "read" args)
        (piem--display-tool-end
      ...
      (let ((hot-error
             (error-message-string
              (should-error (piem--file-target-at-point)
                            :type 'user-error))))
...
(ert-deftest piem-test-cooled-file-target-preserves-explicit-absence-after-refresh ()
  ...
  (with-temp-buffer
    ...
    (piem--display-tool-end
     "custom" '(:path "/tmp/stale.el")
     '((:type "text" :text "src/fallback.el")) nil nil)
    (let* ((overlay (car (piem-test--all-tool-overlays)))
           (block (piem--tool-block-from-overlay overlay)))
      ;; Model a later authoritative refresh that explicitly clears the path.
      (piem--tool-block-sync-path-metadata block nil)
      (goto-char (point-min))
      (search-forward "/tmp/stale.el")
      (should-not (piem--file-target-at-point))
      (search-forward "src/fallback.el")
      (should-not (piem--file-target-at-point))
      (piem--cool-completed-tool-blocks (list overlay))
      (goto-char (point-min))
      (search-forward "/tmp/stale.el")
      (should-not (piem--file-target-at-point))
      (search-forward "src/fallback.el")
...
(ert-deftest piem-test-cooled-file-target-keeps-safe-raw-display-through-fontification ()
  ...
  (with-temp-buffer
    ...
    (let ((path "/tmp/a\tb.el"))
      ...
      (goto-char (point-min))
      (search-forward "src/fallback.el")
      (let ((before (piem--file-target-at-point)))
  ...
  (with-temp-buffer
    ...
    (let* ((overlay (car (piem-test--all-tool-overlays)))
      ...
      (dolist (position (list hot-start (1- hot-end)))
        (goto-char position)
        (should (eq :tool
                    (plist-get (piem--file-target-at-point) :source))))
      ...
      (let* ((target (piem--file-target-at-point))
        ...
        (should (equal "src/outside.el"
                       (plist-get (piem--file-target-at-point) :raw)))))))

(ert-deftest piem-test-cooled-file-target-raw-copy-does-not-transfer-authority ()
  ...
  (with-temp-buffer
    ...
    (let ((start (match-beginning 0))
          ...
          (piem-copy-raw-markdown t))
      (kill-ring-save start end)
      (let ((copied (car kill-ring)))
        (should (equal "src/fallback.el" copied))
        (should-not (get-text-property
                     0 'piem-cold-tool-block copied))
        (piem--display-user-message copied)
        (goto-char (point-max))
...
(ert-deftest piem-test-cooled-file-target-history-refresh-clears-stale-authority ()
  ...
  (with-temp-buffer
    ...
    (let* ((piem-hot-tail-turn-count 0)
      ...
      (should (equal "/tmp/old.el"
                     (plist-get (piem--file-target-at-point)
                                :emacs-path)))
      (piem--rerender-canonical-history)
      (goto-char (point-min))
      (search-forward "src/fallback.el")
...
(ert-deftest piem-test-cool-completed-tool-blocks-rewrites-collapsed-write-as-preview-only-bare-fence ()
  ...
  (with-temp-buffer
    ...
    (let* ((piem-tool-preview-lines 3)
      ...
      (should (string-match-p "more lines" (buffer-string)))
      (should-not (string-match-p "\nline4\n" (buffer-string)))
      (piem--cool-completed-tool-blocks (piem-test--all-tool-overlays))
      (let ((text (buffer-string)))
        (should (string-match-p
                 (regexp-quote
...
(ert-deftest piem-test-cool-completed-tool-blocks-removes-edit-diff-overlays ()
  "Cooling a completed edit block removes diff overlays and hot overlay state."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-tool-preview-lines 10)
          (diff "+ 1     alpha\n- 2     beta\n  3     gamma"))
      (piem--display-tool-start "edit" '(:path "/tmp/example.py"))
      (piem--display-tool-end
       ...
       nil)
      (should (string-match-p "```python" (buffer-string)))
      (should (> (piem-test--count-overlays-with-prop
                  'piem-diff-overlay)
                 0))
      (piem--cool-completed-tool-blocks (piem-test--all-tool-overlays))
      (let ((text (buffer-string)))
...
(ert-deftest piem-test-cool-completed-tool-blocks-keep-plain-hint-for-collapsed-edit-preview ()
...
(ert-deftest piem-test-cool-completed-tool-blocks-does-not-invent-hint-for-expanded-read ()
  ...
  (with-temp-buffer
    ...
    (let* ((piem-tool-preview-lines 3)
           (body (string-join '("def line_one():"
                                ...
                                "    return 3")
                              "\n")))
      (piem--display-tool-start "read" '(:path "/tmp/example.py"))
      (piem--display-tool-end
...
(ert-deftest piem-test-cool-completed-tool-blocks-ignores-pending-tool-blocks ()
  "Cooling should ignore pending tool blocks that have not completed yet."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "bash" '(:command "sleep 100"))
    (let ((before (buffer-string))
          (pending piem--pending-tool-overlay))
      (piem--cool-completed-tool-blocks (piem-test--all-tool-overlays))
      (should (equal before (buffer-string)))
      (should (eq pending piem--pending-tool-overlay))
      (should (= 1 (piem-test--count-overlays-with-prop
                    'piem-tool-block))))))

(ert-deftest piem-test-cool-completed-tool-blocks-can-leave-newer-blocks-hot ()
  "Cooling can target only older completed blocks, leaving newer ones hot."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "bash" '(:command "printf old"))
    (piem--display-tool-end
     "bash" '(:command "printf old")
     '((:type "text" :text "old"))
     nil nil)
    (piem--display-tool-start "write" '(:path "/tmp/example.py" :content "print(1)"))
    (piem--display-tool-end
     "write" '(:path "/tmp/example.py" :content "print(1)" )
     '((:type "text" :text "done"))
     nil nil)
    (let* ((sorted-overlays
            (sort (seq-filter (lambda (ov) (overlay-get ov 'piem-tool-block))
                              (overlays-in (point-min) (point-max)))
                  (lambda (a b) (< (overlay-start a) (overlay-start b)))))
           (older (car sorted-overlays)))
      (should (= 2 (length sorted-overlays)))
      (piem--cool-completed-tool-blocks (list older))
      (should (= 1 (piem-test--count-overlays-with-prop
                    'piem-tool-block)))
      ;; Newer write block is still hot and still carries its typed fence.
      (should (string-match-p "```python" (buffer-string))))))

(ert-deftest piem-test-cool-completed-tool-blocks-is-idempotent ()
  "Cooling twice should leave the second pass with nothing more to do."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "read" '(:path "/tmp/test.py"))
    (piem--display-tool-end
...
(ert-deftest piem-test-agent-end-cools-tool-blocks-outside-hot-tail ()
...
With hot-tail-turn-count 1, only the most recent headed turn stays hot."
  (with-temp-buffer
    (piem-chat-mode)
    (let* ((piem-tool-preview-lines 3)
           (piem-hot-tail-turn-count 1)
           (older-content
            (concat "line1\nline2\nline3\nline4\n```python\nprint('old')\n```")))
      (piem-test--render-completed-tool-turn
       "call-1" "write"
...
(ert-deftest piem-test-session-history-cools-tool-blocks-outside-hot-tail ()
...
(ert-deftest piem-test-cooling-is-idempotent-when-hot-tail-unchanged ()
      ...
      (should (string-match-p
               (regexp-quote
                (concat "read /tmp/one.py\n"
                        ...
                        "```"))
               (buffer-string)))
      (should (= 2 (piem-test--count-overlays-with-prop
                    'piem-tool-block)))
...
(ert-deftest piem-test-agent-end-keeps-multi-tool-turn-hot ()
...
(ert-deftest piem-test-cooling-skips-live-executing-blocks ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-hot-tail-turn-count 1))
      ...
      (should (piem--tool-block-get "live-call"))
...
(ert-deftest piem-test-cooling-hot-block-toggle-works-after-cold-neighbor ()
  ...
  (with-temp-buffer
    ...
    (let* ((piem-tool-preview-lines 5)
      ...
      (let* ((hot-ov (car (piem-test--all-tool-overlays)))
             (btn (piem--find-toggle-button-in-region
                   (overlay-start hot-ov) (overlay-end hot-ov))))
        (should btn)
        (piem--toggle-tool-output btn)
        (should (string-match-p "line-20" (buffer-string)))
        ;; Collapse again
        (let* ((hot-ov2 (car (piem-test--all-tool-overlays)))
               (btn2 (piem--find-toggle-button-in-region
                      (overlay-start hot-ov2) (overlay-end hot-ov2))))
          (should btn2)
          (piem--toggle-tool-output btn2)
          (should-not (string-match-p "line-20" (buffer-string))))))))

(ert-deftest piem-test-cooling-no-buttons-in-cold-blocks ()
  ...
  (with-temp-buffer
    ...
    (let* ((piem-tool-preview-lines 5)
      ...
      (piem-test--render-completed-tool-turn
       ...
       '(:path "/tmp/new.py")
       (list (list :type "text" :text long-body)))
      (let ((boundary (marker-position piem--hot-tail-start)))
...
(ert-deftest piem-test-cooling-bash-error-cools-correctly ()
  "An errored bash tool block cools like any other completed block."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-hot-tail-turn-count 1)
          (piem-tool-preview-lines 10)
          (piem-bash-preview-lines 10))
...
(ert-deftest piem-test-cooling-write-with-fence-content ()
...
(ert-deftest piem-test-apply-diff-overlays-removed-line ()
  ...
  (with-temp-buffer
    ...
    (piem--apply-diff-overlays (point-min) (point-max))
    (goto-char (point-min))
    (let ((indicator-ovs (seq-filter (lambda (ov) (overlay-get ov 'piem-diff-overlay))
                                     (overlays-at (point)))))
      (should indicator-ovs)
      (should (memq 'diff-indicator-removed
                    (mapcar (lambda (ov) (overlay-get ov 'face)) indicator-ovs))))
    (goto-char 9)
    (let ((line-ovs (seq-filter (lambda (ov) (overlay-get ov 'piem-diff-overlay))
                                (overlays-at (point)))))
      (should line-ovs)
      (should (memq 'piem-diff-line-removed
                    (mapcar (lambda (ov) (overlay-get ov 'face)) line-ovs))))))

(ert-deftest piem-test-apply-diff-overlays-multiline ()
  ...
  (with-temp-buffer
    ...
    (piem--apply-diff-overlays (point-min) (point-max))
    ;; Count diff overlays
    (let ((all-ovs (seq-filter (lambda (ov) (overlay-get ov 'piem-diff-overlay))
                               (overlays-in (point-min) (point-max)))))
      ;; Should have 4 overlays: indicator + line for each of 2 lines
      (should (= 4 (length all-ovs))))))

(ert-deftest piem-test-apply-diff-overlays-line-background ()
  ...
  (with-temp-buffer
    ...
    (let ((ovs (seq-filter (lambda (ov) (overlay-get ov 'piem-diff-overlay))
      ...
      (should (memq 'piem-diff-line-added
                    (mapcar (lambda (ov) (overlay-get ov 'face)) ovs))))))

(ert-deftest piem-test-edit-tool-diff-uses-overlays ()
  "Edit tool output should use overlays for diff highlighting."
  (with-temp-buffer
    (piem-chat-mode)
    (setq piem--tool-args-cache (make-hash-table :test 'equal))
    (puthash "test" '(:path "/tmp/test.py") piem--tool-args-cache)
    (piem--display-tool-start "edit" '(:path "/tmp/test.py"))
    ;; Use actual pi format
    ...
    (let ((diff-ovs (seq-filter (lambda (ov) (overlay-get ov 'piem-diff-overlay))
                                (overlays-in (point-min) (point-max)))))
...
(ert-deftest piem-test-edit-tool-diff-keeps-syntax-face-under-diff-overlay ()
  "Edit diff overlays should not remove syntax fontification from code tokens."
  (let ((path (expand-file-name "piem-edit-diff-test.py"
                                temporary-file-directory)))
    (with-temp-buffer
      (piem-chat-mode)
      (piem--display-tool-start "edit" `(:path ,path))
      (piem--display-tool-end
       "edit"
       `(:path ,path)
       '((:type "text" :text "Edit successful"))
      ...
      (let* ((pos (match-beginning 0))
             ...
             (diff-faces (mapcar (lambda (ov) (overlay-get ov 'face))
                                 (seq-filter (lambda (ov)
                                               (overlay-get ov 'piem-diff-overlay))
                                             (overlays-at pos)))))
        (should syntax-face)
        (should (memq 'piem-diff-line-added diff-faces))))))

;;; File Navigation (visit-file)

(ert-deftest piem-test-file-target-tool-contract-local-anywhere-in-block ()
  ...
  (with-temp-buffer
    ...
    (let* ((ov (seq-find (lambda (overlay)
      ...
      (should (= 11 (plist-get (piem--file-target-at-point)
                               :line))))))

(ert-deftest piem-test-file-target-tool-ignores-invalid-line-offset ()
  ...
  (dolist (offset '("oops" 0 -1))
    (with-temp-buffer
      ...
      (let ((overlay (piem--tool-overlay-at-point)))
        (overlay-put overlay 'piem-tool-offset offset)
        (let ((target (piem--file-target-at-point)))
          (should (eq :tool (plist-get target :source)))
          (should (equal "src/app.py" (plist-get target :raw)))
          (should-not (plist-get target :line)))))))

(ert-deftest piem-test-file-target-tool-preserves-multi-hop-path-boundary ()
  ...
  (with-temp-buffer
    ...
    (piem--set-chat-session-identity
     "/ssh:bastion|sudo:root@pi-host:/home/pi/project/")
    (piem--display-tool-start "read" '(:path "src/app.py"))
    (goto-char (overlay-start piem--pending-tool-overlay))
    (let ((target (piem--file-target-at-point)))
      (should (equal "src/app.py" (plist-get target :raw)))
...
(ert-deftest piem-test-file-target-tool-preserves-remote-home-shell-semantics ()
  ...
  (dolist (case '(("~/a b.el" "/ssh:pi-host:~/a b.el" "~/a b.el")
    ...
    (with-temp-buffer
      ...
      (piem--display-tool-start "read" (list :path (nth 0 case)))
      (goto-char (overlay-start piem--pending-tool-overlay))
      (let ((target (piem--file-target-at-point)))
        ...
        (should (equal (piem--file-target-shell-argument target)
                       (concat (car (split-string (nth 2 case) "/")) "/"
                               (shell-quote-argument "a b.el" t))))))))

(ert-deftest piem-test-file-target-keeps-emacs-path-when-shell-conversion-fails ()
  ...
  (dolist (path '("/ssh:pi-host:/:relative"
    ...
    (with-temp-buffer
      ...
      (let ((target (piem--file-target-at-point)))
        ...
        (should-error (piem--file-target-shell-path target)
                      :type 'user-error)
        (should-error (piem--file-target-shell-argument target)
                      :type 'user-error)))))

(ert-deftest piem-test-file-shell-command-rejects-empty-command ()
  "A file operand alone never becomes the command to execute."
  (dolist (command '("" " " "\t" "&" " \t&  "))
    (should-error
     (piem--shell-command-with-file command "'/tmp/report file'")
     :type 'user-error)))

(ert-deftest piem-test-simple-shell-command-grammar-is-explicit ()
  "The no-marker whitelist accepts only one safe command plus safe options."
  (should (string-match-p "BODY := H\\* COMMAND"
                          (documentation
                           'piem--simple-shell-command-p)))
  (should (string-match-p
           "options beginning with"
           (documentation 'piem-shell-command-at-point)))
  (dolist (command '("file" " cat\t" "wc -l" "file --brief"
                     "/usr/bin/file --mime-type" "./bin/check -q"
                     "tool.name --style=short -x/y:z,1"))
    (should (piem--simple-shell-command-p command)))
  (dolist (command '("." ".." "/" "-" "--" "wc lines" "FOO=bar file"
                     "file --brief=yes extra" "file\n-l" "file -l"))
    (should-not (piem--simple-shell-command-p command))))

(ert-deftest piem-test-file-shell-command-appends-only-simple-commands ()
  ...
  (dolist (case '(("file" "file ARG")
    ...
    (should (equal (piem--shell-command-with-file
                    (car case) "ARG")
                   (cadr case)))))

(ert-deftest piem-test-file-shell-command-rejects-non-simple-auto-append ()
  ...
  (dolist (command
    ...
    (let ((err (should-error
                (piem--shell-command-with-file command "ARG")
                :type 'user-error)))
      (should (equal
               "Compound shell commands require an isolated * file placeholder"
               (error-message-string err))))))

(ert-deftest piem-test-file-shell-command-marker-is-textual-dired-style ()
  ...
  (dolist (command '("echo foo*" "echo *foo" "echo *\n" "echo\n*"
                     "echo '*x'" "echo \\*" "echo *\"\""))
    (should-error (piem--shell-command-with-file command "ARG")
                  :type 'user-error)))

(ert-deftest piem-test-file-shell-command-marker-matches-dired-boundaries ()
...
(ert-deftest piem-test-file-shell-command-native-async-classification ()
  "Only a whitespace-delimited terminal single ampersand becomes native async."
  (dolist (case '(("file &" "file ARG &")
                  ("file\t&  " "file ARG\t&  ")
                  ("file * &" "file ARG &")
                  ("cat * | wc -l &" "cat ARG | wc -l &")))
    (let ((built (piem--shell-command-with-file (car case) "ARG")))
...
(ert-deftest piem-test-file-shell-command-scans-are-linear ()
  ...
  (let* ((padding (make-string 16000 ?\s))
         ...
         (start (float-time))
         (result (piem--shell-command-with-file command "ARG")))
...
(ert-deftest piem-test-file-shell-command-keeps-hostile-path-as-data ()
  ...
  (with-temp-buffer
    ...
    (dolist (path '("/tmp/report&" "/tmp/a b;$(bad)`tick\tline\nend"
                    "/tmp/project/-danger"))
      (let* ((target (piem--make-file-target :text path path))
             (argument (piem--file-target-shell-argument target)))
        (dolist (input '("file" "printf '%s' *"))
          (let ((command (piem--shell-command-with-file
                          input argument)))
...
(ert-deftest piem-test-file-shell-command-rejection-never-runs-target ()
    ...
    (unwind-protect
        (progn
          ...
          (dolist (input '("true;" "true\n"))
            (with-temp-buffer
              (piem-chat-mode)
              (piem--set-chat-session-identity directory)
              ...
              (cl-letf (((symbol-function 'read-shell-command)
                ...
                (should-error (piem-shell-command-at-point)
                              :type 'user-error)))
            (should-not (file-exists-p sentinel))))
      (delete-directory directory t))))

(defun piem-test--run-shell-command-at-point
  ...
  (with-temp-buffer
    ...
    (let ((inhibit-read-only t) prompted executed)
      ...
      (cl-letf (((symbol-function 'read-shell-command)
        ...
        (let ((err (should-error (piem-shell-command-at-point)
                                 :type 'user-error)))
          (should (equal "No file at point" (error-message-string err)))))
      (should-not prompted)
      (should-not executed))))

(ert-deftest piem-test-shell-command-at-point-delayed-error-before-prompt ()
...
(ert-deftest piem-test-shell-command-at-point-blank-input-does-not-execute ()
...
(ert-deftest piem-test-shell-command-at-point-cancellation-does-not-execute ()
  ...
  (with-temp-buffer
    ...
    (cl-letf (((symbol-function 'read-shell-command)
      ...
      (condition-case nil
          (progn
            (piem-shell-command-at-point)
            (ert-fail "Cancellation must propagate"))
        (quit t)))))

(ert-deftest piem-test-shell-command-at-point-safe-prompt-and-cwds ()
  ...
  (with-temp-buffer
    ...
    (piem--display-tool-start "read" '(:path "reports/a\nb.el"))
    (goto-char (overlay-start piem--pending-tool-overlay))
    (let ((result (piem-test--run-shell-command-at-point "file")))
...
(ert-deftest piem-test-shell-command-at-point-preserves-buffer-shell-environment ()
...
(defvar piem-test--connection-execution-marker nil
  "Arbitrary connection-local value used by execution snapshot tests.")

(ert-deftest piem-test-shell-command-at-point-snapshot-is-narrow-locally ()
  ...
  (with-temp-buffer
    ...
    (let ((inhibit-read-only t)
      ...
      (setq-local piem-test--connection-execution-marker 'snapshot)
      (setq-local connection-local-variables-alist
                  '((piem-test--connection-execution-marker
                     . snapshot)))
      (cl-letf (((symbol-function 'read-shell-command)
                 (lambda (&rest _)
                   ...
                   (setq-local tramp-remote-process-environment
                               '("REMOTE=LATER"))
                   (setq-local piem-test--connection-execution-marker
                               'later)
                   (setq-local connection-local-variables-alist
                               '((piem-test--connection-execution-marker
                                  . later)))
      ...
      (should (equal
               '(((later-profile
                   (shell-file-name . "/later/profile-shell")))
                 ((piem-test--connection-execution-marker . later))
                 ("REMOTE=LATER") later)
               seen))
...
(defun piem-test--write-snapshot-shell (directory)
...
(defun piem-test--snapshot-shell-command (&optional async)
...
(defun piem-test--wait-for-shell-output (buffer)
...
(defun piem-test--delete-shell-buffer (buffer)
...
(ert-deftest piem-test-shell-command-at-point-real-sync-snapshots-execution ()
  ...
  (let* ((directory (file-name-as-directory
    ...
    (when output (piem-test--delete-shell-buffer output))
    (unwind-protect
        (with-temp-buffer
          (piem-chat-mode)
          (piem--set-chat-session-identity directory)
          (let ((inhibit-read-only t)
                (shell-name
                 (piem-test--write-snapshot-shell directory)))
            (insert "./target.txt")
            (goto-char (+ (point-min) 2))
            ...
            (cl-letf (((symbol-function 'read-shell-command)
                       (lambda (&rest _)
                         ...
                         (piem--set-chat-session-identity
                          later-directory)
                         (piem-test--snapshot-shell-command))))
              (piem-shell-command-at-point))
      ...
      (piem-test--delete-shell-buffer output)
      (delete-directory directory t)
      (delete-directory later-directory t))))

(ert-deftest piem-test-shell-command-at-point-real-async-ignores-output-locals ()
  "Fresh and reused native async buffers cannot replace the chat snapshot."
  (let* ((directory (file-name-as-directory
                     (make-temp-file "pi-shell-async-" t)))
         (shell-name (piem-test--write-snapshot-shell directory))
         (output-name shell-command-buffer-name-async)
         output)
    (unwind-protect
        (dolist (reused '(nil t))
          (piem-test--delete-shell-buffer (get-buffer output-name))
          (when reused
          ...
          (with-temp-buffer
            ...
            (let ((async-shell-command-display-buffer nil))
              (cl-letf (((symbol-function 'read-shell-command)
                         (lambda (&rest _)
                           (piem-test--snapshot-shell-command t))))
                (piem-shell-command-at-point))))
          (setq output (get-buffer output-name))
          (should (buffer-live-p output))
          ...
          (should (equal
                   (format "SWITCH=--pi-switch|MARK=ASYNC-%s|PWD=%s"
                           (if reused "REUSED" "FRESH")
                           (directory-file-name directory))
                   (piem-test--wait-for-shell-output output))))
      (piem-test--delete-shell-buffer output)
      (delete-directory directory t))))

(ert-deftest piem-test-shell-command-at-point-native-async-revert-keeps-snapshot ()
...
(ert-deftest piem-test-shell-command-at-point-native-async-snapshots-terminal-environment ()
...
(ert-deftest piem-test-shell-command-at-point-native-async-snapshots-process-controls ()
...
(ert-deftest piem-test-snapshotted-local-async-uses-local-process-api ()
  "The no-handler async branch uses native's local process API."
  (let* ((directory (file-name-as-directory
                     (make-temp-file "pi-shell-local-api-" t)))
         (snapshot (piem--shell-execution-snapshot directory))
...
(ert-deftest piem-test-snapshotted-async-native-display-actions ()
          ...
          (let ((shell-command-buffer-name-async buffer-name)
            ...
            (cl-letf (((symbol-function 'display-buffer)
              ...
              (should (equal "display-action"
                             (piem-test--wait-for-shell-output
                              output))))
...
(ert-deftest piem-test-shell-command-at-point-native-async-conflict-snapshot ()
        ...
        (with-temp-buffer
          ...
          (should (equal
                   (format "SWITCH=--pi-switch|MARK=ASYNC-CONFLICT|PWD=%s"
                           (directory-file-name directory))
                   (piem-test--wait-for-shell-output generated))))
      (when (process-live-p occupier) (delete-process occupier))
      (piem-test--delete-shell-buffer generated)
      (piem-test--delete-shell-buffer base)
      (delete-directory directory t))))

(ert-deftest piem-test-shell-command-at-point-remote-async-terminal-snapshot ()
  ...
  (with-temp-buffer
    ...
    (let ((directory "/ssh:host:/srv/project/")
          ...
          observed)
      (piem--set-chat-session-identity directory)
      (piem--display-tool-start "read" '(:path "reports/a.el"))
      (goto-char (overlay-start piem--pending-tool-overlay))
      (setq-local comint-terminfo-terminal "SNAPSHOT-TERM")
...
(ert-deftest piem-test-shell-command-at-point-copies-cwd-snapshot ()
  ...
  (with-temp-buffer
    ...
    (let* ((directory (copy-sequence "/tmp/snapshot/"))
           ...
           observed)
      (cl-letf (((symbol-function 'piem--file-target-at-point)
                 (lambda () target))
...
(ert-deftest piem-test-shell-command-at-point-native-multihop-dispatch ()
...
(ert-deftest piem-test-shell-command-at-point-builds-local-target-forms ()
  ...
  (dolist (case `(("src/app.el" 2 "/tmp/project/src/app.el")
    ...
    (with-temp-buffer
      ...
      (let ((result (piem-test--run-shell-command-at-point
                     "printf '%s' *")))
        (should (equal (concat "printf '%s' "
                               (shell-quote-argument (nth 2 case)))
                       (plist-get result :command)))))))

(ert-deftest piem-test-shell-command-at-point-excludes-text-locations ()
...
(ert-deftest piem-test-shell-command-at-point-link-uses-path-not-label-or-fragment ()
...
(ert-deftest piem-test-shell-command-at-point-remote-target-forms ()
  ...
  (dolist
    ...
    (with-temp-buffer
      (piem-chat-mode)
      (piem--set-chat-session-identity (nth 0 case))
      (piem--display-tool-start "read" (list :path (nth 1 case)))
      (goto-char (overlay-start piem--pending-tool-overlay))
      (let ((result (piem-test--run-shell-command-at-point "file *")))
...
(ert-deftest piem-test-shell-command-at-point-remote-errors-stay-controlled ()
  ...
  (dolist (path '("/ssh:other:/tmp/app.el"
    ...
    (with-temp-buffer
      (piem-chat-mode)
      (piem--set-chat-session-identity "/ssh:host:/srv/project/")
      (piem--display-tool-start "read" (list :path path))
      (goto-char (overlay-start piem--pending-tool-overlay))
      (cl-letf (((symbol-function 'read-shell-command)
...
(ert-deftest piem-test-shell-command-at-point-prefix-is-ignored ()
...
(ert-deftest piem-test-shell-command-at-point-snapshots-before-prompt ()
  ...
  (with-temp-buffer
    ...
    (goto-char (+ (point-min) 2))
    (let* ((chat-buffer (current-buffer))
           (resolver (symbol-function 'piem--file-target-at-point))
           (resolve-count 0)
           result)
      (cl-letf (((symbol-function 'piem--file-target-at-point)
                 (lambda ()
                   ...
                   (funcall resolver))))
        (setq result
              (piem-test--run-shell-command-at-point
               "file *"
               (lambda ()
...
(ert-deftest piem-test-shell-command-at-point-tool-authority-hot-and-cold ()
  "Hot and cold tool targets work anywhere; absent/invalid metadata stays final."
  (dolist (cold '(nil t))
    (with-temp-buffer
      (piem-chat-mode)
      (piem--set-chat-session-identity "/tmp/project/")
      (piem--display-tool-start "read" '(:path "src/tool.el"))
      (piem--display-tool-end
        ...
        (let ((positions (list header-position (1- (point)))))
          (dolist (position positions)
            ...
            (should (equal "file /tmp/project/src/tool.el"
                           (plist-get
                            (piem-test--run-shell-command-at-point "file")
                            :command))))))))
  (dolist (cold '(nil t))
    (with-temp-buffer
      (piem-chat-mode)
      (piem--set-chat-session-identity "/tmp/project/")
      (piem--display-tool-start "read" '(:offset 1))
      (piem--display-tool-end
       "read" '(:offset 1)
       '((:type "text" :text "src/body-fallback.el")) nil nil)
      (let ((overlay (car (piem-test--all-tool-overlays))))
...
(ert-deftest piem-test-shell-command-at-point-preserves-parser-errors ()
  "Authoritative semantic parser failures keep their controlled condition."
  (with-temp-buffer
    (piem-chat-mode)
    (cl-letf (((symbol-function 'piem--file-target-at-point)
               (lambda ()
                 (signal 'piem-semantic-link-parser-error '("boom"))))
              ((symbol-function 'read-shell-command)
               (lambda (&rest _) (ert-fail "Parser errors precede prompting"))))
      (should-error (piem-shell-command-at-point)
                    :type 'piem-semantic-link-parser-error))))

(ert-deftest piem-test-shell-command-at-point-leaves-pi-state-unchanged ()
  ...
  (with-temp-buffer
    ...
    (let* ((streaming-marker (copy-marker (point-max) t))
           ...
           (piem--streaming-marker streaming-marker)
           (piem--process 'fake-process)
           (piem--session-transition-generation 7)
           (piem--session-transition-active nil)
           (piem--tool-args-cache tool-cache)
           (piem--live-tool-blocks live-tools)
           (piem--pending-tool-overlay 'tool-snapshot)
           (piem--followup-queue '("newer" "older"))
           (before (list piem--status
                         piem--streaming-marker
                         piem--process
                         piem--session-transition-generation
                         piem--session-transition-active
                         piem--tool-args-cache
                         piem--live-tool-blocks
                         piem--pending-tool-overlay
                         (copy-sequence piem--followup-queue))))
      (should (piem--session-busy-p))
      (piem-test--run-shell-command-at-point "file")
      (should (piem--session-busy-p))
      (should (equal before
                     (list piem--status
...
(ert-deftest piem-test-file-target-does-not-use-pi-rpc-path-boundary ()
  ...
  (with-temp-buffer
    ...
    (let ((inhibit-read-only t))
      (insert "src/app.py"))
    (goto-char (+ (point-min) 3))
    (cl-letf (((symbol-function 'piem--process-local-path)
               (lambda (&rest _)
...
(ert-deftest piem-test-file-target-tool-invalid-path-wins-over-text ()
  ...
  (with-temp-buffer
    ...
    (let ((path "/ssh:127.0.0.1:/tmp/project/src/bad.txt"))
      ...
      (goto-char (point-min))
      (search-forward "src/fallback.el")
      (let ((err (should-error (piem--file-target-at-point)
                               :type 'user-error)))
        (should (string-match-p "not on this session host"
                                (error-message-string err)))))))

(ert-deftest piem-test-file-target-tool-without-path-wins-over-text ()
  "Absent tool metadata returns nil instead of using tool-body text."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "custom" nil)
    (piem--display-tool-end
...
(ert-deftest piem-test-file-target-tool-rejects-nul-and-non-string-paths ()
  ...
  (dolist (case (list (list (concat "/tmp/bad" (string ?\0) "name.el") "NUL")
    ...
    (with-temp-buffer
      (piem-chat-mode)
      (piem--display-tool-start "read" (list :path (car case)))
      (goto-char (overlay-start piem--pending-tool-overlay))
      (let ((err (should-error (piem--file-target-at-point)
                               :type 'user-error)))
        (should (string-match-p (cadr case) (error-message-string err)))))))

(ert-deftest piem-test-file-target-tool-display-is-safe-and-lookup-is-passive ()
  ...
  (with-temp-buffer
    ...
    (let ((path "/tmp/a\tb.el"))
      ...
      (let ((tick (buffer-chars-modified-tick))
            ...
            (target (piem--file-target-at-point)))
...
(ert-deftest piem-test-file-target-text-contract-and-session-anchor ()
  "Plain targets expose locations and use the canonical session directory."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--set-chat-session-identity "/tmp/session/")
    (setq default-directory "/tmp/accidental/")
    (let ((inhibit-read-only t))
...
(ert-deftest piem-test-file-target-text-accepts-diagnostic-separator ()
...
(ert-deftest piem-test-file-target-text-diagnostic-separator-maps-visible-markdown ()
...
(ert-deftest piem-test-file-target-text-diagnostic-context-is-visible ()
...
(ert-deftest piem-test-file-target-text-rejects-ambiguous-diagnostic-colons ()
...
(ert-deftest piem-test-file-target-text-accepts-strict-path-forms ()
...
(ert-deftest piem-test-file-target-text-quotes-and-location-suffixes ()
...
(ert-deftest piem-test-file-target-text-resolves-fontified-emphasis ()
...
(ert-deftest piem-test-file-target-link-multiline-local-host ()
...
(ert-deftest piem-test-file-target-link-multiline-external-owns-label ()
  ...
  (dolist (fontified '(nil t))
    (with-temp-buffer
      ...
      (should (eq :owned-invalid
                  (plist-get
                   (piem--semantic-link-file-target-at-point)
                   :status)))
      (should-not (piem--file-target-at-point)))))

(ert-deftest piem-test-file-target-link-oversized-label-local-destination ()
...
(ert-deftest piem-test-file-target-link-oversized-label-url-owns-path-text ()
...
(ert-deftest piem-test-file-target-link-fake-syntax-in-multiline-contexts ()
...
(ert-deftest piem-test-file-target-link-exact-cap-host-is-parseable ()
  "A complete host exactly at 262,144 characters is not treated as over-cap."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((inhibit-read-only t))
      (insert (make-string
               piem--max-semantic-link-host-length ?x)))
    (goto-char (+ (point-min)
                  (/ piem--max-semantic-link-host-length 2)))
    (let (parsed-host)
      (cl-letf (((symbol-function
        ...
        (should (eq :not-a-link
                    (plist-get
                     (piem--semantic-link-file-target-at-point)
                     :status))))
      (should (= piem--max-semantic-link-host-length
                 (- (plist-get parsed-host :end)
                    (plist-get parsed-host :start)))))))

(ert-deftest piem-test-file-target-link-over-cap-host-fails-closed ()
  ...
  (with-temp-buffer
    ...
    (search-forward "src/fallback.el")
    (goto-char (match-beginning 0))
    (cl-letf (((symbol-function 'piem--semantic-link-captures)
               (lambda (&rest _)
                 (ert-fail "Over-cap host must not reach inline parsing")))
              ((symbol-function 'piem--text-file-target-at-point)
               (lambda ()
...
(ert-deftest piem-test-file-target-link-malformed-scanner-balances-destinations ()
...
(ert-deftest piem-test-file-target-link-malformed-scanner-work-is-linear ()
  "Many incomplete shortcuts consume only linear malformed-scan work."
  (let* ((count 600)
         (source (apply #'concat (make-list count "[a](")))
         (scanned 0)
         (original
          (symbol-function 'piem--semantic-link-malformed-end)))
    (with-temp-buffer
      (piem-chat-mode)
      (let ((inhibit-read-only t)) (insert source))
...
(ert-deftest piem-test-file-target-link-malformed-image-owns-tail ()
...
(ert-deftest piem-test-file-target-link-reference-image-does-not-own-following-parens ()
...
(ert-deftest piem-test-file-target-link-malformed-scanner-does-not-over-own ()
...
(ert-deftest piem-test-file-target-link-captures-are-detached ()
  ...
  (with-temp-buffer
    ...
    (goto-char (+ (point-min) 3))
    (let* ((host (piem--semantic-link-host-at-point))
           (captures (piem--semantic-link-captures
...
(ert-deftest piem-test-file-target-link-malformed-angle-stops-at-line-end ()
...
(ert-deftest piem-test-file-target-link-malformed-title-honors-escapes ()
...
(ert-deftest piem-test-file-target-link-malformed-scanner-angle-and-title ()
...
(ert-deftest piem-test-file-target-link-streamed-escaped-close-completes ()
...
(ert-deftest piem-test-file-target-link-malformed-true-boundary-points ()
...
(ert-deftest piem-test-file-target-link-table-cell-host ()
...
(defun piem-test--insert-semantic-link-variant (text variant)
  ...
  (pcase variant
    ...
    ('streamed
     ...
     (let ((middle (/ (length text) 2)))
       (piem--display-message-delta (substring text 0 middle))
       (piem--display-message-delta (substring text middle))))
    ('reloaded
     (piem--display-history-messages
      (vector (list :role "assistant" :content text
                    :timestamp 1704067200000))))))

(ert-deftest piem-test-file-target-link-nested-label-projection ()
...
(ert-deftest piem-test-file-target-link-nested-label-is-control-safe ()
...
(ert-deftest piem-test-file-target-link-nested-image-owner-precedence ()
  ...
  (dolist (variant '(raw fontified streamed reloaded))
    (dolist (case '(("[![Alt](images/inner.png)](docs/outer.md)"
      ...
      (with-temp-buffer
        (piem-chat-mode)
        (piem--set-chat-session-identity "/tmp/session/")
        (piem-test--insert-semantic-link-variant (car case) variant)
        (goto-char (point-min))
        (search-forward "Alt")
        (goto-char (match-beginning 0))
        (if (nth 1 case)
...
(ert-deftest piem-test-file-target-link-nested-reference-projection ()
...
(ert-deftest piem-test-file-target-link-valid-outer-owns-malformed-child ()
...
(ert-deftest piem-test-file-target-link-malformed-child-stays-visible ()
  ...
  (let ((source "[![Alt](bad destination.md)](docs/outer.md)")
    ...
    (dolist (variant '(raw fontified streamed reloaded))
      (with-temp-buffer
        (piem-chat-mode)
        (piem--set-chat-session-identity "/tmp/session/")
        (piem-test--insert-semantic-link-variant source variant)
...
(ert-deftest piem-test-file-target-link-unresolved-nested-shortcut ()
...
(ert-deftest piem-test-file-target-link-shortcut-recovery-respects-code-spans ()
...
(ert-deftest piem-test-file-target-link-shortcut-recovery-respects-html ()
...
(ert-deftest piem-test-file-target-link-html-label-markup-is-inert ()
...
(ert-deftest piem-test-file-target-link-shortcut-recovery-masks-malformed-child ()
...
(ert-deftest piem-test-file-target-link-recovery-respects-escaped-image-marker ()
  ...
  (dolist (fontified '(nil t))
    (with-temp-buffer
      ...
      (should (equal "docs/in.md"
                     (plist-get
                      (piem--file-target-at-point) :raw)))
      (goto-char (point-min))
      (search-forward "b")
      (goto-char (match-beginning 0))
      (should-not (piem--file-target-at-point)))))

(ert-deftest piem-test-file-target-link-recovery-keeps-valid-inner-link ()
...
(ert-deftest piem-test-file-target-link-recovery-keeps-reference-inner ()
...
(ert-deftest piem-test-file-target-link-recovery-ignores-dest-brackets ()
...
(ert-deftest piem-test-file-target-image-recovery-owns-nested-link ()
...
(ert-deftest piem-test-file-target-link-shortcut-stack-is-linear ()
...
(ert-deftest piem-test-file-target-link-shortcut-recovery-is-linear ()
...
(ert-deftest piem-test-file-target-link-distinct-recovery-is-linear ()
...
(ert-deftest piem-test-file-target-link-deep-nested-label-is-bounded ()
...
(ert-deftest piem-test-file-target-link-shortcut-outer-owns-nested-image ()
...
(ert-deftest piem-test-file-target-link-reference-outer-owns-nested-image ()
...
(ert-deftest piem-test-file-target-image-owns-nested-label-constructs ()
...
(ert-deftest piem-test-file-target-link-nested-image-hidden-source-is-inert ()
...
(ert-deftest piem-test-file-target-link-exact-real-case ()
...
(ert-deftest piem-test-file-target-link-destination-owns-visible-label ()
...
(ert-deftest piem-test-file-target-link-image-label-uses-destination ()
...
(ert-deftest piem-test-file-target-link-angle-space-and-fragment-contract ()
...
(ert-deftest piem-test-file-target-link-empty-label-is-inert ()
...
(ert-deftest piem-test-file-target-link-non-file-destinations-own-label ()
...
(ert-deftest piem-test-file-target-link-malformed-inline-owns-source ()
...
(ert-deftest piem-test-file-target-link-streaming-becomes-complete ()
...
(ert-deftest piem-test-file-target-link-shortcut-keeps-strict-text-contract ()
...
(ert-deftest piem-test-file-target-link-preserves-remote-boundaries ()
...
(ert-deftest piem-test-file-target-link-tri-state-is-explicit ()
...
(ert-deftest piem-test-file-target-link-selects-canonical-host-parser ()
...
(ert-deftest piem-test-file-target-link-requires-trustworthy-host-parser ()
...
(ert-deftest piem-test-file-target-link-parser-failures-fail-closed ()
...
(ert-deftest piem-test-file-target-link-respects-markdown-host-context ()
...
(ert-deftest piem-test-file-target-link-decodes-supported-escape ()
...
(ert-deftest piem-test-file-target-link-stays-below-hot-and-cold-tools ()
  ...
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (search-forward "Report")
    (let ((hot (piem--file-target-at-point)))
...
(ert-deftest piem-test-file-target-link-lookup-cleans-host-endpoints ()
  ...
  (dolist (position '(start end))
    (with-temp-buffer
      ...
      (let ((before (piem-test--semantic-parser-state)))
        (dotimes (_ 2)
          (piem--semantic-link-file-target-at-point)
          (should (equal before
                         (piem-test--semantic-parser-state))))))))

(ert-deftest piem-test-file-target-link-preserves-preexisting-local-parser ()
...
(ert-deftest piem-test-file-target-link-preserves-stale-local-parser ()
  ...
  (with-temp-buffer
    ...
    (let ((before (piem-test--semantic-parser-state))
          (original-captures
           (symbol-function 'piem--semantic-link-captures)))
...
(ert-deftest piem-test-file-target-link-uses-emacs-29-parser-api ()
...
(ert-deftest piem-test-file-target-link-preserves-foreign-new-parser ()
...
(ert-deftest piem-test-file-target-link-preserves-incomplete-parser-overlay ()
  ...
  (with-temp-buffer
    ...
    (let* ((parser (treesit-parser-create 'markdown-inline nil t))
      ...
      (unwind-protect
        ...
        (when (memq parser (piem-test--all-treesit-parsers))
          (treesit-parser-delete parser))))))

(ert-deftest piem-test-file-target-link-disables-local-range-updater ()
...
(ert-deftest piem-test-file-target-link-preserves-foreign-inline-overlay ()
                       ...
                       (lambda (&rest arguments)
                         (unless foreign-parser
                           ...
                           (overlay-put
                            ...
                            (piem--semantic-link-markdown-host-parser))
                           (overlay-put foreign-overlay
...
(ert-deftest piem-test-file-target-link-avoids-cleanup-classification ()
  "Cleanup uses registered identities without fallible metadata rescanning."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((inhibit-read-only t)) (insert "[Report](reports/out.md)"))
    (goto-char (+ (point-min) 3))
    (let ((before-parsers (piem-test--all-treesit-parsers))
          (before-overlays (overlay-lists))
          (original-captures
           (symbol-function 'piem--semantic-link-captures))
          (original-cleanup
           (symbol-function
            'piem--semantic-link-cleanup-parser-state))
...
(ert-deftest piem-test-file-target-link-creates-no-local-parser-state ()
...
(ert-deftest piem-test-file-target-link-retries-resolver-parser-cleanup ()
  ...
  (with-temp-buffer
    ...
    (let ((before (treesit-parser-list))
      ...
      (cl-letf (((symbol-function 'treesit-parser-delete)
        ...
        (should-error (piem--semantic-link-file-target-at-point)
                      :type
                      'piem-semantic-link-parser-error))
...
(ert-deftest piem-test-file-target-link-cleanup-error-restores-overlay ()
...
(ert-deftest piem-test-file-target-link-widens-for-complete-host ()
...
(ert-deftest piem-test-file-target-link-lookup-is-passive ()
  ...
  (with-temp-buffer
    ...
    (let ((tick (buffer-chars-modified-tick))
      ...
      (cl-letf (((symbol-function 'file-exists-p)
        ...
        (should (piem--file-target-at-point)))
      (should (= tick (buffer-chars-modified-tick)))
...
(ert-deftest piem-test-file-target-text-maps-inline-hidden-markup ()
...
(ert-deftest piem-test-file-target-text-fontified-code-stays-strict ()
...
(ert-deftest piem-test-file-target-text-separate-quoted-spans-do-not-cross ()
...
(ert-deftest piem-test-file-target-text-overlong-wrapper-stays-authoritative ()
...
(ert-deftest piem-test-file-target-text-location-suffix-is-case-sensitive ()
    ...
    (let ((case-fold-search fold))
      (should (equal '(12 . 20)
                     (plist-get
                      ...
                      :range)))
      (should-not (piem--parse-text-file-candidate
                   "src/foo.el#l12-l20" nil 0 20)))))

(ert-deftest piem-test-file-target-text-parses-only-local-candidate ()
  ...
  (let* ((text "src/first.el prose src/here.el tail src/last.el")
         ...
         (original (symbol-function
                    'piem--parse-text-file-candidate))
         parsed)
    (cl-letf (((symbol-function 'piem--parse-text-file-candidate)
...
(ert-deftest piem-test-file-target-text-overlong-candidate-is-controlled-nil ()
...
(ert-deftest piem-test-file-target-text-short-target-on-huge-line-is-bounded ()
...
(ert-deftest piem-test-file-target-text-fontified-path-on-huge-line-is-bounded ()
...
(ert-deftest piem-test-file-target-text-visible-clipped-edge-is-rejected ()
...
(ert-deftest piem-test-file-target-text-max-quoted-candidate-keeps-wrapper-context ()
...
(ert-deftest piem-test-file-target-text-unmatched-backtick-does-not-poison-local-wrapper ()
...
(ert-deftest piem-test-file-target-text-single-quotes-have-word-boundaries ()
...
(ert-deftest piem-test-file-target-text-rejects-quoted-prose-and-commands ()
...
(ert-deftest piem-test-file-target-text-rejects-html-closing-tags ()
...
(ert-deftest piem-test-file-target-text-strips-wrappers-and-punctuation ()
...
(ert-deftest piem-test-file-target-text-is-current-line-and-at-point-only ()
...
(ert-deftest piem-test-file-target-text-point-boundaries-are-exact ()
...
(ert-deftest piem-test-file-target-text-rejects-non-path-grammar ()
...
(ert-deftest piem-test-file-target-text-preserves-multi-hop-boundary ()
...
(ert-deftest piem-test-file-target-text-lookup-is-passive ()
...
(defun piem-test--call-tool-target-preserving-narrowing
...
(defun piem-test--tool-row-positions (body-regexp)
...
(ert-deftest piem-test-file-target-narrowed-hot-read-uses-physical-lines ()
...
(ert-deftest piem-test-file-target-narrowed-cold-read-uses-physical-lines ()
...
(ert-deftest piem-test-file-target-narrowed-hot-cold-edit-parity ()
  "Hot and cold edit rows retain physical diff lines under narrowing."
  (dolist (cooled '(nil t))
    (with-temp-buffer
      (piem-chat-mode)
      (piem--display-tool-start "edit" '(:path "/tmp/edit.el"))
      (piem--display-tool-end
...
(ert-deftest piem-test-file-target-narrowed-tool-authority-and-errors ()
    ...
    (dolist (state '(:absent :invalid))
      (with-temp-buffer
        ...
        (let* ((overlay (car (piem-test--all-tool-overlays)))
          ...
          (if (eq state :absent)
              (piem--tool-block-sync-path-metadata block nil)
            (overlay-put overlay 'piem-tool-path nil)
            (overlay-put overlay 'piem-tool-path-error message))
          (when cooled
            (piem--cool-completed-tool-blocks (list overlay)))
...
(ert-deftest piem-test-file-target-narrowed-non-content-is-authoritative ()
...
(ert-deftest piem-test-visit-file-tool-location-validation-is-authoritative ()
  ...
  (dolist (line '(nil 0 -1 1.5 "1" (:malformed)))
    (with-temp-buffer
      ...
      (let ((text (buffer-substring (point-min) (point-max)))
            ...
            (origin (point))
            (status piem--status)
            (session (piem--chat-session-directory)))
        (cl-letf (((symbol-function 'piem--tool-line-at-point)
                   (lambda (_) line))
                  ((symbol-function 'find-file)
                   (lambda (&rest _) (ert-fail "Invalid tool line opened")))
                  ((symbol-function 'find-file-other-window)
                   (lambda (&rest _) (ert-fail "Invalid tool line opened"))))
          (let ((err (should-error (piem-visit-file)
                                   :type 'user-error)))
            (should (equal "No file line at point"
                           (error-message-string err)))))
        ...
        (should (= tick (buffer-chars-modified-tick)))
        (should (= origin (point)))
        (should (eq status piem--status))
        (should (equal session (piem--chat-session-directory)))))))

(ert-deftest piem-test-visit-file-collapsed-map-invalid-lines-fail-closed ()
    ...
    (with-temp-buffer
      ...
      (let ((piem-tool-preview-lines 2))
        ...
        (piem--display-tool-end
         ...
         nil nil))
      (let ((overlay (car (piem-test--all-tool-overlays))))
        (overlay-put overlay 'piem-line-map (vector mapped-line)))
      (goto-char (point-min))
...
(ert-deftest piem-test-visit-file-nontool-location-validation ()
...
(ert-deftest piem-test-visit-file-tool-line-one-remains-valid ()
...
(defun piem-test--open-target-buffer (line-count)
  "Create and return a fake visited buffer with LINE-COUNT lines."
  (set-buffer (get-buffer-create "*piem-test-target*"))
  (setq buffer-file-name "/tmp/piem-test-target")
  (erase-buffer)
...
(defun piem-test--visit-file-line (&optional line-count toggle)
  "Call `piem-visit-file' and return visit metadata.
Returns plist `(:path PATH :line N :open-kind KIND)'.
LINE-COUNT controls the size of the fake visited file.  TOGGLE is
forwarded to `piem-visit-file'."
        ...
        (progn
          (cl-labels ((open-other (path)
                        (setq opened-path path
                              open-kind :other)
                        (piem-test--open-target-buffer line-count))
                      (open-same (path)
                        (setq opened-path path
                              open-kind :same)
                        (piem-test--open-target-buffer line-count)))
            (cl-letf (((symbol-function 'find-file-other-window) #'open-other)
              ...
              (piem-visit-file toggle)))
          (list :path opened-path
                :line (line-number-at-pos)
                :open-kind open-kind))
      (ignore-errors (kill-buffer "*piem-test-target*")))))

(defun piem-test--visit-file-state
...
`piem-visit-file'.  Return the opened path and resulting buffer
state."
  (let ((target-name "*piem-test-visit-target*")
        opened-path state)
    (unwind-protect
        (save-current-buffer
          (cl-labels
...
(defun piem-test--call-with-native-visit-layout (contents function)
...
fixture; a test may dynamically override it inside FUNCTION."
  (let* ((path (make-temp-file "piem-native-visit-" nil ".el"
                                contents))
         (chat (generate-new-buffer " *piem-native-chat*"))
         (input (generate-new-buffer " *piem-native-input*"))
    ...
    (unwind-protect
        (save-window-excursion
          (with-current-buffer chat
            ...
            (setq piem--input-buffer input)
            (let ((inhibit-read-only t))
              (insert path)))
          (with-current-buffer input
            (piem-input-mode)
            (setq piem--chat-buffer chat))
          (delete-other-windows)
...
(ert-deftest piem-test-visit-file-cold-read-keeps-mapped-line ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-tool-preview-lines 3))
      ...
      (goto-char (point-min))
      (search-forward "src/fallback.el")
      (let ((result (piem-test--visit-file-line 30)))
        (should (equal "/tmp/project/src/app.py" (plist-get result :path)))
        (should (= 12 (plist-get result :line)))))))

(ert-deftest piem-test-visit-file-cold-edit-keeps-diff-line ()
...
(ert-deftest piem-test-visit-file-cold-tool-non-content-keeps-line-error ()
...
(ert-deftest piem-test-visit-file-hot-and-cold-tool-errors-stay-authoritative ()
...
(ert-deftest piem-test-visit-file-resolves-authoritative-tool-once ()
  ...
  (with-temp-buffer
    ...
    (let ((calls 0)
      ...
      (cl-letf (((symbol-function 'piem--file-target-at-point)
        ...
        (let ((result (piem-test--visit-file-line 10)))
          (should (= 1 calls))
          (should (equal "/tmp/tool.el" (plist-get result :path)))
          (should (= 3 (plist-get result :line))))))))

(ert-deftest piem-test-visit-file-non-tool-strict-plain-paths ()
  ...
  (dolist (case `(("src/app.el" . "/tmp/session/src/app.el")
    ...
    (with-temp-buffer
      ...
      (let ((inhibit-read-only t))
        (insert (car case)))
      (goto-char (+ (point-min) 2))
      (let ((state (piem-test--visit-file-state "native" 4)))
        (should (equal (cdr case) (plist-get state :path)))
        (should (= 4 (plist-get state :point)))))))

(ert-deftest piem-test-visit-file-non-tool-no-location-keeps-native-point ()
  "A no-location target preserves native point in an existing file buffer."
  (let* ((path (make-temp-file "piem-visit-" nil ".el"
                                "alpha\nbeta\ngamma\n"))
        ...
        (progn
          ...
          (with-temp-buffer
            ...
            (let ((inhibit-read-only t))
              (insert path))
            (goto-char (+ (point-min) 2))
            (let ((piem-visit-file-other-window nil))
              (piem-visit-file))
...
(ert-deftest piem-test-visit-file-non-tool-location-forms ()
    ...
    (with-temp-buffer
      ...
      (if (string-match-p "warning" (car case))
          (progn
            ...
            (goto-char (match-beginning 0)))
        (goto-char (+ (point-min) 3)))
      (let ((state (piem-test--visit-file-state
                    "first\nabcdef\nthird\n" 1)))
...
(ert-deftest piem-test-visit-file-non-tool-point-boundaries ()
    ...
    (dolist (position (list target-start target-end))
      (with-temp-buffer
        ...
        (let ((inhibit-read-only t))
          (insert text))
        (goto-char position)
        (let ((state (piem-test--visit-file-state "native" 3)))
          (should (equal "/tmp/session/src/foo.el"
                         (plist-get state :path))))))
...
(ert-deftest piem-test-visit-file-non-tool-semantic-labels-and-fragment ()
...
(ert-deftest piem-test-visit-file-non-tool-semantic-invalid-is-final ()
...
(ert-deftest piem-test-visit-file-non-tool-ignores-shell-only-error ()
...
(ert-deftest piem-test-visit-file-non-tool-propagates-parser-error ()
  ...
  (with-temp-buffer
    ...
    (cl-letf (((symbol-function
              ...
              ((symbol-function 'find-file-other-window)
               (lambda (&rest _) (ert-fail "Parser error opened a file"))))
      (should-error (piem-visit-file)
                    :type 'piem-semantic-link-parser-error))))

(ert-deftest piem-test-visit-file-non-tool-passes-multi-hop-path ()
  ...
  (with-temp-buffer
    ...
    (let ((inhibit-read-only t))
      (insert "src/app.el"))
    (goto-char (+ (point-min) 3))
    (let ((state (piem-test--visit-file-state "native" 2)))
      (should
       (equal
        "/ssh:bastion|sudo:root@pi-host:/home/pi/project/src/app.el"
        (plist-get state :path))))))

(ert-deftest piem-test-visit-file-non-tool-delegates-missing-file ()
  "RET performs no existence preflight and preserves the opener's failure."
  (let ((path "/tmp/piem-definitely-missing.el")
        opened)
    (with-temp-buffer
      (piem-chat-mode)
      (let ((inhibit-read-only t))
...
(ert-deftest piem-test-visit-file-non-tool-is-passive-until-open ()
  ...
  (with-temp-buffer
    ...
    (let* ((chat (current-buffer))
           ...
           (modified (buffer-modified-p))
           (overlays (overlays-in (point-min) (point-max)))
           (parser-state (piem-test--semantic-parser-state))
           (tool-cache (make-hash-table :test #'equal))
           (live-tools (make-hash-table :test #'equal))
           (streaming-marker (copy-marker (point-max) t))
           (piem--status 'streaming)
           (piem--streaming-marker streaming-marker)
           (piem--process 'process-snapshot)
           (piem--session-transition-generation 9)
           (piem--session-transition-active nil)
           (piem--tool-args-cache tool-cache)
           (piem--live-tool-blocks live-tools)
           (piem--pending-tool-overlay 'tool-snapshot)
           ...
           (pi-state
            (list piem--status
                  ...
                  (piem--chat-session-directory)))
           (resolver
            (symbol-function 'piem--file-target-at-point))
           (target-buffer
            (get-buffer-create "*piem-test-passive-visit*"))
           (resolve-count 0)
           opened)
      (unwind-protect
          (let ((piem-visit-file-other-window nil))
...
(ert-deftest piem-test-visit-file-window-choice-matrix ()
      ...
      (with-temp-buffer
        ...
        (let ((target '(:source :text :emacs-path "/tmp/unit4.el"))
          ...
          (cl-letf (((symbol-function 'piem--file-target-at-point)
            ...
            (let ((piem-visit-file-other-window option)
                  (current-prefix-arg toggle))
              (call-interactively #'piem-visit-file)
              (should (eq option
                          piem-visit-file-other-window))))
          (should (= 1 resolve-count))
          (should (equal (list (list expected "/tmp/unit4.el")) calls)))))))

(ert-deftest piem-test-visit-file-native-same-window-replaces-chat ()
  "Native same-window visiting replaces the selected chat window only."
  (piem-test--call-with-native-visit-layout
   "one\ntwo\nthree\n"
   (lambda (layout)
...
(ert-deftest piem-test-visit-file-native-other-window-creates-window ()
  ...
  (piem-test--call-with-native-visit-layout
   ...
   (lambda (layout)
     (let* ((chat (plist-get layout :chat))
            ...
            (path (plist-get layout :path))
            (window-count (length (window-list))))
       (let ((piem-visit-file-other-window t))
         (piem-visit-file))
...
(ert-deftest piem-test-visit-file-native-other-window-reuses-target ()
...
(ert-deftest piem-test-visit-file-native-directory-location-is-inapplicable ()
  "Directory coordinates do not change the point and marks native Dired chose."
  (require 'dired)
  (dolist (other-window '(nil t))
    (let* ((directory (make-temp-file "piem-directory-" t))
            ...
            (save-window-excursion
              ...
              (unwind-protect
                  (cl-letf (((symbol-function
                    ...
                    (let ((piem-visit-file-other-window
                           other-window))
                      (piem-visit-file))
...
(ert-deftest piem-test-visit-file-native-narrowed-reuses-displayed-window ()
...
(ert-deftest piem-test-visit-file-native-respects-display-policy ()
...
(ert-deftest piem-test-visit-file-native-narrowed-physical-locations ()
...
(ert-deftest piem-test-visit-file-native-narrowed-no-location-is-native ()
...
(ert-deftest piem-test-visit-file-native-narrowed-respects-no-widen ()
...
(ert-deftest piem-test-visit-file-native-existing-region-contract ()
...
(ert-deftest piem-test-visit-file-tramp-opener-boundary ()
      ...
      (with-temp-buffer
        ...
        (let ((target (list :source :text :emacs-path path
          ...
          (cl-letf (((symbol-function 'piem--file-target-at-point)
                    ...
                    ((symbol-function 'piem--shell-command-path)
                     (lambda (&rest _)
            ...
            (let ((piem-visit-file-other-window other-window)
...
(ert-deftest piem-test-visit-file-native-non-tool-keeps-pi-state ()
       ...
       (cl-labels
           ((marker-state
            ...
            (pi-state ()
              (list
               :status piem--status
               :streaming-marker piem--streaming-marker
               :streaming-marker-state
               (marker-state piem--streaming-marker)
               :process piem--process
               :transition-generation
               piem--session-transition-generation
               :transition-active piem--session-transition-active
               :tool-cache piem--tool-args-cache
               :tool-cache-count
               (hash-table-count piem--tool-args-cache)
               :cached-tool-args
               (copy-tree
                (gethash "cached" piem--tool-args-cache))
               :live-tools piem--live-tool-blocks
               :live-tool-count
               (hash-table-count piem--live-tool-blocks)
               :live-sentinel
               (gethash "live-sentinel" piem--live-tool-blocks)
               :pending-tool piem--pending-tool-overlay
               :followups (copy-sequence piem--followup-queue)
               :session-directory
               (piem--chat-session-directory)
               :session-name (piem--chat-session-name))))
         (with-current-buffer chat
           (let ((inhibit-read-only t))
             (erase-buffer))
           (piem--display-tool-start
           ...
           (setq marker (copy-marker (point-max) t)
                 piem--status 'streaming
                 piem--streaming-marker marker
                 piem--process 'unit4-process
                 piem--session-transition-generation 17
                 piem--session-transition-active t
                 piem--followup-queue '("later" "next"))
           (puthash "cached" '(:path "cached.el")
                    piem--tool-args-cache)
           (puthash "live-sentinel" 'unit4-record
                    piem--live-tool-blocks)
           (setq overlays (piem-test--all-tool-overlays)
                 piem--pending-tool-overlay (car overlays))
           (set-buffer-modified-p nil)
...
(ert-deftest piem-test-visit-file-native-opener-error-propagates ()
     ...
     (let* ((chat (plist-get layout :chat))
       ...
       (with-current-buffer chat
         (setq marker (copy-marker (point-max) t)
               piem--status 'streaming
               piem--streaming-marker marker
               piem--process 'unit4-error-process
               piem--session-transition-generation 23
               piem--session-transition-active t
...
(ert-deftest piem-test-standard-link-button-ret-visits-semantic-target ()
  ...
  (dolist (case '(("RET" t :other)
    ...
    (pcase-let ((`(,keys ,option ,expected-kind) case))
      (let ((chat (generate-new-buffer " *piem-link-button*"))
        ...
        (unwind-protect
            (save-window-excursion
              ...
              (should (eq #'piem--dispatch-button
                          (key-binding (kbd "RET"))))
              (cl-letf (((symbol-function 'find-file)
                         (lambda (path)
...
(ert-deftest piem-test-standard-button-ret-keeps-tool-authority ()
              ...
              (with-current-buffer chat
                ...
                (let* ((overlay
                  ...
                  (pcase state
                    ...
                    (:invalid
                     (overlay-put overlay 'piem-tool-path nil)
                     (overlay-put overlay 'piem-tool-path-error
                                  message)))
                  (when cooled
                    (piem--cool-completed-tool-blocks
                     (list overlay))))
...
(ert-deftest piem-test-button-remap-leaves-non-ret-keys-native ()
  ...
  (let ((chat (generate-new-buffer " *pi-non-ret-button*"))
    ...
    (unwind-protect
        (save-window-excursion
          ...
          (use-local-map (copy-keymap piem-chat-mode-map))
          (let ((inhibit-read-only t))
...
(ert-deftest piem-test-local-md-ts-04-button-compatibility ()
...
(ert-deftest piem-test-standard-nonlocal-link-buttons-fail-closed ()
  ...
  (dolist (case '(("[src/fallback.el](https://example.com/x)"
    ...
    (let ((chat (generate-new-buffer " *piem-invalid-button*"))
          (overriding-terminal-local-map nil)
          (overriding-local-map nil)
          (pre-command-hook nil)
          (post-command-hook nil)
...
(ert-deftest piem-test-button-dispatch-leaves-direct-and-mouse-actions-native ()
  "Direct and mouse button activation bypass chat keyboard dispatch."
  (let ((chat (generate-new-buffer " *piem-native-button*"))
        (keyboard-actions 0)
        (mouse-actions 0))
    (unwind-protect
        (save-window-excursion
          ...
          (let ((event (list 'mouse-2
                             (list (selected-window) (point)
                                   '(0 . 0) 0))))
            (should (piem--dispatch-button event)))
          (should (= 1 keyboard-actions))
...
(ert-deftest piem-test-tool-toggle-ret-precedes-file-visitor ()
  "Actual RET activates a tool button; <return> and direct visiting still error."
  (let ((chat (generate-new-buffer " *piem-button-dispatch*"))
    ...
    (unwind-protect
        (save-window-excursion
          ...
          (goto-char (match-beginning 0))
          (should (button-at (point)))
          (should (eq #'piem-visit-file
                      (lookup-key piem-chat-mode-map (kbd "RET"))))
          (should (eq #'piem-visit-file
                      (lookup-key piem-chat-mode-map
                                  (kbd "<return>"))))
          (should (eq #'piem--dispatch-button
                      (key-binding (kbd "RET"))))
          (should (eq #'piem-visit-file
          ...
          (let ((toggle-count 0)
                ...
                (toggle (symbol-function 'piem--toggle-tool-output))
                (native-font-lock (symbol-function 'font-lock-ensure)))
            (cl-letf (((symbol-function 'piem--toggle-tool-output)
                       (lambda (button)
...
(ert-deftest piem-test-installed-md-ts-03-keeps-link-label-unbuttonized ()
  "Installed md-ts-mode 0.3 leaves Markdown link Return dispatch to chat mode."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--set-chat-session-identity "/tmp/")
    (let ((inhibit-read-only t))
    ...
    (should (eq #'piem-visit-file (key-binding (kbd "RET"))))
    (should (eq #'piem-visit-file
                (key-binding (kbd "<return>"))))
    (let ((buffer (current-buffer))
          (overriding-terminal-local-map nil)
...
(ert-deftest piem-test-diff-line-at-point-added ()
  "Should parse line number from added diff line."
  (with-temp-buffer
    (insert "+ 7     added line content")
    (goto-char (point-min))
    (should (= 7 (piem--diff-line-at-point)))))

(ert-deftest piem-test-diff-line-at-point-removed ()
  "Should parse line number from removed diff line."
  (with-temp-buffer
    (insert "-12     removed line content")
    (goto-char (point-min))
    (should (= 12 (piem--diff-line-at-point)))))

(ert-deftest piem-test-diff-line-at-point-context ()
...
(ert-deftest piem-test-diff-line-at-point-mid-line ()
  "Should work when point is anywhere on the line."
  (with-temp-buffer
    (insert "+ 42    some code here")
    (goto-char 15)  ;; Middle of line
    (should (= 42 (piem--diff-line-at-point)))))

(ert-deftest piem-test-code-block-line-at-point-first-line ()
  "Should return 1 for first line of code block content."
  (with-temp-buffer
    (insert "```python\nfirst line\nsecond line\n```")
    (goto-char (point-min))
    (forward-line 1)  ;; On "first line"
    (should (= 1 (piem--code-block-line-at-point)))))

(ert-deftest piem-test-code-block-line-at-point-third-line ()
  "Should return correct line for later lines."
  (with-temp-buffer
    (insert "```python\nline one\nline two\nline three\n```")
    (goto-char (point-min))
    (forward-line 3)  ;; On "line three"
    (should (= 3 (piem--code-block-line-at-point)))))

(ert-deftest piem-test-code-block-line-at-point-on-fence ()
  "Should return nil when on the fence line itself."
  (with-temp-buffer
    (insert "```python\ncontent\n```")
    (goto-char (point-min))  ;; On opening fence
    (should-not (piem--code-block-line-at-point))))

(ert-deftest piem-test-code-block-line-at-point-tilde-fence ()
  "Should support markdown tilde fences as code blocks."
  (with-temp-buffer
    (insert "~~~python\nline one\nline two\n~~~")
    (goto-char (point-min))
    (forward-line 2)  ;; On "line two"
    (should (= 2 (piem--code-block-line-at-point)))))

(ert-deftest piem-test-code-block-line-at-point-ignores-deep-indent-fence ()
...
(ert-deftest piem-test-code-block-line-at-point-after-closing-fence ()
...
(ert-deftest piem-test-code-block-line-at-point-ignores-faux-closing-fence ()
...
(ert-deftest piem-test-code-block-line-at-point-no-fence ()
...
(ert-deftest piem-test-tool-line-at-point-expanded-read-ignores-earlier-unclosed-fence ()
  ...
  (with-temp-buffer
    ...
    (let ((piem-tool-preview-lines 2)
      ...
      (let ((ov (seq-find (lambda (o) (overlay-get o 'piem-tool-block))
                          (overlays-at (point)))))
        (should ov)
        (should (= 6 (piem--tool-line-at-point ov)))))))

(ert-deftest piem-test-tool-overlay-stores-path ()
  ...
  (with-temp-buffer
    ...
    (should (equal "/tmp/test.py"
                   (overlay-get piem--pending-tool-overlay
                                'piem-tool-path)))))

(ert-deftest piem-test-tool-overlay-normalizes-remote-path ()
...
(ert-deftest piem-test-tool-overlay-preserves-multi-hop-remote-path ()
...
(ert-deftest piem-test-toolcall-preview-path-display-is-not-navigable-until-final ()
  ...
  (piem-test--with-streaming-assistant
    (piem-test--send-toolcall-message-update
     "toolcall_start" 0
     (list (piem-test--toolcall "call_1" "write" nil)))
    (piem-test--send-toolcall-message-update
     "toolcall_delta" 0
     (list (piem-test--toolcall
            "call_1" "write"
            '(:path "/tmp/preview.py" :content "line1\nline2\n")))
     "x")
    (let ((ov piem--pending-tool-overlay))
      (should (string-match-p "write /tmp/preview\\.py" (buffer-string)))
      (should-not (overlay-get ov 'piem-tool-path))
      (should-not (overlay-get ov 'piem-tool-raw-path))
      (should-not (overlay-get ov 'piem-tool-path-error))
      (goto-char (point-min))
      (search-forward "line2")
      (beginning-of-line)
      (should-error (piem-visit-file) :type 'user-error)
      (piem--handle-display-event
...
(ert-deftest piem-test-toolcall-delta-invalid-path-keeps-preview-navigation-absent ()
...
(ert-deftest piem-test-tool-execution-start-missing-path-keeps-preview-navigation-absent ()
  "Authoritative execution args with no path leave preview navigation absent."
  (piem-test--with-toolcall "read" '(:path "/tmp/preview.py")
    (let ((ov piem--pending-tool-overlay))
      ...
      (should (eq ov piem--pending-tool-overlay))
      (should-not (overlay-get ov 'piem-tool-path))
      (should-not (overlay-get ov 'piem-tool-raw-path))
      (should-not (overlay-get ov 'piem-tool-path-error)))))

(ert-deftest piem-test-visit-file-path-error-percent-is-literal ()
  "Stored path errors are user-error messages, not format strings."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "read" '(:path "/tmp/ok.py"))
    (let ((ov piem--pending-tool-overlay)
          (message "backend path contains %s marker"))
      (overlay-put ov 'piem-tool-path nil)
      (overlay-put ov 'piem-tool-path-error message)
      (goto-char (overlay-start ov))
      (let ((err (should-error (piem-visit-file)
                               :type 'user-error)))
        (should (equal message (error-message-string err)))))))

(ert-deftest piem-test-mismatched-remote-tool-path-renders-safely ()
    ...
    (let ((path "/ssh:127.0.0.1:/tmp/project/src/a.txt"))
      ...
      (let ((ov (seq-find (lambda (overlay)
        ...
        (should (string-match-p
                 ...
                 (overlay-get ov 'piem-tool-path-error)))
        (should-error (piem-visit-file) :type 'user-error)))))

(ert-deftest piem-test-process-filter-mismatched-remote-tool-path-renders-safely ()
  ...
  (with-temp-buffer
    ...
    (piem--set-chat-session-identity
     "/ssh:localhost:/tmp/project/")
    (let* ((path "/ssh:127.0.0.1:/tmp/project/src/a.txt")
           (proc (start-process "piem-render-test-cat" nil "cat"))
           (encode (lambda (event) (concat (json-encode event) "\n")))
           caught)
      (unwind-protect
          (progn
            (process-put proc 'piem-chat-buffer (current-buffer))
...
(ert-deftest piem-test-process-filter-numeric-text-delta-renders-safely ()
  "Numeric text_delta payloads must not signal from the process filter."
  (with-temp-buffer
    (piem-chat-mode)
    (let* ((proc (start-process "piem-render-test-text-cat" nil "cat"))
           (encode (lambda (event) (concat (json-encode event) "\n")))
           caught)
      (unwind-protect
          (progn
...
(ert-deftest piem-test-process-filter-numeric-thinking-delta-renders-safely ()
  "Numeric thinking_delta payloads must not signal from the process filter."
  (with-temp-buffer
    (piem-chat-mode)
    (let* ((proc (start-process "piem-render-test-thinking-cat" nil "cat"))
           (encode (lambda (event) (concat (json-encode event) "\n")))
           caught)
      (unwind-protect
          (progn
...
(ert-deftest piem-test-process-filter-numeric-bash-command-renders-safely ()
  "Malformed non-path bash metadata must not signal from the process filter."
  (with-temp-buffer
    (piem-chat-mode)
    (let* ((proc (start-process "piem-render-test-cat" nil "cat"))
...
(ert-deftest piem-test-process-filter-numeric-write-content-renders-safely ()
...
(ert-deftest piem-test-edit-tool-numeric-diff-renders-safely ()
...
(ert-deftest piem-test-nul-tool-path-renders-safely ()
...
(ert-deftest piem-test-non-string-tool-path-renders-safely ()
...
(ert-deftest piem-test-malformed-write-path-preview-renders-safely ()
...
(ert-deftest piem-test-visit-file-opens-remote-tool-path ()
...
(ert-deftest piem-test-tool-overlay-stores-path-after-finalize ()
  "Tool overlay should preserve path after finalization."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "edit" '(:path "/tmp/edit.el"))
    (piem--display-tool-end "edit" '(:path "/tmp/edit.el")
                          '((:type "text" :text "done"))
                          '(:diff "+ 1     new line")
                          nil)
    ...
    (let ((ov (seq-find (lambda (o) (overlay-get o 'piem-tool-block))
                        (overlays-in (point-min) (point-max)))))
      (should ov)
      (should (equal "/tmp/edit.el" (overlay-get ov 'piem-tool-path))))))

(ert-deftest piem-test-tool-overlay-stores-offset ()
  "Tool overlay should store read offset for line calculation."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "read" '(:path "/tmp/file.py" :offset 50))
    (piem--display-tool-end "read" '(:path "/tmp/file.py" :offset 50)
    ...
    (let ((ov (seq-find (lambda (o) (overlay-get o 'piem-tool-block))
                        (overlays-in (point-min) (point-max)))))
      (should ov)
      (should (= 50 (overlay-get ov 'piem-tool-offset))))))

(ert-deftest piem-test-tool-overlay-offset-defaults-nil ()
  "Tool overlay offset should be nil when not specified."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "read" '(:path "/tmp/file.py"))
    (piem--display-tool-end "read" '(:path "/tmp/file.py")
                          '((:type "text" :text "content"))
                          nil nil)
    (let ((ov (seq-find (lambda (o) (overlay-get o 'piem-tool-block))
                        (overlays-in (point-min) (point-max)))))
      (should ov)
      (should-not (overlay-get ov 'piem-tool-offset)))))

(ert-deftest piem-test-streaming-tool-overlay-has-path-after-finalize ()
...
the finalized overlay must still have the path for visit-file navigation."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--handle-display-event '(:type "agent_start"))
    (piem--handle-display-event '(:type "message_start"))
    ...
    (let ((ov (seq-find (lambda (o) (overlay-get o 'piem-tool-block))
      ...
      (should (equal "/tmp/foo.py"
                     (overlay-get ov 'piem-tool-path))))))

(ert-deftest piem-test-streaming-tool-path-from-execution-start ()
...
(ert-deftest piem-test-visit-file-from-edit-diff ()
  "visit-file should navigate to correct line from edit diff."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "edit" '(:path "/tmp/test.el"))
    (piem--display-tool-end "edit" '(:path "/tmp/test.el")
                          '((:type "text" :text "done"))
                          '(:diff "+ 42    (defun foo ())")
                          nil)
...
(ert-deftest piem-test-visit-file-read-with-offset ()
  "visit-file should use offset for read tool line calculation."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "read" '(:path "/tmp/big.py" :offset 100))
    (piem--display-tool-end "read" '(:path "/tmp/big.py" :offset 100)
    ...
    (let ((result (piem-test--visit-file-line 200)))
      ;; Line 2 in code block + offset 100 - 1 = 101
      (should (= 101 (plist-get result :line))))))

(ert-deftest piem-test-visit-file-read-beginning-line ()
  "visit-file should navigate to line 1 from first read content line."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "read" '(:path "/tmp/start.txt"))
    (piem--display-tool-end "read" '(:path "/tmp/start.txt")
                          '((:type "text" :text "line1\nline2\nline3"))
                          nil nil)
    (goto-char (point-min))
    (search-forward "line1")
    (beginning-of-line)
    (let ((result (piem-test--visit-file-line 20)))
      (should (= 1 (plist-get result :line))))))

(ert-deftest piem-test-visit-file-toggle-opens-same-window ()
  "Prefix arg should invert `piem-visit-file-other-window'."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-visit-file-other-window t))
      (piem--display-tool-start "read" '(:path "/tmp/start.txt"))
      ...
      (search-forward "line2")
      (beginning-of-line)
      (let ((result (piem-test--visit-file-line 20 t)))
        (should (eq :same (plist-get result :open-kind)))
        (should (= 2 (plist-get result :line)))))))

(ert-deftest piem-test-visit-file-accounts-for-stripped-blank-lines ()
...
File has blanks at lines 3,5. Pressing RET on 'line06' should go to line 6."
  (with-temp-buffer
    ...
    (piem--display-tool-start "read" '(:path "/tmp/test.txt"))
    (piem--display-tool-end "read" '(:path "/tmp/test.txt")
...
(ert-deftest piem-test-visit-file-preserves-blank-lines-when-not-collapsed ()
...
(ert-deftest piem-test-visit-file-preserves-blank-lines-when-expanded ()
...
(ert-deftest piem-test-visit-file-collapsed-closing-fence-errors ()
...
(ert-deftest piem-test-visit-file-collapsed-toggle-line-errors ()
...
(ert-deftest piem-test-visit-file-edit-context-line ()
...
(ert-deftest piem-test-visit-file-edit-context-first-line ()
  "RET on first unchanged line in edit diff should jump to line 1."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "edit" '(:path "/tmp/greeting.py"))
    (piem--display-tool-end
...
(ert-deftest piem-test-visit-file-generic-path-expanded-line ()
  "Generic tool output with :path should map expanded lines correctly."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "custom_tool" '(:path "/tmp/custom.txt"))
    (piem--display-tool-end
...
(ert-deftest piem-test-visit-file-generic-path-collapsed-line ()
...
(ert-deftest piem-test-visit-file-uses-correct-adjacent-finalized-block ()
  "visit-file uses the overlay and line-map for the block at point."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-tool-preview-lines 4))
      (piem--display-tool-start "read" '(:path "/tmp/a.txt" :offset 10))
      (piem--display-tool-end
       "read" '(:path "/tmp/a.txt" :offset 10)
       '((:type "text" :text "a10\na11\na12\na13\na14\na15"))
       nil nil)
      (piem--display-tool-start "read" '(:path "/tmp/b.txt" :offset 100))
      (piem--display-tool-end
...
(ert-deftest piem-test-toggle-tool-output-does-not-affect-adjacent-blocks ()
  "Expanding one finalized tool block leaves the adjacent block untouched."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-tool-preview-lines 4))
      (piem--display-tool-start "read" '(:path "/tmp/a.txt"))
      (piem--display-tool-end
       "read" '(:path "/tmp/a.txt")
       '((:type "text" :text "a1\na2\na3\na4\na5\na6"))
       nil nil)
      (piem--display-tool-start "read" '(:path "/tmp/b.txt"))
      (piem--display-tool-end
       "read" '(:path "/tmp/b.txt")
       '((:type "text" :text "b1\nb2\nb3\nb4\nb5\nb6"))
       nil nil)
...
(ert-deftest piem-test-visit-file-write-ignores-offset ()
  ...
  (with-temp-buffer
    ...
    (search-forward "line2")
    (beginning-of-line)
    (let ((result (piem-test--visit-file-line 120)))
      (should (= 2 (plist-get result :line))))))

(ert-deftest piem-test-visit-file-write-expanded-after-streamed-header-update ()
...
(ert-deftest piem-test-truncate-visual-lines-wrapping ()
  ...
  (let ((long-line (make-string 160 ?a))
    ...
    (let* ((content (concat long-line "\n" short-line))
           ...
           (result (piem--truncate-to-visual-lines content 2 80)))
...
(ert-deftest piem-test-truncate-visual-lines-single-line-byte-limit ()
...
Regression test: single lines should respect byte limit even with no newlines."
  (let ((piem-preview-max-bytes 100))
...
(ert-deftest piem-test-tool-output-truncates-long-lines ()
...
(ert-deftest piem-test-tab-bound-to-toggle-tool-section ()
  "TAB is bound to piem-toggle-tool-section for tool block handling."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "bash" '(:command "ls"))
    (piem--display-tool-end "bash" '(:command "ls")
    ...
    (goto-char (point-min))
    (should (piem--find-tool-block-bounds))
    ;; piem-toggle-tool-section should be bound to TAB and <tab>
    (should (eq (lookup-key piem-chat-mode-map (kbd "TAB")) 'piem-toggle-tool-section))
    (should (eq (lookup-key piem-chat-mode-map (kbd "<tab>")) 'piem-toggle-tool-section))))

(ert-deftest piem-test-tool-error-indicated ()
  "Tool error uses error overlay face but no [error] badge."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "bash" '(:command "false"))
    (piem--display-tool-end "bash" '(:command "false")
...
(ert-deftest piem-test-tool-success-not-error ()
  "Tool with isError :false should not show error indicator."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "bash" '(:command "test"))
...
(ert-deftest piem-test-tool-output-survives-message-render ()
  ...
  (with-temp-buffer
    ...
    (piem--handle-display-event '(:type "message_end"))
...
(ert-deftest piem-test-display-handler-handles-tool-start ()
...
(ert-deftest piem-test-display-handler-handles-tool-end ()
...
(ert-deftest piem-test-display-handler-handles-tool-update ()
  ...
  (with-temp-buffer
    ...
    (piem-test--flush-tool-updates)
    ;; Should show partial content
    (should (string-match-p "streaming output" (buffer-string)))))

(ert-deftest piem-test-tool-update-shows-rolling-tail ()
...
(ert-deftest piem-test-tool-update-truncates-single-long-line ()
...
(ert-deftest piem-test-parallel-tool-execution-keeps-output-with-own-header ()
    ...
    (let ((content (buffer-substring-no-properties (point-min) (point-max))))
      (should (equal content
                     (concat "$ echo one\n"
                             ...
                             "$ echo two\n"
                             "```\nfinal bravo\n```\n\n")))
      (should (= 1 (piem-test--count-matches "\\$ echo one" content)))
      (should (= 1 (piem-test--count-matches "\\$ echo two" content))))
    (should (= 0 (hash-table-count piem--live-tool-blocks)))))

;; ── Coalesced tool_execution_update rendering ──────────────────────

(defun piem-test--send-tool-execution-update (tool-call-id text)
...
(defun piem-test--flush-tool-updates ()
  "Run the coalesced tool-update flush in the current buffer."
  (piem--flush-tool-updates (current-buffer)))

(ert-deftest piem-test-tool-update-queues-without-immediate-render ()
  ...
  (with-temp-buffer
    ...
    (piem--handle-display-event
     '(:type "tool_execution_start"
       ...
       :toolCallId "call_1"
       :args (:command "long-running")))
    (piem-test--send-tool-execution-update "call_1" "line one\n")
    ;; Nothing rendered yet
    (should-not (string-match-p "line one" (buffer-string)))
    ;; ...but the latest partial result is pending and one timer is armed
    (should (assoc "call_1" piem--pending-tool-updates))
    (should (timerp piem--tool-update-flush-timer))))

(ert-deftest piem-test-tool-update-supersedes-without-rearm ()
  ...
  (with-temp-buffer
    ...
    (piem--handle-display-event
     '(:type "tool_execution_start"
       ...
       :toolCallId "call_1"
       :args (:command "long-running")))
    (piem-test--send-tool-execution-update "call_1" "alpha-first\n")
    (let ((first-timer piem--tool-update-flush-timer)
          (replace-calls 0))
      (piem-test--send-tool-execution-update "call_1" "bravo-second\n")
      ;; The pending entry was replaced, the timer was not rearmed
      (should (eq first-timer piem--tool-update-flush-timer))
      (should (= 1 (length piem--pending-tool-updates)))
      (let ((orig (symbol-function 'piem--tool-block-replace-body)))
        (cl-letf (((symbol-function 'piem--tool-block-replace-body)
                   (lambda (&rest args)
      ...
      (should-not piem--pending-tool-updates)
      (should-not piem--tool-update-flush-timer))))

(ert-deftest piem-test-tool-update-flush-renders-parallel-tools-in-own-blocks ()
...
under its own header."
  (with-temp-buffer
    ...
    (piem-test--send-tool-execution-update "c1" "alpha\n")
    (piem-test--send-tool-execution-update "c2" "bravo\n")
    (piem-test--flush-tool-updates)
    (let ((body-c1 (piem-test--tool-stream-body-by-id "c1"))
          (body-c2 (piem-test--tool-stream-body-by-id "c2")))
...
(ert-deftest piem-test-tool-execution-end-discards-pending-preview ()
...
and renders only the authoritative final result."
  (with-temp-buffer
    ...
    (piem-test--send-tool-execution-update "call_1" "preview text\n")
    (should (assoc "call_1" piem--pending-tool-updates))
...
(ert-deftest piem-test-tool-execution-end-keeps-other-pending-updates ()
...
tool's pending preview, and keeps the flush timer armed."
  (with-temp-buffer
    ...
    (should-not (assoc "c1" piem--pending-tool-updates))
    (should (assoc "c2" piem--pending-tool-updates))
    (should (timerp piem--tool-update-flush-timer))
    (piem-test--flush-tool-updates)
    (should (string-match-p "bravo"
                            (piem-test--tool-stream-body-by-id "c2")))))

(ert-deftest piem-test-tool-update-flush-defers-to-typing ()
...
when input clears, the next flush renders the pending preview."
  (with-temp-buffer
    ...
    (piem--handle-display-event
     '(:type "tool_execution_start"
       :toolName "bash" :toolCallId "call_1" :args (:command "test")))
    (piem-test--send-tool-execution-update "call_1" "deferred text\n")
    (let ((armed-timer piem--tool-update-flush-timer))
      ...
      (should (assoc "call_1" piem--pending-tool-updates))
      (should (timerp piem--tool-update-flush-timer))
      (should-not (eq armed-timer piem--tool-update-flush-timer)))
    (piem-test--flush-tool-updates)
    (should (string-match-p "deferred text" (buffer-string)))
    (should-not piem--pending-tool-updates)
    (should-not piem--tool-update-flush-timer)))

(ert-deftest piem-test-agent-end-clears-pending-tool-updates ()
  ...
  (with-temp-buffer
    ...
    (piem--handle-display-event
     '(:type "tool_execution_start"
       :toolName "bash" :toolCallId "call_1" :args (:command "test")))
    (piem-test--send-tool-execution-update "call_1" "preview\n")
    (should (timerp piem--tool-update-flush-timer))
    (piem--handle-display-event '(:type "agent_end"))
    (should-not piem--pending-tool-updates)
    (should-not piem--tool-update-flush-timer)))

(ert-deftest piem-test-tool-update-flush-error-leaves-clean-state ()
...
pending state; later updates flush normally again."
  (with-temp-buffer
    ...
    (piem--handle-display-event
     '(:type "tool_execution_start"
       :toolName "bash" :toolCallId "call_1" :args (:command "test")))
    (piem-test--send-tool-execution-update "call_1" "boom\n")
    (cl-letf (((symbol-function 'piem--display-tool-update)
               (lambda (&rest _) (signal 'error '("render failed")))))
      (should-error (piem-test--flush-tool-updates)
                    :type 'error))
    (should-not piem--pending-tool-updates)
    (should-not piem--tool-update-flush-timer)
    ;; The queue still works afterwards
    (piem-test--send-tool-execution-update "call_1" "recovered\n")
    (piem-test--flush-tool-updates)
    (should (string-match-p "recovered" (buffer-string)))))

(ert-deftest piem-test-clear-render-artifacts-discards-pending-tool-updates ()
...
(ert-deftest piem-test-tool-update-timer-cancelled-on-kill ()
...
hooks, including `kill-buffer-hook'."
  (let ((buf (generate-new-buffer "*piem-test-update-kill*"))
        (piem-quit-without-confirmation t)
        timer)
    (unwind-protect
        (with-current-buffer buf
          ...
          (piem--handle-display-event
           '(:type "tool_execution_start"
             :toolName "bash" :toolCallId "call_1" :args (:command "test")))
          (piem-test--send-tool-execution-update "call_1" "preview\n")
          (setq timer piem--tool-update-flush-timer)
          (should (timerp timer))
          (kill-buffer buf)
...
(defun piem-test--tool-block-overlay-by-id (tool-call-id)
          ...
          (maphash
           (lambda (_content-index stream)
             (when (equal tool-call-id
                          (piem--tool-stream-tool-call-id stream))
               (when-let* ((block
                            (piem--tool-stream-block stream)))
                 (throw 'found (piem--tool-block-overlay block)))))
           piem--toolcall-streams)
          nil))))

(defun piem-test--tool-header-from-overlay (overlay)
  "Return the plain header text from tool block OVERLAY."
  (when-let* ((header-end (overlay-get overlay 'piem-header-end)))
    (buffer-substring-no-properties
     (overlay-start overlay)
     (1- (marker-position header-end)))))

(defun piem-test--tool-header-by-id (tool-call-id)
  "Return the plain header text for TOOL-CALL-ID."
  (when-let* ((overlay
               (piem-test--tool-block-overlay-by-id tool-call-id)))
    (piem-test--tool-header-from-overlay overlay)))

(defun piem-test--tool-stream-body-from-overlay (ov)
  "Return tool overlay OV body as plain text."
  (let ((header-end (overlay-get ov 'piem-header-end)))
    (buffer-substring-no-properties header-end (overlay-end ov))))

(defun piem-test--pending-tool-stream-body ()
  "Return pending tool overlay body as plain text."
  (piem-test--tool-stream-body-from-overlay
   piem--pending-tool-overlay))

(defun piem-test--tool-stream-body-by-id (tool-call-id)
  "Return the streamed body for TOOL-CALL-ID as plain text."
  (piem-test--tool-stream-body-from-overlay
   (piem-test--tool-block-overlay-by-id tool-call-id)))

(defun piem-test--tool-content-lines-from-stream (stream)
...
(defun piem-test--pending-tool-content-lines ()
  "Return streamed content lines for the pending tool overlay only."
  (piem-test--tool-content-lines-from-stream
   (piem-test--pending-tool-stream-body)))

(defun piem-test--tool-content-lines-by-id (tool-call-id)
  "Return streamed content lines for TOOL-CALL-ID only."
  (piem-test--tool-content-lines-from-stream
   (piem-test--tool-stream-body-by-id tool-call-id)))

(ert-deftest piem-test-toolcall-start-after-text-has-blank-line ()
...
(ert-deftest piem-test-toolcall-delta-updates-header-not-path ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (should-not (overlay-get piem--pending-tool-overlay
                             'piem-tool-path))
    (should-not (overlay-get piem--pending-tool-overlay
                             'piem-tool-raw-path))
    (should-not (overlay-get piem--pending-tool-overlay
                             'piem-tool-path-error))))

(ert-deftest piem-test-multitool-preview-update-keeps-path-metadata-absent ()
  "Updating one preview header must not leave another block's path metadata stale."
  (piem-test--with-streaming-assistant
    (let ((initial (list (piem-test--toolcall
    ...
    (should (equal "write /tmp/a.py"
                   (piem-test--tool-header-by-id "call_1")))
    (should (equal "write /tmp/b.py"
                   (piem-test--tool-header-by-id "call_2")))
    (let ((updated (list (piem-test--toolcall
    ...
    (dolist (id '("call_1" "call_2"))
      (let ((ov (piem-test--tool-block-overlay-by-id id)))
        (should ov)
        (should-not (overlay-get ov 'piem-tool-path))
        (should-not (overlay-get ov 'piem-tool-raw-path))
        (should-not (overlay-get ov 'piem-tool-path-error))))))

(ert-deftest piem-test-toolcall-header-updated-at-execution-start ()
...
(ert-deftest piem-test-generic-toolcall-streaming-skips-json ()
...
(ert-deftest piem-test-generic-toolcall-end-restores-json-header ()
...
(ert-deftest piem-test-generic-toolcall-json-header-escapes-controls ()
  ...
  (piem-test--with-streaming-assistant
    (let ((value (concat "x" (string #x85) "y" (string #x202e) "z")))
      ...
      (let ((header (piem-test--tool-header-by-id "call_1")))
        (should (string-match-p (regexp-quote "\\u0085") header))
        (should (string-match-p (regexp-quote "\\u202E") header))
        (should-not (cl-position #x85 header :test #'=))
        (should-not (cl-position #x202e header :test #'=))))))

(ert-deftest piem-test-generic-toolcall-execution-start-restores-json-header ()
...
(ert-deftest piem-test-generic-toolcall-end-finalizes-only-matching-preview ()
  "toolcall_end finalizes only its content block's generic header."
  (piem-test--with-streaming-assistant
    (let ((toolcalls (list (piem-test--toolcall
                            "call_1" "subagent"
...
(ert-deftest piem-test-toolcall-start-creates-overlay ()
  "toolcall_start in message_update creates a keyed preview overlay."
  (piem-test--with-toolcall "write" '(:path "/tmp/foo.py")
    (should (string-match-p "write /tmp/foo\\.py" (buffer-string)))
    (should piem--pending-tool-overlay)
    (should (piem--tool-block-get "call_1"))))

(ert-deftest piem-test-toolcall-delta-replaces-header-exactly-across-multiple-updates ()
  "Repeated header updates should not leave stale trailing characters behind."
  (piem-test--with-streaming-assistant
    (piem-test--send-toolcall-message-update
     "toolcall_start" 0
     (list (piem-test--toolcall "call_1" "read" nil)))
    (dolist (path '("/tmp/a.py" "/tmp/ab.py" "/tmp/abc.py"))
      (piem-test--send-toolcall-message-update
       "toolcall_delta" 0
       (list (piem-test--toolcall "call_1" "read" (list :path path)))
       "x"))
    (let ((content (buffer-string)))
      (should (string-match-p "\nread /tmp/abc\\.py\n\\'" content))
      (should-not (string-match-p "read /tmp/abc\\.pyy+" content)))))

(ert-deftest piem-test-toolcall-multiple-previews-appear-in-source-order ()
...
(ert-deftest piem-test-toolcall-reconcile-removes-stale-preview-blocks ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (let ((first (piem-test--tool-block-overlay-by-id "call_1"))
          (stale (piem-test--tool-block-overlay-by-id "call_2")))
      (piem--handle-display-event
       '(:type "message_end"
         :message (:role "assistant" :stopReason "toolUse"
      ...
      (should (eq first
                  (piem-test--tool-block-overlay-by-id "call_1")))
      (should-not (overlay-buffer stale))
      (should (equal "write /tmp/a.py"
                     (piem-test--tool-header-by-id "call_1")))
      (should-not (piem--tool-block-get "call_2"))
      (should (= 1 (hash-table-count piem--live-tool-blocks))))))

(ert-deftest piem-test-tool-preview-helper-inserts-before-later-live-block ()
  "Earlier preview orders insert before already-live later blocks."
  (with-temp-buffer
    (piem-chat-mode)
    (let* ((later (piem--display-tool-start
                   "write" '(:path "/tmp/b.py") "call_2" 2))
           (earlier (piem--display-tool-start
                     "write" '(:path "/tmp/a.py") "call_1" 1))
           (later-overlay (piem--tool-block-overlay later))
           (earlier-overlay (piem--tool-block-overlay earlier))
           (content (buffer-string)))
      (should (< (string-match "write /tmp/a\\.py" content)
                 (string-match "write /tmp/b\\.py" content)))
...
(ert-deftest piem-test-live-tool-block-ordering-stays-monotonic-after-explicit-previews ()
  "Implicit live blocks should sort after earlier explicit preview orders."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-tool-start "write" '(:path "/tmp/b.py") "call_2" 2)
    (piem--display-tool-start "bash" '(:command "echo x") "call_exec")
    (piem--display-tool-start "write" '(:path "/tmp/a.py") "call_1" 1)
    (should (equal '("call_1" "call_2" "call_exec")
                   (mapcar #'piem--tool-block-tool-call-id
                           (piem--live-tool-blocks-in-order))))))

(ert-deftest piem-test-toolcall-delta-streams-write-content ()
...
(ert-deftest piem-test-tool-execution-update-keeps-bash-output-on-next-line-after-header-update ()
  "Bash execution output should stay on the line after an updated header."
  (piem-test--with-streaming-assistant
    (piem-test--send-toolcall-message-update
     "toolcall_start" 0
     (list (piem-test--toolcall "call_1" "bash" nil)))
    (piem--handle-display-event
     '(:type "message_end" :message (:role "assistant")))
    (piem--handle-display-event
     '(:type "tool_execution_start"
...
(ert-deftest piem-test-toolcall-delta-streams-multiple-write-previews-independently ()
  ...
  (piem-test--with-streaming-assistant
    (let ((toolcalls (list (piem-test--toolcall
      ...
      (should (string-match-p "print(\\\"a\\\")"
                              (piem-test--tool-stream-body-by-id
                               "call_1")))
...
(ert-deftest piem-test-toolcall-delta-uses-fenced-code-block ()
...
(ert-deftest piem-test-toolcall-delta-backtick-safe-fence ()
...
(ert-deftest piem-test-toolcall-delta-streaming-has-keyword-face ()
...
(ert-deftest piem-test-toolcall-delta-fenced-prevents-markdown-bold ()
...
(ert-deftest piem-test-toolcall-delta-survives-restore-tool-properties ()
...
`restore-tool-properties'.  Fenced content must keep its syntax faces."
  (piem-test--with-toolcall "write" '(:path "/tmp/foo.py")
    ...
    (piem--restore-tool-properties (point-min) (point-max))
...
(ert-deftest piem-test-toolcall-delta-incremental-fontify-context ()
...
(ert-deftest piem-test-toolcall-delta-streams-without-mode ()
...
(ert-deftest piem-test-toolcall-delta-same-size-refreshes-preview ()
...
(ert-deftest piem-test-toolcall-delta-same-size-unchanged-skips-redraw ()
...
(ert-deftest piem-test-toolcall-delta-empty-content-clears-preview ()
  ...
  (piem-test--with-toolcall "write" '(:path "/tmp/foo.py")
    ...
    (let ((body (piem-test--pending-tool-stream-body)))
      (should-not (string-match-p "line1" body))
      (should (string-empty-p (string-trim-right body "\n+"))))))

(ert-deftest piem-test-toolcall-delta-updates-on-new-line ()
...
(ert-deftest piem-test-toolcall-delta-stable-line-count ()
...
(ert-deftest piem-test-toolcall-delta-lang-preview-obeys-visual-cap ()
  "Language-aware write streaming enforces visual-line preview limits.
Wrapped lines must stay within `piem-tool-preview-lines'
during streaming updates."
  (piem-test--with-toolcall "write" '(:path "/tmp/foo.py")
    (let ((piem-tool-preview-lines 3)
          (content (concat (make-string 36 ?x) "\nline2\nline3\n")))
      (cl-letf (((symbol-function 'piem--chat-window-width)
                 (lambda () 10)))
        (piem-test--send-delta
         "write" `(:path "/tmp/foo.py" :content ,content))
        (let* ((content-lines (piem-test--pending-tool-content-lines))
               (visual-lines
                (apply #'+
                       (mapcar (lambda (line)
                                 (max 1
                               ...
                               content-lines))))
          (should (<= visual-lines piem-tool-preview-lines)))))))

(ert-deftest piem-test-toolcall-delta-lang-cap-preserves-syntax-face ()
...
(ert-deftest piem-test-toolcall-delta-rewrites-bounded-preview ()
  ...
  (piem-test--with-toolcall "write" '(:path "/tmp/foo.py")
    (let ((piem-tool-preview-lines 3))
      ...
      (let ((body (piem-test--pending-tool-stream-body)))
        ...
        (should (= 1 (piem-test--count-matches "line6" body)))))))

(ert-deftest piem-test-toolcall-execution-start-reuses-preview-block ()
...
(ert-deftest piem-test-delta-only-toolcall-full-event-flow ()
  "Pi's delta-only toolcall stream reuses one block through execution."
  (piem-test--with-streaming-assistant
    (piem-test--send-assistant-message-update
     '(:type "toolcall_start" :contentIndex 0
       :id "call_1" :toolName "write"))
    (let ((preview (piem-test--tool-block-overlay-by-id "call_1")))
      (should preview)
      (should (equal "write ..."
                     (piem-test--tool-header-by-id "call_1")))
      (should (= 1 (length (piem-test--all-tool-overlays))))
      (piem-test--send-assistant-message-update
       '(:type "toolcall_delta" :contentIndex 0
      ...
      (should (equal '("streaming content")
                     (piem-test--tool-content-lines-by-id "call_1")))
      ;; Streaming previews are display-only; navigation waits for execution.
      (should-not (overlay-get preview 'piem-tool-path))
      (piem-test--send-assistant-message-update
       '(:type "toolcall_end" :contentIndex 0
      ...
      (should (eq preview
                  (piem-test--tool-block-overlay-by-id "call_1")))
      (should (equal "/tmp/foo.py"
                     (overlay-get preview 'piem-tool-raw-path)))
      (piem--handle-display-event
       '(:type "tool_execution_end" :toolCallId "call_1"
         :toolName "write"
...
(ert-deftest piem-test-delta-only-toolcalls-interleave-by-content-index ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (let ((first (piem-test--tool-block-overlay-by-id "call_a"))
          (second (piem-test--tool-block-overlay-by-id "call_b")))
      (piem-test--send-assistant-message-update
       '(:type "toolcall_delta" :contentIndex 1
      ...
      (should (eq first
                  (piem-test--tool-block-overlay-by-id "call_a")))
      (should (eq second
                  (piem-test--tool-block-overlay-by-id "call_b")))
      (should (equal '("alpha")
                     (piem-test--tool-content-lines-by-id "call_a")))
      (should (equal '("bravo")
                     (piem-test--tool-content-lines-by-id "call_b")))
      (should (equal '("call_a" "call_b")
                     (mapcar
                      (lambda (index)
                        (piem--tool-stream-tool-call-id
                         (gethash index piem--toolcall-streams)))
                      '(1 3))))
      ...
      (should (eq second
                  (piem-test--tool-block-overlay-by-id "call_b")))
      (should (= 1 (hash-table-count piem--live-tool-blocks)))
      (should (= 2 (length (piem-test--all-tool-overlays))))
      (piem-test--send-assistant-message-update
...
(ert-deftest piem-test-delta-only-toolcall-decodes-preview-strings ()
...
(ert-deftest piem-test-delta-only-toolcall-preserves-invalid-backslashes ()
...
(ert-deftest piem-test-delta-only-write-preview-retains-bounded-tail ()
  "Large streamed write values retain only the configured preview tail."
  (let ((piem-preview-max-bytes 24))
    (piem-test--with-streaming-assistant
      (piem-test--send-assistant-message-update
       '(:type "toolcall_start" :contentIndex 0
         :id "call_1" :toolName "write"))
      (piem-test--send-assistant-message-update
       `(:type "toolcall_delta" :contentIndex 0
         :delta ,(concat "{\"content\":\""
                         (make-string 80 ?x)
                         "TAIL\\n\"}")))
      (let ((body (piem-test--tool-stream-body-by-id "call_1")))
        (should (string-match-p "TAIL" body))
        (should (string-match-p "earlier output" body))
        (should (< (length body) 80))))))

(ert-deftest piem-test-delta-only-late-path-relabels-write-fence ()
...
(ert-deftest piem-test-delta-only-truncation-resets-for-later-content ()
  "A later content key clears obsolete truncation presentation state."
  (let ((piem-preview-max-bytes 12))
    (piem-test--with-streaming-assistant
      (piem-test--send-assistant-message-update
       '(:type "toolcall_start" :contentIndex 0
         :id "call_1" :toolName "write"))
...
(ert-deftest piem-test-delta-only-later-nonstring-clears-preview-value ()
...
(ert-deftest piem-test-message-end-rekeys-streamed-toolcall-preview ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (let ((preview
           (piem-test--tool-block-overlay-by-id "provisional")))
      (piem-test--send-assistant-message-update
       '(:type "toolcall_end" :contentIndex 0
         :toolCall (:type "toolCall" :id "ended" :name "read"
                    :arguments (:path "/tmp/ended.txt"))))
      (should (eq preview
                  (piem-test--tool-block-overlay-by-id "ended")))
      (piem--handle-display-event
       '(:type "message_end"
         :message (:role "assistant" :stopReason "toolUse"
                   :content [(:type "toolCall" :id "final" :name "read"
                              :arguments (:path "/tmp/final.txt"))])))
      (should-not (piem--tool-block-get "provisional"))
      (should-not (piem--tool-block-get "ended"))
      (should (eq preview
                  (piem-test--tool-block-overlay-by-id "final")))
      (should (equal "read /tmp/final.txt"
                     (piem-test--tool-header-by-id "final")))
      (should (= 1 (length (piem-test--all-tool-overlays)))))))

(ert-deftest piem-test-toolcall-end-clears-stale-write-preview-body ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (let ((preview (piem-test--tool-block-overlay-by-id "call_1")))
      ...
      (should (string-empty-p
               (string-trim
                (piem-test--tool-stream-body-from-overlay preview)))))))

(ert-deftest piem-test-empty-start-ids-keep-indexed-previews-independent ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (let* ((overlays (piem-test--all-tool-overlays))
      ...
      (piem-test--send-assistant-message-update
       '(:type "toolcall_delta" :contentIndex 1
         :delta "{\"path\":\"/tmp/b.py\"}"))
      (should (equal "write /tmp/a.py"
                     (piem-test--tool-header-from-overlay first)))
      (should (equal "write /tmp/b.py"
                     (piem-test--tool-header-from-overlay second)))
      (piem-test--send-assistant-message-update
       '(:type "toolcall_end" :contentIndex 0
         :toolCall (:type "toolCall" :id "call_a" :name "write"
                    :arguments (:path "/tmp/a.py"))))
...
(ert-deftest piem-test-metadata-less-toolcall-start-finalizes-at-end ()
...
(ert-deftest piem-test-metadata-less-reversed-ends-preserve-distinct-blocks ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (let ((later (piem-test--tool-block-overlay-by-id "call_b")))
      (piem-test--send-assistant-message-update
       '(:type "toolcall_end" :contentIndex 0
         :toolCall (:type "toolCall" :id "call_a" :name "read"
                    :arguments (:path "/tmp/a"))))
      (let ((earlier (piem-test--tool-block-overlay-by-id "call_a")))
        (should (= 2 (length (piem-test--all-tool-overlays))))
        ...
        (should (equal "read /tmp/a"
                       (piem-test--tool-header-by-id "call_a")))
        (should (equal "read /tmp/b"
                       (piem-test--tool-header-by-id "call_b")))))))

(ert-deftest piem-test-metadata-less-duplicate-final-ids-keep-indexed-blocks ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (piem-test--send-assistant-message-update
     '(:type "toolcall_end" :contentIndex 1
       :toolCall (:type "toolCall" :id "duplicate" :name "read"
                  :arguments (:path "/tmp/b"))))
    (let* ((first-stream (gethash 0 piem--toolcall-streams))
           (second-stream (gethash 1 piem--toolcall-streams))
           (first-block (piem--tool-stream-block first-stream))
           (second-block (piem--tool-stream-block second-stream))
           (first-overlay (piem--tool-block-overlay first-block))
           (second-overlay (piem--tool-block-overlay second-block)))
      ...
      (should (equal "read /tmp/a"
                     (piem-test--tool-header-from-overlay
                      first-overlay))))))

(ert-deftest piem-test-duplicate-start-ids-keep-indexed-previews-independent ()
...
(ert-deftest piem-test-colliding-provisional-id-rekeys-by-content-index ()
         ...
         :message (:role "assistant" :stopReason "toolUse"
                   :content [(:type "toolCall" :id "duplicate" :name "read"
                             ...
                             (:type "toolCall" :id "fixed" :name "read"
                              :arguments (:path "/tmp/b"))])))
      (should (eq first
                  (piem-test--tool-block-overlay-by-id "duplicate")))
      (should (eq second
                  (piem-test--tool-block-overlay-by-id "fixed")))
      (piem--handle-display-event
...
(ert-deftest piem-test-duplicate-final-ids-leave-one-owned-preview ()
       ...
       :message (:role "assistant" :stopReason "toolUse"
                 :content [(:type "toolCall" :id "duplicate" :name "read"
                           ...
                           (:type "toolCall" :id "duplicate" :name "read"
                            :arguments (:path "/tmp/b"))])))
    (should (= 1 (length (piem-test--all-tool-overlays))))
    (should (piem--tool-block-get "duplicate"))
    (piem--handle-display-event
     '(:type "agent_end" :messages [] :willRetry :json-false))
    (should (= 0 (hash-table-count piem--live-tool-blocks)))))

(ert-deftest piem-test-message-end-removal-leaves-no-dangling-preview ()
...
(ert-deftest piem-test-process-exit-finalizes-generation-blocks ()
...
(ert-deftest piem-test-abort-finalizes-empty-id-generation-blocks ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (let ((overlays (piem-test--all-tool-overlays)))
      ...
      (dolist (overlay overlays)
        (should (eq (overlay-get overlay 'face)
                    'piem-tool-block-error)))
      (should (= 0 (hash-table-count piem--toolcall-streams)))
      (should-not piem--pending-tool-overlay))))

(ert-deftest piem-test-toolcall-full-event-flow ()
...
(ert-deftest piem-test-toolcall-non-write-shows-header-only ()
  "Non-write tools show header from toolcall_start but no streaming content."
  (piem-test--with-toolcall "read" '(:path "/tmp/test.txt")
    (piem-test--send-delta
     "read" '(:path "/tmp/test.txt" :offset 1))
    (should (string-match-p "read /tmp/test\\.txt" (buffer-string)))
    (should-not (string-match-p "offset" (buffer-string)))))

(ert-deftest piem-test-toolcall-abort-cleans-up ()
  ...
  (piem-test--with-streaming-assistant
    ...
    (should (equal "write /tmp/new.py"
                   (piem-test--tool-header-by-id "call_2")))
    (should (equal '("new")
                   (piem-test--tool-content-lines-by-id "call_2")))))

(ert-deftest piem-test-abort-finalizes-all-live-tool-blocks ()
  ...
  (with-temp-buffer
    ...
    (piem--handle-display-event
     '(:type "tool_execution_start"
       ...
       :toolCallId "c2"
       :args (:command "echo two")))
    (let* ((ov1 (piem-test--tool-block-overlay-by-id "c1"))
           (ov2 (piem-test--tool-block-overlay-by-id "c2")))
      (should ov1)
      (should ov2)
      (should (= 2 (hash-table-count piem--live-tool-blocks)))
      (should (= 2 (hash-table-count piem--tool-args-cache)))
      ...
      (should (= 0 (hash-table-count piem--live-tool-blocks)))
      (should (= 0 (hash-table-count piem--tool-args-cache)))
      (should (eq (overlay-get ov1 'face) 'piem-tool-block-error))
      (should (eq (overlay-get ov2 'face) 'piem-tool-block-error)))))

(ert-deftest piem-test-toolcall-second-preview-upgrades-on-execution-start ()
...
(ert-deftest piem-test-get-tail-lines-skips-blank-lines ()
...
a slot, so downstream consumers that skip blanks still get N content lines."
  ...
  (let* ((content "line1\nline2\nline3\n\nline4\nline5")
         (result (piem--get-tail-lines content 3)))
  ...
  (let* ((content "line1\nline2\n\n")
         (result (piem--get-tail-lines content 2)))
    (should (equal (car result) "line1\nline2\n\n"))
    (should (eq (cdr result) nil))))

(ert-deftest piem-test-get-tail-lines-empty ()
  "Get-tail-lines handles empty content."
  (let ((result (piem--get-tail-lines "" 5)))
    (should (equal (car result) ""))
    (should (eq (cdr result) nil))))

(ert-deftest piem-test-get-tail-lines-single-line ()
  "Get-tail-lines handles single line content."
  (let ((result (piem--get-tail-lines "just one line" 5)))
    (should (equal (car result) "just one line"))
    (should (eq (cdr result) nil))))

(ert-deftest piem-test-get-tail-lines-zero-lines ()
  "Requesting zero lines returns empty tail without errors."
  (let ((result (piem--get-tail-lines "line1\nline2" 0)))
    (should (equal (car result) ""))
    (should (eq (cdr result) t))))

;;; Fontify Exclusion Helpers

(ert-deftest piem-test-font-lock-ensure-excluding-property-splits-ranges ()
  "Font-lock helper should call only contiguous non-excluded ranges."
  (with-temp-buffer
    (insert "aaaBBBcccDDDeee")
    (put-text-property 4 7 'piem-no-fontify t)
    (put-text-property 10 13 'piem-no-fontify t)
    (let ((calls nil))
      (cl-letf (((symbol-function 'font-lock-ensure)
                 (lambda (start end)
                   (push (cons start end) calls))))
        (piem--font-lock-ensure-excluding-property
         (point-min) (point-max) 'piem-no-fontify))
      (should (equal (nreverse calls)
                     '((1 . 4) (7 . 10) (13 . 16)))))))

(ert-deftest piem-test-font-lock-ensure-excluding-property-excludes-any-non-nil ()
  "Font-lock helper should treat any non-nil PROP value as excluded."
  (with-temp-buffer
    (insert "aaaBBBccc")
    (put-text-property 4 7 'piem-no-fontify :details)
    (let ((calls nil))
      (cl-letf (((symbol-function 'font-lock-ensure)
                 (lambda (start end)
                   (push (cons start end) calls))))
...
(ert-deftest piem-test-font-lock-ensure-excluding-property-fontifies-large-ranges ()
...
(ert-deftest piem-test-font-lock-ensure-excluding-property-error-silent-without-debug ()
...
(ert-deftest piem-test-font-lock-ensure-excluding-property-error-logs-in-debug ()
...
(ert-deftest piem-test-font-lock-ensure-excluding-property-stops-after-first-error ()
...
(ert-deftest piem-test-font-lock-ensure-excluding-property-swallows-errors ()
...
(ert-deftest piem-test-extract-text-from-content-single-block ()
  "Extract-text-from-content handles single text block efficiently."
  (let ((blocks [(:type "text" :text "hello world")]))
    (should (equal (piem--extract-text-from-content blocks)
                   "hello world"))))

(ert-deftest piem-test-extract-text-from-content-multiple-blocks ()
...
(ert-deftest piem-test-extract-text-from-content-empty ()
  "Extract-text-from-content handles empty input."
  (should (equal (piem--extract-text-from-content []) ""))
  (should (equal (piem--extract-text-from-content nil) "")))

(ert-deftest piem-test-tool-update-replaced-by-end ()
...
(ert-deftest piem-test-tool-update-preserves-multiline-command-header ()
...
Commands with embedded newlines should not have any lines deleted."
  (with-temp-buffer
    ...
    (let ((multiline-cmd "echo 'line1'\necho 'line2'"))
      (piem--display-tool-start "bash" `(:command ,multiline-cmd))
...
(ert-deftest piem-test-tool-end-preserves-multiline-command-header ()
  ...
  (with-temp-buffer
    ...
    (let ((multiline-cmd "echo 'first'\necho 'second'\necho 'third'"))
      ...
      ;; Then end the tool
      (piem--display-tool-end "bash" `(:command ,multiline-cmd)
...
(ert-deftest piem-test-display-handler-handles-thinking-delta ()
...
(ert-deftest piem-test-activity-phase-thinking-on-agent-start ()
  "Activity phase becomes thinking on agent_start."
  (with-temp-buffer
    (piem-chat-mode)
    (setq piem--activity-phase "idle")
    (piem--handle-display-event '(:type "agent_start"))
    (should (equal piem--activity-phase "thinking"))))

(ert-deftest piem-test-activity-phase-replying-on-text-delta ()
...
(ert-deftest piem-test-activity-phase-running-on-toolcall-start ()
  "Activity phase becomes running when tool call generation starts."
  (with-temp-buffer
    (piem-chat-mode)
    (setq piem--activity-phase "thinking")
    (piem--handle-display-event
     '(:type "message_update"
       :assistantMessageEvent (:type "toolcall_start" :contentIndex 0)
       :message (:role "assistant"
                 :content [(:type "toolCall"
                            ...
                            :name "read"
                            :arguments (:path "/tmp/file.txt"))])))
    (should (equal piem--activity-phase "running"))))

(ert-deftest piem-test-activity-phase-running-on-tool-start ()
...
(ert-deftest piem-test-activity-phase-thinking-on-tool-end ()
...
(ert-deftest piem-test-activity-phase-compact-on-compaction ()
  ...
  (with-temp-buffer
    ...
    (should (equal piem--activity-phase "compact"))))

(ert-deftest piem-test-activity-phase-idle-on-agent-end ()
...
(ert-deftest piem-test-activity-phase-idle-on-compaction-end ()
  "Activity phase becomes idle on compaction_end without retry."
  (with-temp-buffer
    (piem-chat-mode)
    (setq piem--activity-phase "compact")
    (piem--handle-display-event
     '(:type "compaction_end" :aborted t :result nil))
    (should (equal piem--activity-phase "idle"))))

(ert-deftest piem-test-activity-phase-thinking-on-compaction-end-will-retry ()
...
(ert-deftest piem-test-display-compaction-result-shows-header-tokens-summary ()
  "piem--display-compaction-result shows header, token count, and summary."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-compaction-result 50000 "Key points from discussion.")
    ;; Should have Compaction header
    (should (string-match-p "Compaction" (buffer-string)))
...
(ert-deftest piem-test-display-compaction-result-with-timestamp ()
  "piem--display-compaction-result includes timestamp when provided."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((timestamp (seconds-to-time 1704067200))) ; 2024-01-01 00:00 UTC
      (piem--display-compaction-result 30000 "Summary text." timestamp))
...
(ert-deftest piem-test-display-compaction-result-shows-markdown ()
  "piem--display-compaction-result displays markdown summary as-is."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-compaction-result 10000 "**Bold** and `code`")
    ;; Markdown stays as markdown
    (should (string-match-p "\\*\\*Bold\\*\\*" (buffer-string)))
    (should (string-match-p "`code`" (buffer-string)))))

(ert-deftest piem-test-display-handler-handles-compaction-start ()
...
(ert-deftest piem-test-display-compaction-events-leave-status-to-core ()
  ...
  (dolist (event '((:type "compaction_start" :reason "threshold")
    ...
    (with-temp-buffer
      (piem-chat-mode)
      (setq piem--status 'idle)
      (cl-letf (((symbol-function 'piem--update-state-from-event)
                 (lambda (_event)
                   (setq piem--status 'core-owned))))
        (piem--handle-display-event event))
      (should (eq piem--status 'core-owned)))))

(ert-deftest piem-test-compaction-start-message-does-not-advertise-cancel ()
...
(ert-deftest piem-test-compaction-end-will-retry-keeps-session-busy ()
...
(ert-deftest piem-test-display-handler-handles-compaction-end ()
...
(ert-deftest piem-test-display-handler-shows-compaction-failure ()
  ...
  (with-temp-buffer
    ...
    (let ((sent-text nil)
          (shown-message nil))
      (setq piem--status 'compacting)
      (setq piem--followup-queue '("queued after recovery"))
      (cl-letf (((symbol-function 'piem--send-prompt)
                 (lambda (text &optional on-success &rest _)
                   (setq sent-text text)
                   (when on-success (funcall on-success))))
      ...
      (should (equal piem--followup-queue '("queued after recovery")))
      (should (null sent-text)))))

(ert-deftest piem-test-display-handler-handles-compaction-aborted ()
...
(ert-deftest piem-test-thinking-rendered-as-blockquote ()
  ...
  (let ((piem-thinking-display 'visible))
    (with-temp-buffer
      ...
      (piem--handle-display-event '(:type "message_end" :message (:role "assistant")))
...
(ert-deftest piem-test-thinking-blockquote-has-face ()
...
(ert-deftest piem-test-thinking-multiline-blockquote ()
...
(ert-deftest piem-test-agent-end-clears-thinking-marker-buffer ()
  "agent_end should detach thinking markers and clear thinking stream state."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (piem--display-thinking-start)
    (let ((marker piem--thinking-marker)
          (start-marker piem--thinking-start-marker))
      (should (stringp piem--thinking-raw))
      (piem--display-agent-end)
      (should-not piem--thinking-marker)
      (should-not piem--thinking-start-marker)
      (should-not piem--thinking-raw)
      (should-not (marker-buffer marker))
      (should-not (marker-buffer start-marker)))))

(defun piem-test--assert-message-start-clears-thinking-state (event)
...
(ert-deftest piem-test-message-start-clears-previous-thinking-marker ()
  "message_start should clear stale thinking markers and stream state."
  (piem-test--assert-message-start-clears-thinking-state
   '(:type "message_start" :message (:role "assistant"))))

(ert-deftest piem-test-message-start-user-clears-previous-thinking-marker ()
  "message_start for user should also clear stale thinking state."
  (piem-test--assert-message-start-clears-thinking-state
   '(:type "message_start"
     :message (:role "user" :content [(:type "text" :text "hi")]))))

(ert-deftest piem-test-message-start-custom-clears-previous-thinking-marker ()
  "message_start for custom messages should clear stale thinking state."
  (piem-test--assert-message-start-clears-thinking-state
   '(:type "message_start"
     :message (:role "custom" :display t :content "done"))))

(ert-deftest piem-test-read-tool-gets-syntax-highlighting ()
...
(ert-deftest piem-test-generic-tool-with-path-uses-path-language ()
...
(ert-deftest piem-test-markdown-fence-delimiter-defaults-to-backticks ()
  "Fence delimiter should use backticks when content has no backtick fence."
  (should (equal "```"
                 (piem--markdown-fence-delimiter "plain text"))))

(ert-deftest piem-test-markdown-fence-delimiter-avoids-tilde-collisions ()
  "Fence delimiter should exceed the longest tilde run in content."
  (let ((content "before\n~~~~\n```bash\necho hi\n```\nafter"))
    (should (equal "~~~~~"
                   (piem--markdown-fence-delimiter content)))))

(ert-deftest piem-test-wrap-in-src-block-uses-safe-fence ()
  "Wrapped source blocks should use a delimiter that cannot close content."
  (let ((wrapped (piem--wrap-in-src-block
                  "```elisp\n(message \"hi\")\n```\n~~~~"
                  "markdown")))
    (should (string-prefix-p "~~~~~markdown\n" wrapped))
    (should (string-suffix-p "\n~~~~~" wrapped))))

(ert-deftest piem-test-read-tool-fences-handle-nested-backticks ()
...
Inner backtick fences in read output must not affect later wrappers."
  (with-temp-buffer
    ...
    (piem--display-tool-start "read" '(:path "/tmp/test.md"))
    (piem--display-tool-end
...
(ert-deftest piem-test-thinking-markdown-after-collapsed-read ()
...
(ert-deftest piem-test-thinking-delta-after-toolcall-start-stays-blockquote ()
...
(ert-deftest piem-test-write-tool-gets-syntax-highlighting ()
...
(ert-deftest piem-test-streaming-fires-modification-hooks ()
  ...
  (let ((hook-called nil))
    (cl-flet ((test-hook (beg end len) (setq hook-called t)))
      (with-temp-buffer
        ...
        (piem--display-message-delta "Test delta")
        (should hook-called)))))

(ert-deftest piem-test-thinking-delta-fires-modification-hooks ()
  ...
  (let ((hook-called nil))
    (cl-flet ((test-hook (beg end len) (setq hook-called t)))
      (with-temp-buffer
        ...
        (add-hook 'after-change-functions #'test-hook nil t)
        (setq hook-called nil)
        (piem--display-thinking-delta "Test thinking")
        (should hook-called)))))

(ert-deftest piem-test-tool-update-fires-modification-hooks ()
...
(ert-deftest piem-test-streaming-fontify-does-not-bleed-into-tool ()
...
(ert-deftest piem-test-tool-header-no-markdown-damage ()
...
invisible, or markdown face properties."
  (with-temp-buffer
    ...
    (piem--handle-display-event
     '(:type "message_update"
       :assistantMessageEvent (:type "text_delta" :delta "Running.\n")))
    (piem--display-tool-start "bash" '(:command "echo"))
...
(ert-deftest piem-test-restore-tool-properties-restores-all-live-tool-headers ()
...
(ert-deftest piem-test-restore-tool-properties-repairs-finalized-tool-headers ()
...
(ert-deftest piem-test-normal-insert-does-call-hooks ()
...
(ert-deftest piem-test-tool-update-header-skips-when-unchanged ()
...
events where the header text hasn't changed."
  (piem-test--with-toolcall "read" '(:path "/tmp/foo.py")
    (let ((tick-before (buffer-modified-tick)))
      ;; Send delta with same args — header should be identical
      (piem--display-tool-update-header "read" '(:path "/tmp/foo.py"))
      (should (= (buffer-modified-tick) tick-before)))))

;;;; Built-in Slash Command Dispatch

(ert-deftest piem-test-dispatch-builtin-compact ()
  "Dispatching /compact calls piem-compact with no args."
  (let (called-with)
    (cl-letf (((symbol-function 'piem-compact)
               (lambda (&optional args) (setq called-with (list 'compact args)))))
      (should (piem--dispatch-builtin-command "/compact"))
      (should (equal called-with '(compact nil))))))

(ert-deftest piem-test-dispatch-builtin-compact-with-args ()
  "Dispatching /compact with args passes them through."
  (let (called-with)
    (cl-letf (((symbol-function 'piem-compact)
               (lambda (&optional args) (setq called-with (list 'compact args)))))
      (should (piem--dispatch-builtin-command "/compact keep API details"))
      (should (equal called-with '(compact "keep API details"))))))

(ert-deftest piem-test-dispatch-builtin-new ()
  "Dispatching /new calls piem-new-session."
  (let (called)
    (cl-letf (((symbol-function 'piem-new-session)
               (lambda () (setq called t))))
      (should (piem--dispatch-builtin-command "/new"))
      (should called))))

(ert-deftest piem-test-dispatch-builtin-model ()
  "Dispatching /model calls piem-select-model with no args."
  (let (called-with)
    (cl-letf (((symbol-function 'piem-select-model)
               (lambda (&optional input) (setq called-with (list 'model input)))))
      (should (piem--dispatch-builtin-command "/model"))
      (should (equal called-with '(model nil))))))

(ert-deftest piem-test-dispatch-builtin-model-with-search ()
  "Dispatching /model opus passes search term as initial-input."
  (let (called-with)
    (cl-letf (((symbol-function 'piem-select-model)
               (lambda (&optional input) (setq called-with (list 'model input)))))
      (should (piem--dispatch-builtin-command "/model opus"))
      (should (equal called-with '(model "opus"))))))

(ert-deftest piem-test-dispatch-builtin-name-with-arg ()
  "Dispatching /name foo calls piem-set-session-name with arg."
  (let (called-with)
    (cl-letf (((symbol-function 'piem-set-session-name)
               (lambda (name) (setq called-with name))))
      (should (piem--dispatch-builtin-command "/name my-session"))
      (should (equal called-with "my-session")))))

(ert-deftest piem-test-dispatch-builtin-name-no-arg-prompts ()
  ...
  (let (interactive-called)
    (cl-letf (((symbol-function 'call-interactively)
      ...
      (should (piem--dispatch-builtin-command "/name"))
      (should (eq interactive-called 'piem-set-session-name)))))

(ert-deftest piem-test-dispatch-builtin-export-with-path ()
  "Dispatching /export /tmp/out.html passes path to handler."
  (let (called-with)
    (cl-letf (((symbol-function 'piem-export-html)
               (lambda (&optional path) (setq called-with path))))
      (should (piem--dispatch-builtin-command "/export /tmp/out.html"))
      (should (equal called-with "/tmp/out.html")))))

(ert-deftest piem-test-dispatch-builtin-export-no-path ()
  "Dispatching /export with no path passes nil."
  (let (called-with)
    (cl-letf (((symbol-function 'piem-export-html)
               (lambda (&optional path) (setq called-with (list 'called path)))))
      (should (piem--dispatch-builtin-command "/export"))
      (should (equal called-with '(called nil))))))

(ert-deftest piem-test-dispatch-returns-nil-for-unknown ()
  "Dispatching unknown /command returns nil (falls through to RPC)."
  (should-not (piem--dispatch-builtin-command "/greet"))
  (should-not (piem--dispatch-builtin-command "/skill:test")))

(ert-deftest piem-test-dispatch-returns-nil-for-non-slash ()
  "Dispatching non-slash text returns nil."
  (should-not (piem--dispatch-builtin-command "hello")))

(ert-deftest piem-test-prepare-and-send-dispatches-builtin ()
  ...
  (let (new-called prompt-sent)
    (cl-letf (((symbol-function 'piem-new-session)
      ...
      (with-temp-buffer
        (piem-chat-mode)
        (piem--prepare-and-send "/new")))
    (should new-called)
    (should-not prompt-sent)))

(ert-deftest piem-test-input-resume-opens-session-browser-locally ()
    ...
    (unwind-protect
        (progn
          ...
          (with-current-buffer input-buf
            ...
            (setq piem--chat-buffer chat-buf)
            (insert "/resume")
            (cl-letf (((symbol-function 'piem-session-browser)
                       (lambda (&rest _)
...
(ert-deftest piem-test-prepare-and-send-passes-through-extension ()
  ...
  (let (prompt-sent)
    (cl-letf (((symbol-function 'piem--send-prompt)
      ...
      (with-temp-buffer
        (piem-chat-mode)
        (piem--prepare-and-send "/my-extension arg")))
    (should (equal prompt-sent "/my-extension arg"))))

(ert-deftest piem-test-agent-end-updates-hot-tail-boundary ()
  "agent_end recomputes the hot-tail boundary after finishing a turn."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((called nil))
      (cl-letf (((symbol-function 'piem--update-hot-tail-boundary)
                 (lambda ()
                   (setq called t))))
        (piem--handle-display-event '(:type "agent_end"))
        (should called)))))

(ert-deftest piem-test-display-session-history-updates-hot-tail-boundary ()
...
(provide 'piem-render-test)
;;; piem-render-test.el ends here
