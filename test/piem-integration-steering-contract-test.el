;;; piem-integration-steering-contract-test.el --- Shared steering contracts -*- lexical-binding: t; -*-
...
(piem-integration-deftest
  ...
  (let ((got-agent-start nil)
    ...
    (let ((prompt-response (piem--rpc-sync
                            ...
                            `(:type "prompt"
                              :message
                              ,piem-integration--prompt-steering-initial-message)
    ...
    (let ((steer-response (piem--rpc-sync
                           proc
                           `(:type "steer"
                             :message
                             ,piem-integration--prompt-steering-queued-message)
                           piem-test-rpc-timeout)))
      (should steer-response)
      (should (eq (plist-get steer-response :success) t))
      (should (equal (plist-get steer-response :command) "steer")))
    (with-timeout (piem-test-integration-timeout
    ...
    (let ((queued-msg (seq-find
                       ...
                       user-message-events)))
      (should queued-msg))
    (let* ((state (piem--rpc-sync proc '(:type "get_state")
                                             piem-test-rpc-timeout))
           (data (plist-get state :data)))
      (should (eq (plist-get data :isStreaming) :false)))))

(provide 'piem-integration-steering-contract-test)
;;; piem-integration-steering-contract-test.el ends here
