;;; piem-integration-prompt-contract-test.el --- Shared prompt contracts -*- lexical-binding: t; -*-
...
(piem-integration-deftest
  ...
  (let* ((initial-state (piem--rpc-sync proc '(:type "get_state")
    ...
    (setq prompt-response
          (piem--rpc-sync
           proc
           `(:type "prompt"
             :message ,piem-integration--prompt-lifecycle-message)
           piem-test-rpc-timeout))
  ...
  (let ((got-agent-start nil)
    ...
    (setq prompt-response
          (piem--rpc-sync
           proc
           `(:type "prompt"
             :message ,piem-integration--prompt-abort-message)
           piem-test-rpc-timeout))
    (should (eq (plist-get prompt-response :success) t))
...
(provide 'piem-integration-prompt-contract-test)
;;; piem-integration-prompt-contract-test.el ends here
