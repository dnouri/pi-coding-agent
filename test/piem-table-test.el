;;; piem-table-test.el --- Tests for piem-table -*- lexical-binding: t; -*-
...
(ert-deftest piem-test-decorate-tables-preserves-raw-buffer-text ()
...
(ert-deftest piem-test-decorate-tables-is-idempotent ()
...
(ert-deftest piem-test-decorate-tables-skips-fenced-table ()
...
(ert-deftest piem-test-decorate-tables-only-outside-fence ()
...
(ert-deftest piem-test-table-overlays-have-no-font-faces ()
...
font configuration."
  (with-temp-buffer
    ...
    (font-lock-ensure)
    (piem--decorate-tables-in-region (point-min) (point-max) 60)
    (let ((ovs (seq-filter
                (lambda (ov) (overlay-get ov 'piem-table-display))
                (overlays-in (point-min) (point-max)))))
      (should (>= (length ovs) 1))
...
(ert-deftest piem-test-neutralize-fonts-strips-family ()
  ...
  (let* ((str (propertize "hello" 'face '(:family "Courier" :foreground "red")))
         (result (piem--neutralize-fonts str)))
    (should (equal (get-text-property 0 'face result) '(:foreground "red")))))

(ert-deftest piem-test-neutralize-fonts-resolves-symbolic-face ()
...
(ert-deftest piem-test-neutralize-fonts-preserves-plain-text ()
  "Text without face properties passes through unchanged."
  (let ((result (piem--neutralize-fonts "plain")))
    (should (equal result "plain"))
    (should-not (get-text-property 0 'face result))))

(ert-deftest piem-test-neutralize-fonts-multi-span ()
...
(ert-deftest piem-test-markdown-visible-string-falls-back-on-font-lock-error ()
  ...
  (unwind-protect
      (let ((debug-on-error nil))
        (cl-letf (((symbol-function 'font-lock-ensure)
          ...
          (should (equal (piem--markdown-visible-string "`0xAF`")
                         "`0xAF`"))))
    (piem--cleanup-visible-string-buffer)))

(ert-deftest piem-test-display-user-message-decorates-table ()
...
(ert-deftest piem-test-render-complete-message-decorates-table ()
...
(ert-deftest piem-test-render-history-text-decorates-table ()
...
(ert-deftest piem-test-render-history-text-survives-font-lock-error ()
...
(ert-deftest piem-test-display-compaction-decorates-table ()
  "Compaction summary with tables gets display-only decoration."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-compaction-result
     50000 "| Key | Value |\n|-----|-------|\n| ctx | 50k |")
    (let ((ovs (seq-filter
                (lambda (ov) (overlay-get ov 'piem-table-display))
                (overlays-in (point-min) (point-max)))))
...
(ert-deftest piem-test-custom-message-decorates-table ()
...
(ert-deftest piem-test-decorate-table-preserves-trailing-newline ()
  "Display string preserves trailing newline from the raw table."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((inhibit-read-only t))
      (insert "before\n\n" piem-test--wide-table "\nafter\n"))
    (font-lock-ensure)
    (piem--decorate-tables-in-region (point-min) (point-max) 40)
    (let* ((ovs (seq-filter
                 (lambda (ov) (overlay-get ov 'piem-table-display))
...
(ert-deftest piem-test-table-decoration-copy-returns-raw ()
...
(ert-deftest piem-test-table-line-mapping-1to1 ()
  "Non-wrapping table maps each raw line to one wrapped line."
  (let* ((raw-lines '("| A | B |" "|---|---|" "| 1 | 2 |"))
         (wrap-lines '("| A | B |" "| - | - |" "| 1 | 2 |"))
         (mapping (piem--table-line-mapping raw-lines wrap-lines)))
...
(ert-deftest piem-test-table-line-mapping-data-wraps ()
  ...
  (let* ((raw "| Name | Desc |\n|------|------|\n| Auth | OAuth2 with refresh tokens and renewal |")
         ...
         (mapping (piem--table-line-mapping
                   (split-string raw "\n")
...
(ert-deftest piem-test-table-line-mapping-header-wraps ()
...
(ert-deftest piem-test-table-line-mapping-multiple-data-rows ()
...
(ert-deftest piem-test-table-line-mapping-nil-without-separator ()
  "Mapping returns nil when no separator is found."
  (let ((mapping (piem--table-line-mapping
                  '("| A |" "| B |")
                  '("| A |" "| B |"))))
    (should (null mapping))))

(ert-deftest piem-test-decorate-table-creates-per-line-overlays ()
    ...
    (let ((raw-lines (split-string
      ...
      (dolist (pos table-line-positions)
        (let ((ovs-at (seq-filter
                       (lambda (ov)
                         (and (overlay-get ov 'piem-table-display)
                              (<= (overlay-start ov) pos)
                              (> (overlay-end ov) pos)))
                       (overlays-in (1- pos) (1+ pos)))))
          (should (= (length ovs-at) 1)))))))

(ert-deftest piem-test-decorate-table-single-line-copy-returns-raw ()
...
(ert-deftest piem-test-decorate-table-backtick-cells-aligned ()
...
(ert-deftest piem-test-decorate-table-prettifies-visible-separators ()
  ...
  (with-temp-buffer
    ...
    (let ((all-display (mapconcat #'identity
                                  (piem-test--table-overlay-displays-in-region
                                   (point-min) (point-max))
                                  "\n")))
...
(ert-deftest piem-test-decorate-table-preserves-pipes-when-prettify-off ()
  "With prettify disabled, rendered tables use standard markdown pipes."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((piem-prettify-tables nil)
          (inhibit-read-only t))
      (insert "| Name | Value |\n|------|-------|\n| Alpha | Beta |\n")
      (font-lock-ensure)
      (piem--decorate-tables-in-region (point-min) (point-max) 40)
...
(ert-deftest piem-test-table-overlay-suppresses-buffer-face ()
...
formatting (bold, italic) in the display string's text properties."
  (with-temp-buffer
    ...
    (let ((ovs (sort (seq-filter
      ...
      (should (piem-test--string-has-face-attr-p
               (overlay-get (nth 2 ovs) 'display) :weight 'bold)))))

(ert-deftest piem-test-decorate-table-hides-inline-markup-in-display ()
  ...
  (with-temp-buffer
    ...
    (font-lock-ensure)
    (piem--decorate-tables-in-region (point-min) (point-max) 30)
...
(ert-deftest piem-test-table-cell-render-function-is-used ()
  "A non-nil `piem-table-cell-render-function' renders cells.
...
fontification would never produce."
  (let ((piem-table-cell-render-function
...
(ert-deftest piem-test-table-cell-render-function-faces-survive ()
  "Faces applied by a custom render function reach the display string.
`piem--neutralize-fonts' resolves them to attribute plists,
  ...
  (let ((piem-table-cell-render-function
    ...
    (with-temp-buffer
      ...
      (let ((display (mapconcat #'identity
                                ...
                                "\n")))
        (should (string-match-p "alice" display))
        (should (piem-test--string-has-face-attr-p display :weight 'bold))))))

(ert-deftest piem-test-decorate-table-keeps-blockquote-prefix ()
  ...
  (with-temp-buffer
    ...
    (font-lock-ensure)
    (piem--decorate-tables-in-region (point-min) (point-max) 40)
    (dolist (display (piem-test--table-overlay-displays-in-region
                      (point-min) (point-max)))
      (dolist (line (split-string (string-trim-right display "\n+") "\n"))
        (should (string-prefix-p "> " line))))))

(ert-deftest piem-test-decorate-table-keeps-indentation-prefix ()
...
(ert-deftest piem-test-decorate-table-empty-row-follows-treesit-truth ()
...
what the parser recognizes as a `pipe_table'."
  (with-temp-buffer
    ...
    (font-lock-ensure)
    (piem--decorate-tables-in-region (point-min) (point-max) 20)
    (should (= (piem-test--table-overlay-count) 0))))

(ert-deftest piem-test-decorate-table-keeps-dash-only-data-row-visible ()
  ...
  (with-temp-buffer
    ...
    (font-lock-ensure)
    (piem--decorate-tables-in-region (point-min) (point-max) 20)
    (let ((dash-row-display (nth 2 (piem-test--table-overlay-displays-in-region
                                    (point-min) (point-max)))))
      (should (string-match-p "----" dash-row-display)))))

;;; Per-line overlay interaction verification (Phase 4)

(ert-deftest piem-test-table-copy-mixed-selection-coherent ()
...
(ert-deftest piem-test-table-search-finds-cell-content ()
...
(ert-deftest piem-test-table-overlay-independent-of-tool-overlay ()
    ...
    (let ((inhibit-read-only t))
      ...
      (let ((tool-start (point)))
        ...
        (let ((ov (make-overlay tool-start (point))))
          (overlay-put ov 'piem-tool-overlay t))))
    (font-lock-ensure)
    (piem--decorate-tables-in-region (point-min) (point-max) 40)
    ;; Both overlay types exist
    (should (>= (piem-test--table-overlay-count) 1))
    (should (>= (length (seq-filter
                         (lambda (ov) (overlay-get ov 'piem-tool-overlay))
                         (overlays-in (point-min) (point-max))))
                1))
    ;; Remove table overlays
    (piem--remove-table-overlays (point-min) (point-max))
...
(defun piem-test--string-has-face-attr-p (str attr value)
...
(defun piem-test--table-overlay-count ()
  "Count table display overlays in the current buffer."
  (length (seq-filter
           (lambda (ov) (overlay-get ov 'piem-table-display))
           (overlays-in (point-min) (point-max)))))

(defun piem-test--table-overlay-displays-in-region (beg end)
...
(ert-deftest piem-test-streaming-table-no-decoration-without-newline ()
  "Streaming a partial table row without newline creates no table overlays."
  (with-temp-buffer
    (piem-chat-mode)
    (piem--display-agent-start)
    (piem--display-message-delta "| Feature | Status |")
    (should (= (piem-test--table-overlay-count) 0))))

(ert-deftest piem-test-streaming-table-no-decoration-header-sep-only ()
...
(ert-deftest piem-test-streaming-table-decorates-on-first-data-row ()
  ...
  (with-temp-buffer
    ...
    (piem--display-message-delta
     "| Feature | Status |\n|---------|--------|\n")
    (should (= (piem-test--table-overlay-count) 0))
    (piem--display-message-delta "| Auth | Done |\n")
    (should (>= (piem-test--table-overlay-count) 1))))

(ert-deftest piem-test-streaming-table-updates-on-later-rows ()
  ...
  (with-temp-buffer
    ...
    (piem--display-message-delta
     "| Feature | Status |\n|---------|--------|\n| Auth | Done |\n")
    (let ((count-after-first (piem-test--table-overlay-count)))
      (should (>= count-after-first 1))
      (piem--display-message-delta "| DB | WIP |\n")
      ;; More overlays now (4 lines instead of 3)
      (should (> (piem-test--table-overlay-count)
                 count-after-first)))))

(ert-deftest piem-test-streaming-table-raw-text-unchanged ()
...
(ert-deftest piem-test-streaming-table-fenced-ignored ()
...
(ert-deftest piem-test-streaming-table-text-end-finalizes ()
...
(ert-deftest piem-test-streaming-table-prose-after-table-preserves ()
  ...
  (with-temp-buffer
    ...
    (piem--display-message-delta
     "| Feature | Status |\n|---------|--------|\n| Auth | Done |\n")
    (let ((count-before-prose (piem-test--table-overlay-count)))
      (should (>= count-before-prose 1))
      (piem--display-message-delta "\nSome prose after the table.\n")
      ;; Table overlays should still exist
      (should (= (piem-test--table-overlay-count) count-before-prose)))))

(ert-deftest piem-test-streaming-table-second-table-preserves-first ()
  ...
  (with-temp-buffer
    ...
    (piem--display-message-delta
     "| A | B |\n|---|---|\n| 1 | 2 |\n")
    (let ((first-count (piem-test--table-overlay-count)))
      ...
      (should (> (piem-test--table-overlay-count) first-count)))))

(ert-deftest piem-test-streaming-table-message-end-safety-net ()
...
(ert-deftest piem-test-hot-tail-refresh-updates-hot-table-only ()
  ...
  (with-temp-buffer
    ...
    (let ((hot-start nil)
      ...
      (insert piem-test--wide-table)
      (font-lock-ensure)
      (piem--decorate-tables-in-region (point-min) (point-max) 80)
      (move-marker piem--hot-tail-start hot-start)
      (let ((cold-before (piem-test--table-overlay-displays-in-region
                          (point-min) hot-start))
            (hot-before (piem-test--table-overlay-displays-in-region
                         hot-start (point-max))))
        (piem--refresh-hot-tail-tables 40)
...
(ert-deftest piem-test-hot-tail-refresh-skips-height-only-change ()
  "Window configuration changes without a width change do not refresh tables."
  (with-temp-buffer
    (piem-chat-mode)
    (setq piem--last-table-display-width 80)
    (let ((called nil))
      (cl-letf (((symbol-function 'piem--chat-window-width)
                 (lambda () 80))
                ((symbol-function 'piem--refresh-hot-tail-tables)
                 (lambda (width)
                   (setq called width))))
        (piem--maybe-refresh-hot-tail-tables)
        (should-not called)))))

(ert-deftest piem-test-hot-tail-refresh-runs-on-width-change ()
  ...
  (with-temp-buffer
    ...
    (let ((called nil))
      (cl-letf (((symbol-function 'piem--chat-window-width)
        ...
        (piem--maybe-refresh-hot-tail-tables)
        (should (= called 40))
        (should (= piem--last-table-display-width 40))))))

(ert-deftest piem-test-hot-tail-refresh-skips-incomplete-streaming-table ()
  ...
  (with-temp-buffer
    ...
    (piem--display-message-delta
     "| Feature | Status |\n|---------|--------|\n")
    (move-marker piem--hot-tail-start (point-min))
    (piem--refresh-hot-tail-tables 40)
    (should (= (piem-test--table-overlay-count) 0))
    (piem--display-message-delta "| Auth | Done |\n")
    (should (>= (piem-test--table-overlay-count) 1))))

(ert-deftest piem-test-chat-buffer-hidden-p-sees-visible-window-on-other-frame ()
  ...
  (with-temp-buffer
    (let ((noninteractive nil)
      ...
      (cl-letf (((symbol-function 'get-buffer-window)
        ...
        (should-not (piem--chat-buffer-hidden-p))
        (should (equal (nreverse calls) '(nil visible)))))))

(ert-deftest piem-test-chat-buffer-hidden-p-returns-nil-in-batch-without-window ()
...
(ert-deftest piem-test-chat-window-width-excludes-fringe-columns ()
  ...
  (with-temp-buffer
    (let ((fake-window (selected-window)))
      (cl-letf (((symbol-function 'get-buffer-window)
        ...
        (should (= (piem--chat-window-width) 76))))))

(ert-deftest piem-test-chat-window-width-falls-back-to-visible-window-on-other-frame ()
      ...
      (cl-letf (((symbol-function 'get-buffer-window)
                ...
                ((symbol-function 'window-max-chars-per-line)
                 (lambda (window &optional _face)
                   ...
                   64)))
        (should (= (piem--chat-window-width) 64))
        (should (equal (nreverse calls) '(nil visible)))))))

(ert-deftest piem-test-chat-window-width-prefers-selected-frame-window ()
...
(ert-deftest piem-test-toggle-on-table-reveals-raw ()
  ...
  (with-temp-buffer
    ...
    (let ((inhibit-read-only t))
      (insert piem-test--wide-table))
    (font-lock-ensure)
    (piem-test--decorate-all-tables)
    (should (>= (length (piem-test--display-overlays)) 1))
    (should (null (piem-test--raw-overlays)))
    (goto-char (point-min))
    (piem-toggle-table-pretty)
    (should (null (piem-test--display-overlays)))
    (should (= 1 (length (piem-test--raw-overlays))))))

(ert-deftest piem-test-toggle-on-table-restores-pretty ()
  ...
  (with-temp-buffer
    ...
    (piem-test--decorate-all-tables)
    (goto-char (point-min))
    (piem-toggle-table-pretty) ; pretty -> raw
    (should (null (piem-test--display-overlays)))
    (piem-toggle-table-pretty) ; raw -> pretty
    (should (>= (length (piem-test--display-overlays)) 1))
    (should (null (piem-test--raw-overlays)))))

(ert-deftest piem-test-toggle-off-table-toggles-all ()
  "Point off-table toggles every table in the buffer."
  (with-temp-buffer
    (piem-chat-mode)
    (let ((inhibit-read-only t))
      (insert piem-test--two-tables))
    (font-lock-ensure)
    (piem-test--decorate-all-tables)
    (should (= 2 (length (piem--treesit-table-regions
                          (point-min) (point-max)))))
    ;; Point after the last table (off-table).
    (goto-char (point-max))
    (piem-toggle-table-pretty) ; all pretty -> all raw
    (should (null (piem-test--display-overlays)))
    (should (= 2 (length (piem-test--raw-overlays))))
    (piem-toggle-table-pretty) ; all raw -> all pretty
    (should (>= (length (piem-test--display-overlays)) 2))
    (should (null (piem-test--raw-overlays)))))

(ert-deftest piem-test-toggle-c-u-forces-pretty-on-all ()
  ...
  (with-temp-buffer
    ...
    (piem-toggle-table-pretty '(4))
    (should (>= (length (piem-test--display-overlays)) 2))
    (should (null (piem-test--raw-overlays)))))

(ert-deftest piem-test-toggle-c-u-c-u-forces-raw-on-all ()
  ...
  (with-temp-buffer
    ...
    (piem-test--decorate-all-tables)
    (should (>= (length (piem-test--display-overlays)) 2))
    (piem-toggle-table-pretty '(16))
    (should (null (piem-test--display-overlays)))
    (should (= 2 (length (piem-test--raw-overlays))))))

(ert-deftest piem-test-toggle-region-toggles-tables-in-region ()
  ...
  (with-temp-buffer
    ...
    (let ((transient-mark-mode t))
      ...
      (let ((raw-start (overlay-start
                        (car (piem-test--raw-overlays)))))
        (should (> raw-start (point-min))))
      (should (>= (length (piem-test--display-overlays)) 1)))))

(ert-deftest piem-test-toggle-raw-survives-redecoration ()
...
(ert-deftest piem-test-toggle-preserves-buffer-text ()
  ...
  (with-temp-buffer
    ...
    (let ((before (buffer-string)))
      (piem-test--decorate-all-tables)
      (piem-toggle-table-pretty)        ; all pretty -> all raw
      (piem-toggle-table-pretty '(4))   ; force pretty
      (piem-toggle-table-pretty '(16))  ; force raw
      (piem-test--decorate-all-tables)  ; resize path
      (should (equal before (buffer-string))))))

(provide 'piem-table-test)
;;; piem-table-test.el ends here
