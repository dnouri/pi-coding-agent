;;; piem-build-test.el --- Tests for batch build helpers -*- lexical-binding: t; -*-
...
(defconst piem-test-build--repo-root
  (expand-file-name ".." (file-name-directory load-file-name))
  "Repository root used by build script tests.")

(load (expand-file-name "scripts/piem-build.el"
                        piem-test-build--repo-root)
      t t)

(ert-deftest piem-test-build-package-requirements-follow-package-header ()
  ...
  (should (equal '((transient . (0 9 0))
                 ...
                 (piem-build-package-requirements))))

(ert-deftest piem-test-build-package-requirements-fallback-without-lm-package-requires ()
  "Read package requirements even when `lm-package-requires' is unavailable."
  (let ((piem-build-main-file
         (make-temp-file "piem-build" nil ".el"
...
;; Package-Requires: ((emacs \"29.1\") (transient \"0.9.0\"))\n")))
    (unwind-protect
        (cl-letf (((symbol-function 'lm-package-requires) nil))
          (should (equal '((transient . (0 9 0)))
                         (piem-build-package-requirements))))
      (delete-file piem-build-main-file))))

(ert-deftest piem-test-build-install-deps-installs-missing-or-outdated-packages ()
...
(ert-deftest piem-test-build-install-deps-errors-when-dependency-stays-missing ()
...
(ert-deftest piem-test-build-install-grammars-reports-ready-and-installed-counts ()
  ...
  (let ((available '(markdown))
    ...
    (cl-letf (((symbol-function 'treesit-language-available-p)
      ...
      (let ((result (piem-build-install-grammars '(markdown bash javascript))))
...
(ert-deftest piem-test-build-install-grammars-errors-when-a-grammar-fails ()
  ...
  (let* ((emacs (expand-file-name invocation-name invocation-directory))
         ...
         (exit-code
          (call-process emacs nil output-buffer nil
                        ...
                        (expand-file-name "scripts/piem-build.el"
                                          piem-test-build--repo-root)
    ...
    (unwind-protect
      ...
      (dolist (elc '("scripts/piem-build.elc"
                     "scripts/install-deps.elc"
                     "scripts/install-ts-grammars.elc"))
        (let ((path (expand-file-name elc piem-test-build--repo-root)))
          (when (file-exists-p path)
            (delete-file path)))))))

(ert-deftest piem-test-build-install-deps-script-delegates-to-helper ()
  "The dependency install wrapper should call the shared build helper."
  (let ((called nil))
    (cl-letf (((symbol-function 'piem-build-install-deps)
               (lambda (&rest _)
                 (setq called t)
                 t)))
...
(ert-deftest piem-test-build-install-ts-grammars-script-delegates-to-helper ()
  "The grammar install wrapper should call the shared build helper."
  (let ((called nil))
    (cl-letf (((symbol-function 'piem-build-install-grammars)
               (lambda (&rest _)
                 (setq called t)
                 t)))
...
(provide 'piem-build-test)
;;; piem-build-test.el ends here
