;;; piem-jsonl-test.el --- Tests for JSONL session module -*- lexical-binding: t; -*-

;;; Commentary:

;; Test suite for piem-jsonl.el: JSONL session reading, raw
...
;; implementation must provide them under the `piem--jsonl-'
;; prefix:
;;
;;   (piem--jsonl-extract-text CONTENT &optional MAX-LEN)
;;   (piem--jsonl-normalize-preview TEXT)
;;   (piem--jsonl-shorten-path PATH)
;;   (piem--jsonl-arg-number ARGS KEY)
...
(defconst piem-test--jsonl-session-file "browse-session.jsonl"
  "Golden session fixture for the jsonl module.")

(defun piem-test--jsonl-session-path ()
  "Return the absolute path of the golden session fixture."
  (expand-file-name piem-test--jsonl-session-file
                    piem-test--fixture-dir))

(defun piem-test--write-jsonl (path lines)
...
encode as their JSON counterparts."
  (with-temp-file path
    (dolist (line lines)
      (insert (json-encode (piem-test--jsonl-literalize line))
              "\n"))))

(defun piem-test--jsonl-normalize (data)
...
counterparts first."
  (cond
   ...
   ((eq data :false) 'false)
   ((vectorp data)
    (vconcat (mapcar #'piem-test--jsonl-normalize data)))
   ((consp data)
    (let (out)
      (while (consp data)
        (let ((key (pop data)))
          (push key out)
          (push (if (consp data)
                    (piem-test--jsonl-normalize (pop data))
                  nil)
                out)))
      (nreverse out)))
   (t data)))

(defun piem-test--jsonl-literalize (data)
...
values `json-encode' serializes as null, true, and false."
  (cond
   ...
   ((vectorp data)
    (vconcat (mapcar #'piem-test--jsonl-literalize data)))
   ((consp data)
    (let (out)
      (while (consp data)
        (let ((key (pop data)))
          (push key out)
          (push (if (consp data)
                    (piem-test--jsonl-literalize (pop data))
                  nil)
                out)))
      (nreverse out)))
   (t data)))

(defun piem-test--jsonl-canonicalize (data)
  "Canonicalize plist tree DATA through a JSON round-trip."
  (json-parse-string
   (json-encode (piem-test--jsonl-normalize data))
   :object-type 'plist))

(defun piem-test--jsonl-fixture-canonical (filename)
  "Read JSON fixture FILENAME and canonicalize it like actual results."
  (piem-test--jsonl-canonicalize
   (piem-test--read-json-fixture filename)))

(defun piem-test--jsonl-build-lines (lines)
  "Write LINES (header first) to a temp file and build the raw tree.
Returns the (:tree ... :leafId ...) plist from `build-tree'."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-build"))
         (path (expand-file-name "session.jsonl" dir))
         (session (progn (piem-test--write-jsonl path lines)
                         (piem-jsonl-read-file path))))
    (piem-jsonl-build-tree (plist-get session :entries))))

(defun piem-test--jsonl-project-lines (lines)
...
Returns the (:tree ... :leafId ...) plist from `project-tree'."
  (pcase-let ((`(,raw-tree ,raw-leaf)
               (pcase (piem-test--jsonl-build-lines lines)
                 ((map (:tree rt) (:leafId rl)) (list rt rl)))))
    (piem-jsonl-project-tree raw-tree raw-leaf)))

(defun piem-test--jsonl-find (tree id)
...
(defun piem-test--jsonl-find-raw (tree id)
...
(defconst piem-test--jsonl-header
...
(defun piem-test--jsonl-entry (type id parent second &rest payload)
...
(defun piem-test--jsonl-msg (id parent second message)
  "Return a message entry whose message plist is MESSAGE."
  (piem-test--jsonl-entry "message" id parent second
                                     :message message))

(defun piem-test--jsonl-asst (id parent second &rest props)
...
(ert-deftest piem-test-jsonl-build-tree-golden ()
  "build-tree over the golden session matches browse-raw.json."
  (let* ((session (piem-jsonl-read-file
                   (piem-test--jsonl-session-path)))
         (built (piem-jsonl-build-tree
                 (plist-get session :entries))))
    (should (equal (piem-test--jsonl-canonicalize built)
                   (piem-test--jsonl-fixture-canonical
                    "browse-raw.json")))))

(ert-deftest piem-test-jsonl-project-tree-golden ()
  "project-tree over browse-raw.json matches browse-projected.json."
  (let* ((raw (piem-test--read-json-fixture "browse-raw.json"))
         (result (piem-jsonl-project-tree
                  (plist-get raw :tree) (plist-get raw :leafId))))
    (should (equal (piem-test--jsonl-canonicalize result)
                   (piem-test--jsonl-fixture-canonical
                    "browse-projected.json")))))

(ert-deftest piem-test-jsonl-round-trip-golden ()
...
(ert-deftest piem-test-jsonl-project-session-file-golden ()
  ...
  (let* ((path (piem-test--jsonl-session-path))
         (direct (piem-jsonl-project-session-file path)))
    (should direct)
    (should (equal (piem-test--jsonl-canonicalize direct)
                   (piem-test--jsonl-fixture-canonical
                    "browse-projected.json"))))
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-psf"))
         (headerless (expand-file-name "headerless.jsonl" dir)))
    (piem-test--write-jsonl
     headerless
     (list (piem-test--jsonl-msg
            "h1" nil 0 '(:role "user" :content "decoy"))))
    (should-not (piem-jsonl-project-session-file headerless))
    (should-not (piem-jsonl-project-session-file
                 (expand-file-name "missing.jsonl" dir)))))

;;;; read-file

(ert-deftest piem-test-jsonl-read-file-missing ()
  "A nonexistent path reads as nil."
  (should-not (piem-jsonl-read-file
               "/nonexistent/pi-jsonl/no-such-file.jsonl")))

(ert-deftest piem-test-jsonl-read-file-empty-or-headerless ()
  "Empty files and files whose first nonblank line is not the session
header read as nil."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-bad"))
    ...
    (with-temp-file garbage
      (insert "{\"type\":\"message\",\"id\":\"g1\"\n"
              "this is not json\n"
              "\n"))
    (should-not (piem-jsonl-read-file empty))
    (should-not (piem-jsonl-read-file garbage))))

(ert-deftest piem-test-jsonl-read-file-header-only ()
  "A header-only session reads as an empty entry vector with nil leaf."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-hdr"))
         (path (expand-file-name "hdr.jsonl" dir))
         (data (progn (piem-test--write-jsonl
                       path (list piem-test--jsonl-header))
                      (piem-jsonl-read-file path))))
    (should data)
...
(ert-deftest piem-test-jsonl-read-file-fixture-shape ()
  "File order, leaf id, and session name for the golden session."
  (let* ((data (piem-jsonl-read-file
                (piem-test--jsonl-session-path)))
...
(ert-deftest piem-test-jsonl-read-file-skips-bad-lines ()
  "Malformed and blank lines are skipped silently."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-skip"))
         (path (expand-file-name "bad.jsonl" dir))
         (data (progn
                 (with-temp-file path
                   (insert (json-encode piem-test--jsonl-header) "\n"
                           "{not json at all\n"
                           "\n"
...
(ert-deftest piem-test-jsonl-session-header-first-nonblank-line-rule ()
...
read-session-info, and navigation-lines all return nil."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-line0"))
         (blank (expand-file-name "blank-first.jsonl" dir))
         (junk (expand-file-name "junk-first.jsonl" dir))
         (header-line (json-encode piem-test--jsonl-header))
    ...
    (let ((session (piem-jsonl-read-file blank)))
      (should session)
    ...
    (let ((info (piem-jsonl-read-session-info blank)))
      (should info)
    ...
    (let ((lines (piem-jsonl-navigation-lines blank "u2")))
      (should lines)
    ...
    (should-not (piem-jsonl-read-file junk))
    (should-not (piem-jsonl-read-session-info junk))
    ;; The leaf id exists in the would-be entries, so a nil here can
    ;; only come from the header rule.
    (should-not (piem-jsonl-navigation-lines junk "u1"))))

(ert-deftest piem-test-jsonl-session-header-bom-tolerated ()
...
reordering."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-bom"))
...
(ert-deftest piem-test-jsonl-label-folding ()
  "Labels replay in file order: set, clear, re-set; latest wins."
  (let* ((built (piem-test--jsonl-build-lines
                 (list piem-test--jsonl-header
                       (piem-test--jsonl-msg
                        "u1" nil 0 '(:role "user" :content "root"))
                       (piem-test--jsonl-entry
...
(ert-deftest piem-test-jsonl-label-clear-without-reset ()
...
(ert-deftest piem-test-jsonl-label-empty-string-clears ()
  ...
  (let* ((built (piem-test--jsonl-build-lines
         ...
         (u1 (piem-test--jsonl-find-raw
              (plist-get built :tree) "u1")))
    (should u1)
    (should-not (plist-member u1 :label))
    (should-not (plist-member u1 :labelTimestamp))))

(ert-deftest piem-test-jsonl-read-file-session-name-trim ()
  "Session names trim; a blank latest name clears to nil."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-name"))
         (padded (expand-file-name "padded.jsonl" dir))
         (cleared (expand-file-name "cleared.jsonl" dir)))
    (piem-test--write-jsonl
     padded (list piem-test--jsonl-header
                  (piem-test--jsonl-entry
                   "session_info" "s1" nil 0 :name "  Build hunt  ")))
    (should (equal (plist-get (piem-jsonl-read-file padded) :name)
                   "Build hunt"))
    (piem-test--write-jsonl
     cleared (list piem-test--jsonl-header
                   (piem-test--jsonl-entry
                    "session_info" "s1" nil 0 :name "Build")
                   (piem-test--jsonl-entry
                    "session_info" "s2" "s1" 1 :name "   ")))
    (should-not (plist-get (piem-jsonl-read-file cleared) :name))))

(ert-deftest piem-test-jsonl-roots-and-cycles ()
  ...
  (let* ((built (piem-test--jsonl-build-lines
    ...
    (should-not (piem-test--jsonl-find-raw tree "x1"))
    (should-not (piem-test--jsonl-find-raw tree "x2"))
    ...
    (should (equal (mapcar (lambda (node) (plist-get node :id))
                           (append (plist-get
                                    (piem-jsonl-project-tree tree)
                                    :tree)
                                   nil))
                   '("r1" "s1" "o1")))))

(ert-deftest piem-test-jsonl-sibling-sort-and-stable-ties ()
...
(ert-deftest piem-test-jsonl-role-mapping ()
  "Known roles pass through; anything else maps to unknown with rawRole."
  (let* ((result (piem-test--jsonl-project-lines
                  (list piem-test--jsonl-header
                        (piem-test--jsonl-msg
    ...
    (should (equal (plist-get (piem-test--jsonl-find tree "u1")
                              :role)
                   "user"))
    (should (equal (plist-get (piem-test--jsonl-find tree "c1")
                              :role)
                   "custom"))
    (should (equal (plist-get (piem-test--jsonl-find tree "b1")
                              :role)
                   "branchSummary"))
    (should (equal (plist-get (piem-test--jsonl-find tree "k1")
                              :role)
                   "compactionSummary"))
    (let ((weird (piem-test--jsonl-find tree "w1")))
...
(ert-deftest piem-test-jsonl-malformed-payloads-degrade ()
...
can carry them.  The reference throws here; we project empties."
  (let* ((result (piem-test--jsonl-project-lines
         ...
         (tree (plist-get result :tree)))
    (let ((m1 (piem-test--jsonl-find tree "m1")))
      (should (equal (plist-get m1 :role) "unknown"))
      (should-not (plist-get m1 :rawRole))
      (should (equal (plist-get m1 :preview) "")))
    (should (equal (plist-get (piem-test--jsonl-find tree "m2")
                              :preview)
                   ""))
    (should (equal (plist-get (piem-test--jsonl-find tree "m3")
                              :preview)
                   "after null"))
    (let ((m4 (piem-test--jsonl-find tree "m4")))
      (should (equal (plist-get m4 :preview) "(no content)"))
      (should (plist-member m4 :stopReason)))))

(ert-deftest piem-test-jsonl-assistant-preview-fallbacks ()
  "Assistant preview precedence: text, aborted, errorMessage, no content."
  (let* ((result (piem-test--jsonl-project-lines
                  (list piem-test--jsonl-header
                        (piem-test--jsonl-asst
    ...
    (let ((a1 (piem-test--jsonl-find tree "a1")))
    ...
    (let ((a2 (piem-test--jsonl-find tree "a2")))
    ...
    ;; not a fallback.
    (should (equal (plist-get (piem-test--jsonl-find tree "a3")
                              :preview)
                   ""))
    (should (equal (plist-get (piem-test--jsonl-find tree "a4")
                              :preview)
                   "(aborted)"))
    ;; errorMessage previews normalized, without the 200 cap.
    (let ((a5 (piem-test--jsonl-find tree "a5")))
...
(ert-deftest piem-test-jsonl-fork-branch-scoped-resolution ()
  ...
  (let* ((session (piem-jsonl-read-file
         ...
         (result (piem-jsonl-project-tree
                  (plist-get built :tree) (plist-get built :leafId)))
         (tree (plist-get result :tree))
         (ra (piem-test--jsonl-find tree "aa00000a"))
         (rb (piem-test--jsonl-find tree "aa00001a")))
...
(ert-deftest piem-test-jsonl-branch-map-sibling-isolation ()
...
the redefinition also wins) never overrides the branch one."
  (let* ((result (piem-test--jsonl-project-lines
         ...
         (tree (plist-get result :tree)))
    (let ((t1 (piem-test--jsonl-find tree "t1")))
      (should (equal (plist-get t1 :formattedToolCall) "[read: /a.py]"))
      (should (equal (plist-get t1 :toolName) "read")))
    (should (equal (plist-get (piem-test--jsonl-find tree "t2")
                              :formattedToolCall)
                   "[bash: echo hi]"))))

(ert-deftest piem-test-jsonl-projected-leaf-resolution ()
  ...
  (let* ((raw (piem-test--read-json-fixture "browse-raw.json"))
    ...
    (should-not (plist-get (piem-jsonl-project-tree tree)
                           :leafId))
    (should-not (plist-get (piem-jsonl-project-tree tree "deadbeef")
                           :leafId))
    ;; The raw leaf is a label entry: walk up the bookkeeping chain.
    (should (equal (plist-get (piem-jsonl-project-tree
...
(ert-deftest piem-test-jsonl-preview-truncation ()
  ...
  (let* ((s199 (make-string 199 ?a))
         ...
         (result (piem-test--jsonl-project-lines
                  (list piem-test--jsonl-header
                        (piem-test--jsonl-msg
         ...
         (tree (plist-get result :tree)))
    (should (equal (plist-get (piem-test--jsonl-find tree "u199")
                              :preview)
                   s199))
    (should (equal (plist-get (piem-test--jsonl-find tree "u200")
                              :preview)
                   s200))
    (should (equal (plist-get (piem-test--jsonl-find tree "u201")
                              :preview)
                   s200))
    ;; Blocks join with a space first, then slice.
    (should (equal (plist-get (piem-test--jsonl-find tree "ujoin")
                              :preview)
                   (concat (make-string 150 ?x) " " (make-string 49 ?y))))
    ;; Slice happens before trimming, so trailing padding disappears.
    (should (equal (plist-get (piem-test--jsonl-find tree "utrim")
                              :preview)
                   "keep"))))

;;;; format-tool-call unit tables

(ert-deftest piem-test-jsonl-format-tool-call-read ()
  "read previews: shortening, offset/limit arithmetic, falsy ends."
  (let ((process-environment '("HOME=/home/tester")))
    (should (equal (piem-jsonl-format-tool-call
    ...
    (should (equal (piem-jsonl-format-tool-call
                   ...
                   "[read: /tmp/x.py:10-9]"))
    (should (equal (piem-jsonl-format-tool-call "read" '())
                   "[read: ]"))
...
(ert-deftest piem-test-jsonl-format-tool-call-write-edit ()
...
(ert-deftest piem-test-jsonl-format-tool-call-bash ()
  ...
  (let ((process-environment '("HOME=/home/tester"))
    ...
    (should (equal (piem-jsonl-format-tool-call
                    "bash" '(:command ""))
                   "[bash: ]"))
    (should (equal (piem-jsonl-format-tool-call "bash" '())
                   "[bash: ]"))))

(ert-deftest piem-test-jsonl-format-tool-call-search ()
  ...
  (let ((process-environment '("HOME=/home/tester")))
    ...
    (should (equal (piem-jsonl-format-tool-call
                   ...
                   "[find: *.el in /tmp]"))
    (should (equal (piem-jsonl-format-tool-call "find" '())
                   "[find:  in .]"))
    (should (equal (piem-jsonl-format-tool-call
                    "ls" '(:path "/tmp"))
                   "[ls: /tmp]"))
    (should (equal (piem-jsonl-format-tool-call "ls" '())
                   "[ls: .]"))))

(ert-deftest piem-test-jsonl-format-tool-call-default-json ()
  ...
  (let ((process-environment '("HOME=/home/tester")))
    ...
    (let* ((args40 (list :k (make-string 32 ?x)))
      ...
      (should (equal (piem-jsonl-format-tool-call "toolx" args40)
                     (concat "[toolx: " json40 "]")))
      (should (equal (piem-jsonl-format-tool-call "toolx" args41)
...
(ert-deftest piem-test-jsonl-shorten-path ()
  "HOME (then USERPROFILE) prefixes collapse to ~, blindly."
  (let ((process-environment '("HOME=/home/tester")))
    (should (equal (piem--jsonl-shorten-path "/home/tester/x/el")
                   "~/x/el"))
    (should (equal (piem--jsonl-shorten-path "/opt/etc/conf")
                   "/opt/etc/conf"))
    ;; Blind prefix replacement, ported as-is (do not "fix").
    (let ((process-environment '("HOME=/home/tes")))
      (should (equal (piem--jsonl-shorten-path "/home/tester/x")
                     "~ter/x")))
    ;; USERPROFILE applies when HOME is unset.
    (let ((process-environment '("USERPROFILE=/up")))
      (should (equal (piem--jsonl-shorten-path "/up/f.txt")
                     "~/f.txt"))
      (should (equal (piem--jsonl-shorten-path "/var/log/x")
                     "/var/log/x")))
...
(ert-deftest piem-test-jsonl-extract-text ()
  "Strings pass through; block vectors join text blocks with spaces."
  (should (equal (piem--jsonl-extract-text "hello world" 5)
                 "hello"))
  (should (equal (piem--jsonl-extract-text "hello") "hello"))
  ;; Slicing clamps at the end of the string.
  (should (equal (piem--jsonl-extract-text "plain" 200) "plain"))
  (should (equal (piem--jsonl-extract-text "abc" 0) ""))
  (should (equal (piem--jsonl-extract-text nil) ""))
  (should (equal (piem--jsonl-extract-text "") ""))
  (should (equal (piem--jsonl-extract-text (vector)) ""))
...
(ert-deftest piem-test-jsonl-normalize-preview ()
  "Newlines and tabs become spaces and the result is trimmed; \\r stays."
  (should (equal (piem--jsonl-normalize-preview "a\nb\tc") "a b c"))
  (should (equal (piem--jsonl-normalize-preview "  x  ") "x"))
  (should (equal (piem--jsonl-normalize-preview "a\rb") "a\rb"))
  (should (equal (piem--jsonl-normalize-preview "\n\t x \t\n") "x")))

(ert-deftest piem-test-jsonl-arg-number ()
  "Numeric arg lookup: zero counts as present; JSON null reads as
present-but-null; strings do not."
  (should (equal (piem--jsonl-arg-number '(:offset 0) :offset) 0))
  (should (equal (piem--jsonl-arg-number '(:limit 0) :limit) 0))
  (should (equal (piem--jsonl-arg-number '(:offset 5) :offset) 5))
  (should (eq (piem--jsonl-arg-number '(:offset :null) :offset)
              :null))
  (should-not (piem--jsonl-arg-number '() :offset))
  (should-not (piem--jsonl-arg-number '(:offset "5") :offset))
  (should-not (piem--jsonl-arg-number '(:limit 5) :offset)))

(ert-deftest piem-test-jsonl-duplicate-id-cycle-terminates ()
...
(ert-deftest piem-test-jsonl-deep-chain ()
  "A 2500-entry chain with a forked tip builds and projects iteratively."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-deep"))
         (path (expand-file-name "deep.jsonl" dir))
         (lines (list piem-test--jsonl-header)))
    (dotimes (i 2500)
      (push (piem-test--jsonl-msg
             (format "dc%04d" i)
    ...
    (push (piem-test--jsonl-msg
          ...
          lines)
    (piem-test--write-jsonl path (nreverse lines))
    (let* ((session (piem-jsonl-read-file path))
           (built (piem-jsonl-build-tree
                   (plist-get session :entries)))
           (result (piem-jsonl-project-tree
...
(ert-deftest piem-test-jsonl-session-dir-for-cwd ()
...
wrap the result in --…-- under ROOT (default: sessions-root)."
  ...
  (let ((process-environment '("PI_CODING_AGENT_DIR=/tmp/fake-root")))
    (should (equal (expand-file-name
                    (piem-jsonl-session-dir-for-cwd "/a/b"))
...
(ert-deftest piem-test-jsonl-sessions-root ()
...
are ignored."
  ...
  (let ((process-environment '("HOME=/tmp/fake-home")))
    (should (equal (piem-jsonl-sessions-root)
  ...
  (let ((process-environment '("HOME=/tmp/fake-home")))
    (should (equal (piem-jsonl-sessions-root "/local/anchor.jsonl")
                   "/tmp/fake-home/.pi/agent/sessions/"))))

(ert-deftest piem-test-jsonl-read-session-info-fields ()
...
the five-message parse budget).  label and custom entries are ignored."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-info"))
         (path (expand-file-name "session.jsonl" dir))
    ...
    (should (equal (piem-jsonl-read-session-info path)
                   (list :path path
...
(ert-deftest piem-test-jsonl-read-session-info-keeps-name-before-malformed-tail ()
  "A malformed session_info tail does not clear the latest parseable name."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-name-tail"))
         (path (expand-file-name "session.jsonl" dir)))
    (unwind-protect
        (progn
          (piem-test--write-jsonl
          ...
          (should (equal (plist-get
                          (piem-jsonl-read-session-info path)
                          :name)
                         "Keep me")))
      (delete-directory dir t))))

(ert-deftest piem-test-jsonl-read-session-info-fallbacks ()
...
nil.  No session_info means no :name key at all."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-fb"))
    ...
    (should (equal (plist-get (piem-jsonl-read-session-info
                               late-user)
                              :firstMessage)
                   "pick me"))
    ...
    (let ((lines (list piem-test--jsonl-header)))
      (dotimes (i 6)
        (push (piem-test--jsonl-asst
      ...
      (push (piem-test--jsonl-msg
            ...
            lines)
      (piem-test--write-jsonl budget (nreverse lines)))
    (let ((info (piem-jsonl-read-session-info budget)))
    ...
    (should-not (piem-jsonl-read-session-info no-header))
    ;; Junk as the first nonblank line (a well-formed header follows
    ;; on the next line): the file is not a session file.
    (with-temp-file garbage
      (insert "{not json at all\n"
              (json-encode piem-test--jsonl-header) "\n"))
    (should-not (piem-jsonl-read-session-info garbage))
    ;; Empty file: nil.
    (with-temp-file empty)
    (should-not (piem-jsonl-read-session-info empty))
    ...
    (unless (zerop (user-uid))
      (let ((unreadable (expand-file-name "unreadable.jsonl" dir)))
        (piem-test--write-jsonl
         unreadable (list piem-test--jsonl-header))
        (set-file-modes unreadable #o000)
        (unwind-protect
            (should-not (piem-jsonl-read-session-info unreadable))
    ...
    (should-not (plist-member (piem-jsonl-read-session-info unnamed)
                              :name))))

(ert-deftest piem-test-jsonl-read-session-info-modified-iso ()
...
through `date-to-time' and orders lexicographically like time."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-iso"))
         (older (expand-file-name "older.jsonl" dir))
         (newer (expand-file-name "newer.jsonl" dir))
         (t1 (encode-time 10 30 12 2 3 2026))
         (t2 (time-add t1 90)))
    (piem-test--write-jsonl
     older (list piem-test--jsonl-header))
    (piem-test--write-jsonl
     newer (list piem-test--jsonl-header))
    (set-file-times older t1)
    (set-file-times newer t2)
    (let* ((mod-a (plist-get (piem-jsonl-read-session-info older)
                             :modified))
           (mod-b (plist-get (piem-jsonl-read-session-info newer)
                             :modified)))
...
(defun piem-test--jsonl-line-string (line)
  "Return the raw JSON string `--write-jsonl' writes for LINE."
  (json-encode (piem-test--jsonl-literalize line)))

(defun piem-test--jsonl-session-at (lines)
  "Write LINES (header first) to a temp file and return the session.
The `piem-jsonl-read-file' result is what
`piem-jsonl-navigation-target' consumes."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-nav"))
         (path (expand-file-name "session.jsonl" dir)))
    (piem-test--write-jsonl path lines)
    (piem-jsonl-read-file path)))

(ert-deftest piem-test-jsonl-navigation-target-user-message ()
...
omits :prefill entirely; the rewind to the parent still applies."
  (let* ((long (make-string 300 ?z))
         (session (piem-test--jsonl-session-at
                   (list piem-test--jsonl-header
                         (piem-test--jsonl-msg
                          "u1" nil 0 '(:role "user" :content "root"))
                         (piem-test--jsonl-msg
...
Empty content omits :prefill; :customType never becomes the prefill."
  (let ((session (piem-test--jsonl-session-at
                  (list piem-test--jsonl-header
                        (piem-test--jsonl-msg
                         "u1" nil 0 '(:role "user" :content "root"))
                        (piem-test--jsonl-entry
    ...
    (should (equal (piem-jsonl-navigation-target session "c1")
                   (list :leaf-id "u1" :prefill "re-edit me" :current-p nil)))
    (should (equal (piem-jsonl-navigation-target session "c2")
                   (list :leaf-id "c1" :prefill "part onepart two"
                         :current-p nil)))
    ;; Empty content: rewind to the parent, no :prefill key at all.
    (should (equal (piem-jsonl-navigation-target session "c3")
                   (list :leaf-id "c2" :current-p nil)))))

(ert-deftest piem-test-jsonl-navigation-target-non-user-self-leaf ()
...
resolved-position truth table; earlier self targets are not."
  (let ((session (piem-test--jsonl-session-at
    ...
    (dolist (id '("a1" "t1" "k1" "mo1" "th1" "bs1"))
      (should (equal (piem-jsonl-navigation-target session id)
                     (list :leaf-id id :current-p (equal id "bs1")))))))

(ert-deftest piem-test-jsonl-navigation-target-unknown-nil ()
  "An id no entry carries reads as nil — no target, no signal."
  (let ((session (piem-test--jsonl-session-at
                  (list piem-test--jsonl-header
                        (piem-test--jsonl-msg
                         "u1" nil 0 '(:role "user" :content "root"))))))
    (should-not (piem-jsonl-navigation-target session "deadbeef"))))

(ert-deftest piem-test-jsonl-navigation-target-current-position ()
...
chain resolves both sides to nothing."
  ...
  (let ((session (piem-test--jsonl-session-at
    ...
    (should (equal (piem-jsonl-navigation-target session "a1")
  ...
  (let ((session (piem-test--jsonl-session-at
    ...
    (should (equal (piem-jsonl-navigation-target session "b1")
  ...
  (let ((session (piem-test--jsonl-session-at
    ...
    (should (equal (piem-jsonl-navigation-target session "l1")
                   (list :leaf-id "l1" :current-p t)))))

(ert-deftest piem-test-jsonl-navigation-lines-chain-to-end ()
...
off-branch sibling lead."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-navord"))
         (path (expand-file-name "session.jsonl" dir))
         (header piem-test--jsonl-header)
         (lines
          (list header
                (piem-test--jsonl-msg
                 "u1" nil 0 '(:role "user" :content "root"))
    ...
    (should (equal (piem-jsonl-navigation-lines path "u2")
                   (vconcat
                    (mapcar #'piem-test--jsonl-line-string
                            (list (nth 0 lines) (nth 2 lines) (nth 3 lines)
                                  (nth 1 lines) (nth 4 lines) (nth 5 lines)
                                  (nth 6 lines) (nth 7 lines))))))))

(ert-deftest piem-test-jsonl-navigation-lines-consecutive-cross-branch-navigation ()
...
than finish on that shared root."
  (let* ((dir (piem-test--make-temp-directory
    ...
    (let* ((original
           ...
           (first (piem-jsonl-navigation-lines path "a2")))
      ...
      (let ((coding-system-for-write 'no-conversion))
        (write-region
         ...
         nil path nil 0))
      (let ((second (piem-jsonl-navigation-lines path "b2")))
...
(ert-deftest piem-test-jsonl-navigation-lines-preserve-malformed-and-blank ()
...
lines carry no trailing newline — the caller joins and terminates."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-navml"))
  ...
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-navnil"))
    ...
    (piem-test--write-jsonl
     ...
     (list piem-test--jsonl-header
           (piem-test--jsonl-msg
            "u1" nil 0 '(:role "user" :content "root"))))
    (should-not (piem-jsonl-navigation-lines missing "u1"))
    (should-not (piem-jsonl-navigation-lines empty "u1"))
    (should-not (piem-jsonl-navigation-lines headerless "u1"))
    (should-not (piem-jsonl-navigation-lines valid "deadbeef"))
    (should-not (piem-jsonl-navigation-lines valid nil))))

(ert-deftest piem-test-jsonl-navigation-lines-identity ()
...
unchanged."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-navid"))
    ...
    (piem-test--write-jsonl plain plain-lines)
    (should (equal (piem-jsonl-navigation-lines plain "u2")
                   (vconcat (mapcar #'piem-test--jsonl-line-string
                                    plain-lines)))))
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-navlab"))
    ...
    (piem-test--write-jsonl labeled labeled-lines)
    (should (equal (piem-jsonl-navigation-lines labeled "l1")
                   (vconcat (mapcar #'piem-test--jsonl-line-string
                                    labeled-lines))))))

(ert-deftest piem-test-jsonl-navigation-lines-large-chain-linear ()
...
quadratic nth-per-access version measured 16s+ against."
  (let* ((dir (piem-test--make-temp-directory "pi-jsonl-navbig"))
    ...
    (let* ((start (float-time))
           (lines (piem-jsonl-navigation-lines
...
(provide 'piem-jsonl-test)
;;; piem-jsonl-test.el ends here
